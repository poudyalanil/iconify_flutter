///Discover all icons of this iconset at https://andronasef.ninja/iconify_flutter/collection/pajamas
class Pajamas {
/// Icon Data of abuse

  static const String abuse =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M5.308 1.5 1.5 5.308v5.384L5.308 14.5h5.384l3.808-3.808V5.308L10.692 1.5H5.308ZM11.314 0H4.686L0 4.686v6.628L4.686 16h6.628L16 11.314V4.686L11.314 0ZM9 11a1 1 0 11-2 0 1 1 0 012 0Zm-.25-6.25a.75.75 0 00-1.5 0v3.5a.75.75 0 001.5 0v-3.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of accessibility

  static const String accessibility =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M13.434 4.187a.874.874 0 01-.67 1.04l-2.718.587a1 1 0 00-.79.977v.88a8 8 0 00.507 2.8l1.624 4.348a.874.874 0 11-1.638.612l-1.554-4.16a.5.5 0 00-.937 0l-1.554 4.16a.874.874 0 01-1.638-.611l1.625-4.35a8 8 0 00.506-2.8v-.879a1 1 0 00-.789-.977L2.69 5.226a.874.874 0 01.37-1.709l3.822.826a4 4 0 001.69 0l3.822-.826a.874.874 0 011.04.67ZM7.684 0a1.749 1.749 0 110 3.497 1.749 1.749 0 010-3.497Z"/></svg>';
/// Icon Data of account

  static const String account =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4 6.5a1.5 1.5 0 100-3 1.5 1.5 0 000 3ZM7 5a2.99 2.99 0 01-.87 2.113A3.997 3.997 0 018 10.5V12a2 2 0 01-2 2H2a2 2 0 01-2-2v-1.5c0-1.427.747-2.679 1.87-3.387A3 3 0 117 5Zm-5.5 5.5a2.5 2.5 0 015 0V12a.5.5 0 01-.5.5H2a.5.5 0 01-.5-.5v-1.5Zm12.251-7.165c.21.122.462.163.685.07l.313-.133a.5.5 0 01.628.21l.308.535a.5.5 0 01-.13.649l-.272.205c-.193.146-.283.387-.283.629s.09.483.283.629l.271.205a.5.5 0 01.131.649l-.308.534a.5.5 0 01-.628.21l-.313-.131c-.223-.094-.475-.053-.685.069-.209.121-.374.32-.404.56l-.042.337a.5.5 0 01-.496.438h-.618a.5.5 0 01-.496-.438l-.042-.337c-.03-.24-.195-.439-.404-.56-.21-.122-.462-.163-.685-.07l-.313.133a.5.5 0 01-.628-.21l-.308-.535a.5.5 0 01.13-.649l.272-.205C9.91 5.983 10 5.742 10 5.5s-.09-.483-.283-.629l-.271-.205a.5.5 0 01-.131-.649l.308-.534a.5.5 0 01.628-.21l.313.131c.223.094.475.053.685-.069.209-.121.374-.32.404-.56l.042-.337A.5.5 0 0112.191 2h.618a.5.5 0 01.496.438l.042.337c.03.24.195.439.404.56ZM13.5 5.5a1 1 0 11-2 0 1 1 0 012 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of admin

  static const String admin =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="m7.879 5 1.06-1.06 1.421-1.422a3.5 3.5 0 00-3.653 4.674l.326.897-.675.674-3.797 3.798a.621.621 0 10.878.878l3.798-3.797.674-.675.897.325a3.5 3.5 0 004.674-3.653L12.06 7.062 11 8.12 9.94 7.06l-1-1L7.878 5Zm6.173-1.93A4.987 4.987 0 0115 6a5 5 0 01-6.703 4.703L4.5 14.5a2.121 2.121 0 01-3-3l3.797-3.797A5 5 0 0113 2l-1.076 1.076-.863.863L10 5l1 1 1.06-1.06.864-.864L14 3l.052.07Z" clip-rule="evenodd"/></svg>';
/// Icon Data of api

  static const String api =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M11.876.165a.25.25 0 01.296-.146l1.022.279a.25.25 0 01.181.276l-.127.893c.551.317.995.774 1.299 1.31l.895-.12a.25.25 0 01.275.185l.27 1.024a.25.25 0 01-.15.295l-.836.336a3.501 3.501 0 01-.485 1.781l.55.715a.25.25 0 01-.022.33l-.751.745a.25.25 0 01-.33.02l-.711-.558a3.496 3.496 0 01-1.784.47l-.344.835a.25.25 0 01-.296.146l-1.022-.278a.25.25 0 01-.182-.277l.128-.893a3.496 3.496 0 01-1.299-1.31l-.895.12a.25.25 0 01-.275-.185l-.27-1.024a.25.25 0 01.15-.295l.836-.336a3.504 3.504 0 01.485-1.781l-.55-.715a.25.25 0 01.022-.33l.751-.745a.25.25 0 01.33-.02l.711.558A3.496 3.496 0 0111.532 1l.344-.835Zm1.554 4.86a2 2 0 11-3.86-1.05 2 2 0 013.86 1.05ZM5.777 6.22A.25.25 0 005.53 6H4.471a.25.25 0 00-.248.219l-.11.88a3.977 3.977 0 00-1.244.515l-.7-.544a.25.25 0 00-.33.02l-.749.749a.25.25 0 00-.02.33l.544.7a3.977 3.977 0 00-.515 1.244l-.88.11A.25.25 0 000 10.47v1.058a.25.25 0 00.219.248l.88.11c.101.448.278.867.515 1.244l-.544.7a.25.25 0 00.02.33l.749.749a.25.25 0 00.33.02l.7-.544c.377.237.796.414 1.244.515l.11.88a.25.25 0 00.247.22h1.058a.25.25 0 00.248-.219l.11-.88a3.974 3.974 0 001.244-.515l.7.544a.25.25 0 00.33-.02l.749-.749a.25.25 0 00.02-.33l-.544-.7c.237-.377.414-.796.515-1.244l.88-.11a.25.25 0 00.22-.247v-1.058a.25.25 0 00-.219-.248l-.88-.11a3.977 3.977 0 00-.515-1.244l.544-.7a.25.25 0 00-.02-.33l-.75-.75a.25.25 0 00-.33-.02l-.7.544A3.977 3.977 0 005.887 7.1l-.11-.88ZM7.5 11a2.5 2.5 0 11-5 0 2.5 2.5 0 015 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of appearance

  static const String appearance =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="m14.489 8.388-.001.006a.115.115 0 01-.027.028.428.428 0 01-.264.082h-3.186c-3.118 0-4.68 3.77-2.476 5.974a6.5 6.5 0 115.953-6.09Zm-.292 1.616c.913 0 1.736-.618 1.79-1.529a8 8 0 10-7.032 7.468c1.243-.147 1.527-1.639.641-2.525-1.26-1.26-.367-3.414 1.415-3.414h3.186ZM10 5a1 1 0 11-2 0 1 1 0 012 0ZM6 7a1 1 0 100-2 1 1 0 000 2Zm0 2a1 1 0 11-2 0 1 1 0 012 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of applications

  static const String applications =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M3 2.5h2a.5.5 0 01.5.5v2a.5.5 0 01-.5.5H3a.5.5 0 01-.5-.5V3a.5.5 0 01.5-.5ZM1 3a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H3a2 2 0 01-2-2V3Zm2 7.5h2a.5.5 0 01.5.5v2a.5.5 0 01-.5.5H3a.5.5 0 01-.5-.5v-2a.5.5 0 01.5-.5ZM1 11a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H3a2 2 0 01-2-2v-2Zm12-8.5h-2a.5.5 0 00-.5.5v2a.5.5 0 00.5.5h2a.5.5 0 00.5-.5V3a.5.5 0 00-.5-.5ZM11 1a2 2 0 00-2 2v2a2 2 0 002 2h2a2 2 0 002-2V3a2 2 0 00-2-2h-2Zm0 9.5h2a.5.5 0 01.5.5v2a.5.5 0 01-.5.5h-2a.5.5 0 01-.5-.5v-2a.5.5 0 01.5-.5ZM9 11a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2Z" clip-rule="evenodd"/></svg>';
/// Icon Data of approval

  static const String approval =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4 6.5a1.5 1.5 0 100-3 1.5 1.5 0 000 3ZM7 5a2.99 2.99 0 01-.87 2.113A3.997 3.997 0 018 10.5V12a2 2 0 01-2 2H2a2 2 0 01-2-2v-1.5c0-1.427.747-2.679 1.87-3.387A3 3 0 117 5Zm-5.5 5.5a2.5 2.5 0 015 0V12a.5.5 0 01-.5.5H2a.5.5 0 01-.5-.5v-1.5Zm14.28-5.22a.75.75 0 00-1.06-1.06L12 6.94l-1.22-1.22a.75.75 0 10-1.06 1.06l1.75 1.75a.75.75 0 001.06 0l3.25-3.25Z" clip-rule="evenodd"/></svg>';
/// Icon Data of approval_solid

  static const String approval_solid =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4 6.5a1.5 1.5 0 100-3 1.5 1.5 0 000 3ZM7 5a2.99 2.99 0 01-.87 2.113A3.997 3.997 0 018 10.5V12a2 2 0 01-2 2H2a2 2 0 01-2-2v-1.5c0-1.427.747-2.679 1.87-3.387A3 3 0 117 5Zm8.78.28a.75.75 0 00-1.06-1.06L12 6.94l-1.22-1.22a.75.75 0 10-1.06 1.06l1.75 1.75a.75.75 0 001.06 0l3.25-3.25Z" clip-rule="evenodd"/></svg>';
/// Icon Data of archive

  static const String archive =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M1.5 4.5v-2h13v2h-13ZM1 6a1 1 0 01-1-1V2a1 1 0 011-1h14a1 1 0 011 1v3a1 1 0 01-1 1v8a1 1 0 01-1 1H2a1 1 0 01-1-1V6Zm1.5 0v7.5h11V6h-11ZM4 8.75A.75.75 0 014.75 8h6.5a.75.75 0 010 1.5h-6.5A.75.75 0 014 8.75Z" clip-rule="evenodd"/></svg>';
/// Icon Data of arrow_down

  static const String arrow_down =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M10.72 10.159a.75.75 0 111.06 1.06l-3.25 3.25L8 15l-.53-.53-3.25-3.25a.75.75 0 011.06-1.061l1.97 1.97V1.75a.75.75 0 111.5 0v10.379l1.97-1.97Z" clip-rule="evenodd"/></svg>';
/// Icon Data of arrow_left

  static const String arrow_left =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M5.841 5.28a.75.75 0 00-1.06-1.06L1.53 7.47 1 8l.53.53 3.25 3.25a.75.75 0 001.061-1.06l-1.97-1.97H14.25a.75.75 0 000-1.5H3.871l1.97-1.97Z" clip-rule="evenodd"/></svg>';
/// Icon Data of arrow_right

  static const String arrow_right =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M10.159 10.72a.75.75 0 101.06 1.06l3.25-3.25L15 8l-.53-.53-3.25-3.25a.75.75 0 00-1.061 1.06l1.97 1.97H1.75a.75.75 0 100 1.5h10.379l-1.97 1.97Z" clip-rule="evenodd"/></svg>';
/// Icon Data of arrow_up

  static const String arrow_up =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M10.72 5.841a.75.75 0 101.06-1.06L8.53 1.53 8 1l-.53.53-3.25 3.25a.75.75 0 001.06 1.061l1.97-1.97V14.25a.75.75 0 001.5 0V3.871l1.97 1.97Z" clip-rule="evenodd"/></svg>';
/// Icon Data of assignee

  static const String assignee =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4 6.5a1.5 1.5 0 100-3 1.5 1.5 0 000 3ZM7 5a2.99 2.99 0 01-.87 2.113A3.997 3.997 0 018 10.5V12a2 2 0 01-2 2H2a2 2 0 01-2-2v-1.5c0-1.427.747-2.679 1.87-3.387A3 3 0 117 5Zm-5.5 5.5a2.5 2.5 0 015 0V12a.5.5 0 01-.5.5H2a.5.5 0 01-.5-.5v-1.5ZM13 10a.75.75 0 01-.75-.75v-1.5h-1.5a.75.75 0 010-1.5h1.5v-1.5a.75.75 0 011.5 0v1.5h1.5a.75.75 0 010 1.5h-1.5v1.5A.75.75 0 0113 10Z" clip-rule="evenodd"/></svg>';
/// Icon Data of at

  static const String at =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M9.774 1.747a6.5 6.5 0 101.142 12.062.75.75 0 01.673 1.34A8 8 0 1116 8v1.25a2.75 2.75 0 01-5.072 1.475A4 4 0 1112 8v1.25a1.25 1.25 0 002.5 0V8a6.5 6.5 0 00-4.726-6.253ZM10.5 8a2.5 2.5 0 10-5 0 2.5 2.5 0 005 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of attention

  static const String attention =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="m6.04 8-3.174 5.5a1 1 0 00.866 1.5h2.173a1 1 0 00.724-.31L13 8 6.629 1.31A1 1 0 005.905 1H3.732a1 1 0 00-.866 1.5L6.04 8Zm4.889 0L5.69 13.5H4.598L7.34 8.75 7.773 8l-.433-.75L4.598 2.5H5.69L10.93 8Z" clip-rule="evenodd"/></svg>';
/// Icon Data of attention_solid

  static const String attention_solid =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" d="M13 8 6.629 1.31A1 1 0 005.905 1H3.732a1 1 0 00-.866 1.5L6.04 8l-3.175 5.5a1 1 0 00.866 1.5h2.173a1 1 0 00.724-.31L13 8Z"/></svg>';
/// Icon Data of attention_solid_sm

  static const String attention_solid_sm =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" d="M12.004 8 7.538 3.31A1 1 0 006.813 3H5.732a1 1 0 00-.866 1.5L6.886 8l-2.02 3.5a1 1 0 00.866 1.5h1.081a1 1 0 00.725-.31L12.004 8Z"/></svg>';
/// Icon Data of autoplay

  static const String autoplay =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M11.194.665a8 8 0 104.194 10.404.75.75 0 00-1.385-.575 6.5 6.5 0 11-.526-5.994H11.75a.75.75 0 000 1.5H16V1.75a.75.75 0 00-1.5 0v1.586a8 8 0 00-3.306-2.67Zm-.423 6.913a.5.5 0 010 .844l-4.003 2.54A.5.5 0 016 10.538V5.461a.5.5 0 01.768-.422l4.003 2.539Z" clip-rule="evenodd"/></svg>';
/// Icon Data of bitbucket

  static const String bitbucket =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" d="M1.454 1.816a.452.452 0 00-.345.153.435.435 0 00-.103.358L2.91 13.684c.049.287.3.498.596.5h9.135a.447.447 0 00.449-.37L14.994 2.33a.435.435 0 00-.1-.358.452.452 0 00-.342-.156H1.454Zm8.018 8.208H6.557l-.79-4.05h4.412l-.707 4.05Z"/></svg>';
/// Icon Data of bold

  static const String bold =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4 2a1 1 0 00-1 1v10a1 1 0 001 1h5.5a3.5 3.5 0 001.852-6.47A3.5 3.5 0 008.5 2H4Zm4.5 5a1.5 1.5 0 100-3H5v3h3.5ZM5 9v3h4.5a1.5 1.5 0 000-3H5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of book

  static const String book =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M2.25 2a2 2 0 012-2H13a.75.75 0 01.75.75v10.5A.75.75 0 0113 12H7.5v1.5H13a.75.75 0 010 1.5H7.5v1H6v-1H4.5a2.25 2.25 0 01-2.25-2.25V2ZM6 13.5V12H4.5a.75.75 0 000 1.5H6Zm6.25-12v9H4.5c-.263 0-.515.045-.75.128V2a.5.5 0 01.5-.5h8Z" clip-rule="evenodd"/></svg>';
/// Icon Data of bookmark

  static const String bookmark =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="m8 9.293 1.06 1.06 3.44 3.44V2a.5.5 0 00-.5-.5H4a.5.5 0 00-.5.5v11.793l3.44-3.44L8 9.293Zm1.06 3.182 3.233 3.232c.63.63 1.707.184 1.707-.707V2a2 2 0 00-2-2H4a2 2 0 00-2 2v13c0 .89 1.077 1.337 1.707.707l3.232-3.232L8 11.415l1.06 1.06Z" clip-rule="evenodd"/></svg>';
/// Icon Data of branch

  static const String branch =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M11.5 4.5a1 1 0 100-2 1 1 0 000 2Zm2.5-1a2.501 2.501 0 01-1.872 2.42A3.502 3.502 0 018.75 8.5h-1.5a2 2 0 00-1.965 1.626 2.501 2.501 0 11-1.535-.011v-4.23a2.501 2.501 0 111.5 0v1.742a3.484 3.484 0 012-.627h1.5a2 2 0 001.823-1.177A2.5 2.5 0 1114 3.5Zm-8.5 9a1 1 0 11-2 0 1 1 0 012 0Zm0-9a1 1 0 11-2 0 1 1 0 012 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of branch_deleted

  static const String branch_deleted =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M10.28 1.22a.75.75 0 10-1.06 1.06l1.22 1.22-1.22 1.22a.75.75 0 001.06 1.06l1.22-1.22 1.22 1.22a.75.75 0 101.06-1.06L12.56 3.5l1.22-1.22a.75.75 0 00-1.06-1.06L11.5 2.44l-1.22-1.22Zm-2.5 6a.75.75 0 010 1.06l-2.045 2.046A2.499 2.499 0 014.5 15a2.5 2.5 0 01-.75-4.886V5.886a2.501 2.501 0 111.5 0v2.803l1.47-1.47a.75.75 0 011.06 0ZM5.5 12.5a1 1 0 11-2 0 1 1 0 012 0Zm0-9a1 1 0 11-2 0 1 1 0 012 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of brand_zoom

  static const String brand_zoom =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M1.45 3.333c-.801 0-1.45.65-1.45 1.45v4.986c0 1.6 1.298 2.898 2.898 2.898h6.986c.8 0 1.45-.649 1.45-1.45V6.233a2.9 2.9 0 00-2.899-2.899H1.45ZM16 4.643v6.715c0 .544-.618.86-1.059.539l-2.059-1.498a1.333 1.333 0 01-.549-1.078V6.679c0-.427.204-.827.55-1.078l2.058-1.498a.667.667 0 011.059.54Z" clip-rule="evenodd"/></svg>';
/// Icon Data of bug

  static const String bug =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4 1.75A.75.75 0 014.75 1h.5C6.216 1 7 1.784 7 2.75v.35a5.023 5.023 0 012 0v-.35C9 1.784 9.784 1 10.75 1h.5a.75.75 0 010 1.5h-.5a.25.25 0 00-.25.25v.919A5.02 5.02 0 0112.584 6h1.666a.75.75 0 010 1.5h-1.275c.017.164.025.331.025.5v.5h1.25a.75.75 0 010 1.5H13c0 .342-.034.677-.1 1h1.35a.75.75 0 010 1.5h-1.919A4.998 4.998 0 018 15a4.998 4.998 0 01-4.331-2.5H1.75a.75.75 0 010-1.5H3.1a5.022 5.022 0 01-.1-1H1.75a.75.75 0 010-1.5H3V8c0-.169.008-.336.025-.5H1.75a.75.75 0 010-1.5h1.666A5.02 5.02 0 015.5 3.669V2.75a.25.25 0 00-.25-.25h-.5A.75.75 0 014 1.75ZM11.5 8v2a3.501 3.501 0 01-2.75 3.42V7.5h2.715c.023.163.035.33.035.5Zm-4.25-.5v5.92A3.501 3.501 0 014.5 10V8c0-.17.012-.337.035-.5H7.25ZM10.873 6H5.127A3.496 3.496 0 018 4.5c1.19 0 2.24.593 2.873 1.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of bulb

  static const String bulb =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4.17 8.533C2.21 5.5 4.388 1.5 8 1.5s5.79 4 3.83 7.033L9.592 12H6.408L4.17 8.533ZM5 12.584 2.91 9.347C.305 5.315 3.2 0 8 0s7.694 5.315 5.09 9.347L11 12.584V14a2 2 0 01-2 2H7a2 2 0 01-2-2v-1.416Zm1.5.916v.5a.5.5 0 00.5.5h2a.5.5 0 00.5-.5v-.5h-3Z" clip-rule="evenodd"/></svg>';
/// Icon Data of bullhorn

  static const String bullhorn =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="m8.5 8.974 6 2.333V2.693l-6 2.333v3.948Zm6.138-7.944L7 4H3a3 3 0 000 6h.969l1.066 4.445A2.029 2.029 0 007.008 16c1.524 0 2.504-1.618 1.797-2.969l-1.53-2.924 7.363 2.863A1 1 0 0016 12.038V1.962a1 1 0 00-1.362-.932ZM5.51 10h.016l1.95 3.726a.529.529 0 11-.983.369L5.511 10ZM3 5.5h4v3H3a1.5 1.5 0 110-3Z" clip-rule="evenodd"/></svg>';
/// Icon Data of calendar

  static const String calendar =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4.75 1a.75.75 0 01.75.75V3h5V1.75a.75.75 0 011.5 0V3h2a1 1 0 011 1v10a1 1 0 01-1 1H2a1 1 0 01-1-1V4a1 1 0 011-1h2V1.75A.75.75 0 014.75 1ZM2.5 4.5V6h11V4.5h-11Zm0 9v-6h11v6h-11ZM11 11a1 1 0 100-2 1 1 0 000 2Z" clip-rule="evenodd"/></svg>';
/// Icon Data of cancel

  static const String cancel =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M12.035 13.096a6.5 6.5 0 01-9.131-9.131l9.131 9.131Zm1.061-1.06L3.965 2.903a6.5 6.5 0 019.131 9.131ZM16 8A8 8 0 110 8a8 8 0 0116 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of canceled_circle

  static const String canceled_circle =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M14.5 8a6.5 6.5 0 11-13 0 6.5 6.5 0 0113 0ZM16 8A8 8 0 110 8a8 8 0 0116 0ZM5.28 4.22a.75.75 0 00-1.06 1.06l6.5 6.5a.75.75 0 101.06-1.06l-6.5-6.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of car

  static const String car =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M11 2.5H5a.5.5 0 00-.5.5v2h7V3a.5.5 0 00-.5-.5ZM1.75 4.25H3V3a2 2 0 012-2h6a2 2 0 012 2v1.25h1.25a.75.75 0 010 1.5h-.918A3.995 3.995 0 0115 9v5a1 1 0 11-2 0v-1H3v1a1 1 0 11-2 0V9c0-1.339.658-2.524 1.668-3.25H1.75a.75.75 0 010-1.5ZM12 11.5h1.5V9A2.5 2.5 0 0011 6.5H5A2.5 2.5 0 002.5 9v2.5H4V11a1 1 0 011-1h6a1 1 0 011 1v.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of chart

  static const String chart =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M3.75 1a.75.75 0 01.75.75v9.75h9.75a.75.75 0 010 1.5H4.5v1.25a.75.75 0 01-1.5 0V13H1.75a.75.75 0 010-1.5H3V1.75A.75.75 0 013.75 1Zm9.75 4.75a.75.75 0 00-1.5 0v3.5a.75.75 0 001.5 0v-3.5ZM9.75 3a.75.75 0 01.75.75v5.5a.75.75 0 01-1.5 0v-5.5A.75.75 0 019.75 3ZM7.5 7.75a.75.75 0 00-1.5 0v1.5a.75.75 0 001.5 0v-1.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of check

  static const String check =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M12.78 4.62a.75.75 0 010 1.06l-6.097 6.097a.75.75 0 01-1.069-.009L3.211 9.284a.75.75 0 111.078-1.043l1.873 1.936L11.72 4.62a.75.75 0 011.06 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of check_circle

  static const String check_circle =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M14.5 8a6.5 6.5 0 11-13 0 6.5 6.5 0 0113 0ZM16 8A8 8 0 110 8a8 8 0 0116 0Zm-4.22-1.72a.75.75 0 00-1.06-1.06L6.75 9.19 5.53 7.97a.75.75 0 00-1.06 1.06l1.75 1.75a.75.75 0 001.06 0l4.5-4.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of check_circle_dashed

  static const String check_circle_dashed =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M6.701 1.08a.75.75 0 00-.934-.5A7.75 7.75 0 00.573 5.785a.75.75 0 101.438.429 6.25 6.25 0 014.188-4.199.75.75 0 00.502-.934Zm2.598 0a.75.75 0 01.934-.501 7.75 7.75 0 015.194 5.206.75.75 0 11-1.438.429 6.25 6.25 0 00-4.188-4.199.75.75 0 01-.502-.934Zm2.481 4.14a.75.75 0 010 1.06l-4.5 4.5a.75.75 0 01-1.06 0L4.47 9.03a.75.75 0 011.06-1.06l1.22 1.22 3.97-3.97a.75.75 0 011.06 0Zm-1.547 10.2a.75.75 0 11-.432-1.436 6.251 6.251 0 004.188-4.199.75.75 0 111.438.429 7.75 7.75 0 01-5.194 5.206Zm-4.466 0a.75.75 0 10.432-1.436 6.25 6.25 0 01-4.188-4.199.75.75 0 10-1.438.429 7.75 7.75 0 005.194 5.206Z" clip-rule="evenodd"/></svg>';
/// Icon Data of check_circle_filled

  static const String check_circle_filled =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M8 16A8 8 0 108 0a8 8 0 000 16Zm3.78-9.72a.75.75 0 00-1.06-1.06L6.75 9.19 5.53 7.97a.75.75 0 00-1.06 1.06l1.75 1.75a.75.75 0 001.06 0l4.5-4.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of check_sm

  static const String check_sm =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="12" height="12" preserveAspectRatio="xMidYMid meet" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M10.78 2.62a.75.75 0 010 1.06L4.683 9.777a.75.75 0 01-1.069-.009L1.211 7.284a.75.75 0 011.078-1.043l1.873 1.936L9.72 2.62a.75.75 0 011.06 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of cherry_pick_commit

  static const String cherry_pick_commit =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M8.75.75a.75.75 0 00-1.5 0v2.528a10.555 10.555 0 00-2.55.51 9.11 9.11 0 00-.952.375 6.415 6.415 0 00-.337.167l-.022.012-.007.004-.002.002h-.001L3.75 5l-.372-.651A.75.75 0 003 5v2.25a.75.75 0 001.5 0V5.47A9.002 9.002 0 018 4.75a9.001 9.001 0 013.5.72v1.78a.75.75 0 001.5 0V5a.75.75 0 00-.378-.651L12.25 5l.372-.651-.002-.001-.002-.002-.007-.004-.022-.012a6.42 6.42 0 00-.337-.168 9.315 9.315 0 00-.952-.374 10.555 10.555 0 00-2.55-.51V.75ZM10.5 12a2.5 2.5 0 11-5 0 2.5 2.5 0 015 0Zm1.43.75a4.001 4.001 0 01-7.86 0H.75a.75.75 0 010-1.5h3.32a4.001 4.001 0 017.86 0h3.32a.75.75 0 010 1.5h-3.32Z" clip-rule="evenodd"/></svg>';
/// Icon Data of chevron_double_lg_left

  static const String chevron_double_lg_left =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M7.721 2.22a.75.75 0 011.061 1.06L4.061 8.002l4.721 4.721a.75.75 0 01-1.06 1.061L2.47 8.532a.75.75 0 010-1.06L7.722 2.22Zm5 0a.75.75 0 011.061 1.06L9.061 8.002l4.721 4.721a.75.75 0 01-1.06 1.061L7.47 8.532a.75.75 0 010-1.06l5.252-5.252Z" clip-rule="evenodd"/></svg>';
/// Icon Data of chevron_double_lg_right

  static const String chevron_double_lg_right =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M3.53 2.22a.75.75 0 00-1.06 1.06l4.72 4.722-4.72 4.721a.75.75 0 001.06 1.061l5.252-5.252a.75.75 0 000-1.06L3.53 2.22Zm5 0a.75.75 0 00-1.06 1.06l4.721 4.722-4.721 4.721a.75.75 0 001.06 1.061l5.252-5.252a.75.75 0 000-1.06L8.53 2.22Z" clip-rule="evenodd"/></svg>';
/// Icon Data of chevron_down

  static const String chevron_down =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4.22 6.22a.75.75 0 011.06 0L8 8.94l2.72-2.72a.75.75 0 111.06 1.06l-3.25 3.25a.75.75 0 01-1.06 0L4.22 7.28a.75.75 0 010-1.06Z" clip-rule="evenodd"/></svg>';
/// Icon Data of chevron_left

  static const String chevron_left =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M9.78 4.22a.75.75 0 010 1.06L7.06 8l2.72 2.72a.75.75 0 11-1.06 1.06L5.47 8.53a.75.75 0 010-1.06l3.25-3.25a.75.75 0 011.06 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of chevron_lg_down

  static const String chevron_lg_down =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M2.22 5.22a.75.75 0 000 1.06l5.252 5.252a.75.75 0 001.06 0l5.252-5.252a.75.75 0 10-1.06-1.06L8.001 9.94 3.28 5.22a.75.75 0 00-1.06 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of chevron_lg_left

  static const String chevron_lg_left =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M10.78 2.22a.75.75 0 00-1.06 0L4.468 7.472a.75.75 0 000 1.06l5.252 5.252a.75.75 0 101.06-1.06L6.06 8.001l4.72-4.721a.75.75 0 000-1.06Z" clip-rule="evenodd"/></svg>';
/// Icon Data of chevron_lg_right

  static const String chevron_lg_right =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M5.22 2.22a.75.75 0 011.06 0l5.252 5.252a.75.75 0 010 1.06L6.28 13.784a.75.75 0 11-1.06-1.06l4.72-4.723L5.22 3.28a.75.75 0 010-1.06Z" clip-rule="evenodd"/></svg>';
/// Icon Data of chevron_lg_up

  static const String chevron_lg_up =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M2.22 10.78a.75.75 0 010-1.06l5.252-5.252a.75.75 0 011.06 0l5.252 5.252a.75.75 0 11-1.06 1.06L8.001 6.06 3.28 10.78a.75.75 0 01-1.06 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of chevron_right

  static const String chevron_right =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M6.22 4.22a.75.75 0 000 1.06L8.94 8l-2.72 2.72a.75.75 0 101.06 1.06l3.25-3.25a.75.75 0 000-1.06L7.28 4.22a.75.75 0 00-1.06 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of chevron_up

  static const String chevron_up =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4.22 9.78a.75.75 0 001.06 0L8 7.06l2.72 2.72a.75.75 0 101.06-1.06L8.53 5.47a.75.75 0 00-1.06 0L4.22 8.72a.75.75 0 000 1.06Z" clip-rule="evenodd"/></svg>';
/// Icon Data of clear

  static const String clear =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M8 16A8 8 0 108 0a8 8 0 000 16ZM4.22 4.22a.75.75 0 011.06 0L8 6.94l2.72-2.72a.75.75 0 111.06 1.06L9.06 8l2.72 2.72a.75.75 0 11-1.06 1.06L8 9.06l-2.72 2.72a.75.75 0 01-1.06-1.06L6.94 8 4.22 5.28a.75.75 0 010-1.06Z" clip-rule="evenodd"/></svg>';
/// Icon Data of clear_all

  static const String clear_all =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M15.963 7.23A8 8 0 01.044 8.841a.75.75 0 011.492-.158 6.5 6.5 0 109.964-6.16V4.25a.75.75 0 01-1.5 0V0h4.25a.75.75 0 010 1.5h-1.586a8.001 8.001 0 013.299 5.73ZM7 2a1 1 0 100-2 1 1 0 000 2Zm-2.25.25a1 1 0 11-2 0 1 1 0 012 0ZM1.5 6a1 1 0 100-2 1 1 0 000 2Z" clip-rule="evenodd"/></svg>';
/// Icon Data of clock

  static const String clock =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M14.5 8a6.5 6.5 0 11-13 0 6.5 6.5 0 0113 0ZM16 8A8 8 0 110 8a8 8 0 0116 0ZM8.75 3.75a.75.75 0 00-1.5 0v4.56l.22.22 2.254 2.254a.75.75 0 101.06-1.06L8.75 7.689V3.75Z" clip-rule="evenodd"/></svg>';
/// Icon Data of close

  static const String close =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4.28 3.22a.75.75 0 00-1.06 1.06L6.94 8l-3.72 3.72a.75.75 0 101.06 1.06L8 9.06l3.72 3.72a.75.75 0 101.06-1.06L9.06 8l3.72-3.72a.75.75 0 00-1.06-1.06L8 6.94 4.28 3.22Z" clip-rule="evenodd"/></svg>';
/// Icon Data of cloud_gear

  static const String cloud_gear =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M8 2.5a3.75 3.75 0 00-3.745 3.552l-.04.787-.785-.08a1.75 1.75 0 00-1.055 3.257.75.75 0 01-.75 1.299 3.249 3.249 0 011.215-6.04 5.251 5.251 0 0110.037-.973 3.75 3.75 0 011.561 6.744.75.75 0 01-.876-1.218 2.25 2.25 0 00-1.256-4.077l-.55-.014-.152-.528A3.752 3.752 0 008 2.5Zm-.803 3.924c-.05.405-.355.724-.731.88-.323.135-.624.31-.898.52-.324.248-.752.352-1.129.193a.485.485 0 00-.608.204l-.322.558a.485.485 0 00.127.629c.325.246.45.668.397 1.073a4.04 4.04 0 000 1.038c.053.405-.072.827-.397 1.073a.485.485 0 00-.127.629l.322.558c.122.212.383.3.608.204.377-.159.805-.055 1.129.193.274.21.575.385.898.52.376.156.68.475.731.88.03.242.236.424.48.424h.645c.245 0 .45-.182.481-.424.05-.405.355-.724.731-.88.323-.135.624-.31.898-.52.324-.248.752-.352 1.128-.193a.485.485 0 00.61-.204l.321-.558a.485.485 0 00-.127-.629c-.325-.246-.45-.668-.397-1.073a4.046 4.046 0 000-1.038c-.053-.405.072-.827.397-1.073a.485.485 0 00.127-.629l-.322-.558a.485.485 0 00-.609-.204c-.376.159-.804.055-1.128-.193a3.994 3.994 0 00-.898-.52c-.376-.156-.68-.475-.731-.88A.484.484 0 008.323 6h-.645a.484.484 0 00-.481.424ZM10.5 11a2.5 2.5 0 11-5 0 2.5 2.5 0 015 0ZM8 10a1 1 0 110 2 1 1 0 010-2Z" clip-rule="evenodd"/></svg>';
/// Icon Data of code

  static const String code =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M9.466 2.028a.75.75 0 01.53.919L7.185 13.44a.75.75 0 11-1.45-.388L8.549 2.559a.75.75 0 01.918-.53ZM4.84 11.78a.75.75 0 000-1.06L2.12 8l2.72-2.72a.75.75 0 00-1.06-1.06L.53 7.47 0 8l.53.53 3.25 3.25a.75.75 0 001.06 0Zm6.32-1.06a.75.75 0 101.06 1.06l3.25-3.25L16 8l-.53-.53-3.25-3.25a.75.75 0 10-1.06 1.06L13.88 8l-2.72 2.72Z" clip-rule="evenodd"/></svg>';
/// Icon Data of collapse

  static const String collapse =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M11.78 3.16a.75.75 0 00-1.06 0L8.75 5.13V.75a.75.75 0 00-1.5 0v4.38L5.28 3.16a.75.75 0 00-1.06 1.06l3.25 3.25L8 8l-.53.53-3.25 3.25a.75.75 0 001.06 1.061l1.97-1.97v4.379a.75.75 0 001.5 0v-4.379l1.97 1.97a.75.75 0 101.06-1.06L8.53 8.53 8 8l.53-.53 3.25-3.25a.75.75 0 000-1.061Z" clip-rule="evenodd"/></svg>';
/// Icon Data of collapse_left

  static const String collapse_left =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M0 12.25a.75.75 0 001.5 0v-8.5a.75.75 0 00-1.5 0v8.5Zm7.841-8.03a.75.75 0 010 1.06l-1.97 1.97h9.379a.75.75 0 010 1.5H5.871l1.97 1.97a.75.75 0 11-1.06 1.06L3.53 8.53 3 8l.53-.53 3.25-3.25a.75.75 0 011.061 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of collapse_right

  static const String collapse_right =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M14.5 3.75a.75.75 0 011.5 0v8.5a.75.75 0 01-1.5 0v-8.5Zm-6.341.47a.75.75 0 000 1.06l1.97 1.97H.75a.75.75 0 000 1.5h9.379l-1.97 1.97a.75.75 0 101.06 1.06l3.25-3.25L13 8l-.53-.53-3.25-3.25a.75.75 0 00-1.061 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of comment

  static const String comment =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M13 12H7l-2.5 2.5L3 16v-4a3 3 0 01-3-3V5a3 3 0 013-3h10a3 3 0 013 3v4a3 3 0 01-3 3Zm-8.5 0v.379l1.44-1.44.439-.439H13A1.5 1.5 0 0014.5 9V5A1.5 1.5 0 0013 3.5H3A1.5 1.5 0 001.5 5v4A1.5 1.5 0 003 10.5h1.5V12Z" clip-rule="evenodd"/></svg>';
/// Icon Data of comment_dots

  static const String comment_dots =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4.5 14.5 7 12h6a3 3 0 003-3V5a3 3 0 00-3-3H3a3 3 0 00-3 3v4a3 3 0 003 3v4l1.5-1.5Zm0-2.121V10.5H3A1.5 1.5 0 011.5 9V5A1.5 1.5 0 013 3.5h10A1.5 1.5 0 0114.5 5v4a1.5 1.5 0 01-1.5 1.5H6.379l-.44.44L4.5 12.378ZM11 8a1 1 0 100-2 1 1 0 000 2ZM9 7a1 1 0 11-2 0 1 1 0 012 0ZM5 8a1 1 0 100-2 1 1 0 000 2Z" clip-rule="evenodd"/></svg>';
/// Icon Data of comment_next

  static const String comment_next =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M13 12H7l-2.5 2.5L3 16v-4a3 3 0 01-3-3V5a3 3 0 013-3h10a3 3 0 013 3v4a3 3 0 01-3 3Zm-8.5 0v.379l1.44-1.44.439-.439H13A1.5 1.5 0 0014.5 9V5A1.5 1.5 0 0013 3.5H3A1.5 1.5 0 001.5 5v4A1.5 1.5 0 003 10.5h1.5V12Zm4.72-6.78a.75.75 0 00-.029 1.03H4.75a.75.75 0 000 1.5h4.441a.75.75 0 001.09 1.03l1.249-1.25.53-.53-.53-.53-1.25-1.25a.75.75 0 00-1.06 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of comments

  static const String comments =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M2 0a2 2 0 00-2 2v10.06l1.28-1.28 1.53-1.53H4V11a2 2 0 002 2h7l1.5 1.5L16 16V6a2 2 0 00-2-2h-2V2a2 2 0 00-2-2H2Zm8.5 4V2a.5.5 0 00-.5-.5H2a.5.5 0 00-.5.5v6.44l.47-.47.22-.22H4V6a2 2 0 012-2h4.5Zm3.56 7.94.44.439V6a.5.5 0 00-.5-.5H6a.5.5 0 00-.5.5v5a.5.5 0 00.5.5h7.621l.44.44Z" clip-rule="evenodd"/></svg>';
/// Icon Data of commit

  static const String commit =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M8 10.5a2.5 2.5 0 100-5 2.5 2.5 0 000 5ZM8 12c1.953 0 3.579-1.4 3.93-3.25h3.32a.75.75 0 000-1.5h-3.32a4.001 4.001 0 00-7.86 0H.75a.75.75 0 000 1.5h3.32A4.001 4.001 0 008 12Z" clip-rule="evenodd"/></svg>';
/// Icon Data of comparison

  static const String comparison =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4.5 3.5a1 1 0 11-2 0 1 1 0 012 0Zm-.25 2.386a2.501 2.501 0 10-1.5 0V10.5a2.75 2.75 0 002.75 2.75h.629l-.47.47a.75.75 0 101.061 1.06l1.75-1.75.53-.53-.53-.53-1.75-1.75a.747.747 0 00-1.061 0 .75.75 0 000 1.06l.47.47H5.5c-.69 0-1.25-.56-1.25-1.25V5.886ZM9.28 1.22a.75.75 0 111.06 1.06l-.47.47h.63a2.75 2.75 0 012.75 2.75v4.614a2.501 2.501 0 11-1.5 0V5.5c0-.69-.56-1.25-1.25-1.25h-.63l.47.47a.75.75 0 11-1.06 1.06L7.53 4.03 7 3.5l.53-.53 1.75-1.75ZM12.5 13.5a1 1 0 100-2 1 1 0 000 2Z" clip-rule="evenodd"/></svg>';
/// Icon Data of connected

  static const String connected =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M14.78 2.28a.75.75 0 00-1.06-1.06l-1.617 1.616a4 4 0 00-5.275.336L5.53 4.47l-.25-.25a.75.75 0 00-1.06 1.06l.25.25-1.302 1.302a4 4 0 00-.336 5.275L1.22 13.72a.75.75 0 101.06 1.06l1.613-1.612a4 4 0 005.275-.336l1.302-1.302.25.25a.75.75 0 101.06-1.06l-.25-.25 1.298-1.298a4 4 0 00.336-5.275L14.78 2.28Zm-4.31 7.13 1.297-1.297a2.5 2.5 0 000-3.536l-.343-.343.404-.405-.404.405a2.5 2.5 0 00-3.536 0L6.591 5.53l3.879 3.88ZM5.53 6.591l-1.3 1.301a2.5 2.5 0 000 3.536l-.4.4.4-.4.343.343a2.5 2.5 0 003.536 0L9.41 10.47 5.53 6.59Z" clip-rule="evenodd"/></svg>';
/// Icon Data of container_image

  static const String container_image =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M8.38 1.103a.75.75 0 00-.76 0L.37 5.353a.75.75 0 000 1.294l7.25 4.25a.75.75 0 00.76 0l7.25-4.25a.75.75 0 000-1.294l-7.25-4.25ZM8 9.381 2.233 6 8 2.62 13.767 6 8 9.38Zm-6.87-.028a.75.75 0 00-.76 1.294l7.25 4.25a.75.75 0 00.76 0l7.25-4.25a.75.75 0 00-.76-1.294L8 13.381 1.13 9.353Z" clip-rule="evenodd"/></svg>';
/// Icon Data of copy_to_clipboard

  static const String copy_to_clipboard =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M8 2.5a1 1 0 11-2 0 1 1 0 012 0ZM9.45 2a2.5 2.5 0 00-4.9 0H3a1 1 0 00-1 1v10a1 1 0 001 1h2v-1.5H3.5v-9h1V5h5V3.5h1V7H12V3a1 1 0 00-1-1H9.45ZM7.5 9.5h1.25a.75.75 0 000-1.5h-1.5C6.56 8 6 8.56 6 9.25v1.5a.75.75 0 001.5 0V9.5Zm1.25 5H7.5v-1.25a.75.75 0 00-1.5 0v1.5c0 .69.56 1.25 1.25 1.25h1.5a.75.75 0 000-1.5Zm3.75-5h-1.25a.75.75 0 010-1.5h1.5c.69 0 1.25.56 1.25 1.25v1.5a.75.75 0 01-1.5 0V9.5Zm-1.25 5h1.25v-1.25a.75.75 0 011.5 0v1.5c0 .69-.56 1.25-1.25 1.25h-1.5a.75.75 0 010-1.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of credit_card

  static const String credit_card =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M14 3.5H2a.5.5 0 00-.5.5v1h13V4a.5.5 0 00-.5-.5ZM1.5 12V6.5h13V12a.5.5 0 01-.5.5H2a.5.5 0 01-.5-.5ZM2 2a2 2 0 00-2 2v8a2 2 0 002 2h12a2 2 0 002-2V4a2 2 0 00-2-2H2Zm1.75 7.5a.75.75 0 000 1.5h3.5a.75.75 0 000-1.5h-3.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of dash

  static const String dash =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M2 8a.75.75 0 01.75-.75h10.5a.75.75 0 010 1.5H2.75A.75.75 0 012 8Z" clip-rule="evenodd"/></svg>';
/// Icon Data of dashboard

  static const String dashboard =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M13.545 2.1a.75.75 0 01.274 1.025l-3.472 6.007a3 3 0 11-1.208-.908l1.759-3.042a6.5 6.5 0 00-2.148-.639V5a.75.75 0 11-1.5 0v-.457a6.5 6.5 0 00-1.829.49l.229.396a.75.75 0 11-1.3.75l-.228-.396a6.5 6.5 0 00-1.339 1.34l.396.227a.75.75 0 01-.75 1.3l-.396-.229a6.5 6.5 0 00-.498 1.905.75.75 0 01-1.492-.155A8 8 0 0111.65 3.88l.87-1.506a.75.75 0 011.025-.274Zm-.107 4.047a.75.75 0 011.047.169 8 8 0 011.51 4.963.75.75 0 11-1.499-.052 6.5 6.5 0 00-1.227-4.033.75.75 0 01.17-1.047ZM9.5 11a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of deployments

  static const String deployments =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M3.75 1a.75.75 0 000 1.5h6.5a3.25 3.25 0 013.25 3.25V9.5h-.75a.75.75 0 000 1.5H15V5.75A4.75 4.75 0 0010.25 1h-6.5ZM13 14.25a.75.75 0 01-.75.75h-6.5A4.75 4.75 0 011 10.25V5h2.25a.75.75 0 010 1.5H2.5v3.75a3.25 3.25 0 003.25 3.25h6.5a.75.75 0 01.75.75ZM6.22 5.22a.75.75 0 011.06 0l2.25 2.25.53.53-.53.53-2.25 2.25a.75.75 0 11-1.06-1.06L7.94 8 6.22 6.28a.75.75 0 010-1.06Z" clip-rule="evenodd"/></svg>';
/// Icon Data of details_block

  static const String details_block =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M0 4.596v1.428a.5.5 0 00.725.446l.886-.446.377-.19L2 5.827l1.404-.708.07-.036.662-.333.603-.305a.5.5 0 000-.892l-.603-.305-.662-.333-.07-.036L2 2.172l-.012-.005-.377-.19-.886-.447A.5.5 0 000 1.976v2.62ZM7.75 3a.75.75 0 100 1.5h6.5a.25.25 0 01.25.25v7.5a.25.25 0 01-.25.25h-9.5a.25.25 0 01-.25-.25v-4.5a.75.75 0 10-1.5 0v4.5c0 .966.784 1.75 1.75 1.75h9.5A1.75 1.75 0 0016 12.25v-7.5A1.75 1.75 0 0014.25 3h-6.5Zm-1 3.5a.75.75 0 100 1.5h5.5a.75.75 0 000-1.5h-5.5ZM6 9.75A.75.75 0 016.75 9h3.5a.75.75 0 010 1.5h-3.5A.75.75 0 016 9.75Z" clip-rule="evenodd"/></svg>';
/// Icon Data of disk

  static const String disk =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M3 2.5h10a.5.5 0 01.5.5v5.063A2.004 2.004 0 0013 8H3c-.173 0-.34.022-.5.063V3a.5.5 0 01.5-.5ZM2.5 10v3a.5.5 0 00.5.5h10a.5.5 0 00.5-.5v-3a.5.5 0 00-.5-.5H3a.5.5 0 00-.5.5ZM1 10V3a2 2 0 012-2h10a2 2 0 012 2v10a2 2 0 01-2 2H3a2 2 0 01-2-2v-3Zm11 1.5a1 1 0 11-2 0 1 1 0 012 0Zm-4 1a1 1 0 100-2 1 1 0 000 2Z" clip-rule="evenodd"/></svg>';
/// Icon Data of doc_changes

  static const String doc_changes =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M3.5 13.5v-12H8v2.75C8 5.216 8.784 6 9.75 6h3.375a.76.76 0 00.063-.003A.75.75 0 0014 5.25v-.774a1 1 0 00-.282-.695L10.363.305A1 1 0 009.643 0H3a1 1 0 00-1 1v13a1 1 0 001 1h4.25a.75.75 0 000-1.5H3.5Zm8.828-9L9.5 1.57v2.68c0 .138.112.25.25.25h2.578ZM10 15.25a.75.75 0 01.75-.75h4.5a.75.75 0 010 1.5h-4.5a.75.75 0 01-.75-.75Zm3-2a.75.75 0 01-.75-.75V11h-1.5a.75.75 0 010-1.5h1.5V8a.75.75 0 011.5 0v1.5h1.5a.75.75 0 010 1.5h-1.5v1.5a.75.75 0 01-.75.75Z" clip-rule="evenodd"/></svg>';
/// Icon Data of doc_chart

  static const String doc_chart =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M14.5 8v4.5h-13v-9H10v2.75c0 .966.784 1.75 1.75 1.75h2.75Zm-3-4.379L14.379 6.5H11.75a.25.25 0 01-.25-.25V3.621ZM1 2a1 1 0 00-1 1v10a1 1 0 001 1h14a1 1 0 001-1V6.414a1 1 0 00-.293-.707l-3.414-3.414A1 1 0 0011.586 2H1Zm2.75 5a.75.75 0 00-.75.75v2.5a.75.75 0 001.5 0v-2.5A.75.75 0 003.75 7ZM6 5.75a.75.75 0 011.5 0v4.5a.75.75 0 01-1.5 0v-4.5ZM9.75 9a.75.75 0 00-.75.75v.5a.75.75 0 001.5 0v-.5A.75.75 0 009.75 9Z" clip-rule="evenodd"/></svg>';
/// Icon Data of doc_code

  static const String doc_code =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M12.5 14.5V6H9.75A1.75 1.75 0 018 4.25V1.5H3.5v13h9Zm-.121-10L9.5 1.621V4.25c0 .138.112.25.25.25h2.629ZM2 1a1 1 0 011-1h6.586a1 1 0 01.707.293l3.414 3.414a1 1 0 01.293.707V15a1 1 0 01-1 1H3a1 1 0 01-1-1V1Zm5.75 10.5a.75.75 0 000 1.5h2.5a.75.75 0 000-1.5h-2.5ZM6.28 6.22a.75.75 0 00-1.06 1.06L6.44 8.5 5.22 9.72a.75.75 0 101.06 1.06l1.75-1.75.53-.53-.53-.53-1.75-1.75Z" clip-rule="evenodd"/></svg>';
/// Icon Data of doc_compressed

  static const String doc_compressed =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M12.5 14.5V6H9.75A1.75 1.75 0 018 4.25V1.5H3.5v13H5V13h2v1.5h5.5ZM7 11h2v2H7v-2Zm0 0V9h2V7H7V5H5v2h2v2H5v2h2Zm2.5-9.379L12.379 4.5H9.75a.25.25 0 01-.25-.25V1.621ZM3 0a1 1 0 00-1 1v14a1 1 0 001 1h10a1 1 0 001-1V4.414a1 1 0 00-.293-.707L10.293.293A1 1 0 009.586 0H3Z" clip-rule="evenodd"/></svg>';
/// Icon Data of doc_expand

  static const String doc_expand =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M3.5 1.5v12h5.75a.75.75 0 010 1.5H3a1 1 0 01-1-1V1a1 1 0 011-1h6.644a1 1 0 01.72.305l3.355 3.476a1 1 0 01.281.695v.774a.75.75 0 01-.813.747.76.76 0 01-.062.003H9.75A1.75 1.75 0 018 4.25V1.5H3.5Zm6 .07 2.828 2.93H9.75a.25.25 0 01-.25-.25V1.57Zm6.353 12.228L13.5 16l-2.353-2.202c-.314-.295-.091-.798.353-.798h1.25v-3H11.5c-.444 0-.667-.503-.353-.798L13.5 7l2.353 2.202c.314.295.091.798-.353.798h-1.25v3h1.25c.444 0 .667.503.353.798Z" clip-rule="evenodd"/></svg>';
/// Icon Data of doc_image

  static const String doc_image =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M12.5 6v8.5h-9v-13H8v2.75C8 5.216 8.784 6 9.75 6h2.75Zm-.121-1.5L9.5 1.621V4.25c0 .138.112.25.25.25h2.629ZM2 1a1 1 0 011-1h6.586a1 1 0 01.707.293l3.414 3.414a1 1 0 01.293.707V15a1 1 0 01-1 1H3a1 1 0 01-1-1V1Zm7.125 9L11 12.143V13H5v-.9l1.125-1.35 1.143 1.372L9.125 10ZM8 8.5a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of doc_new

  static const String doc_new =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M3.5 1.5v13h5.75a.75.75 0 010 1.5H3a1 1 0 01-1-1V1a1 1 0 011-1h6.644a1 1 0 01.72.305l3.355 3.476a1 1 0 01.281.695V6.25a.75.75 0 01-1.5 0V6H9.75A1.75 1.75 0 018 4.25V1.5H3.5Zm6 .07 2.828 2.93H9.75a.25.25 0 01-.25-.25V1.57ZM13 15a.75.75 0 01-.75-.75v-1.5h-1.5a.75.75 0 010-1.5h1.5v-1.5a.75.75 0 011.5 0v1.5h1.5a.75.75 0 010 1.5h-1.5v1.5A.75.75 0 0113 15Z" clip-rule="evenodd"/></svg>';
/// Icon Data of doc_symlink

  static const String doc_symlink =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M3 0a1 1 0 00-1 1v6a.75.75 0 001.5 0V1.5H8v2.75C8 5.216 8.784 6 9.75 6h2.75v8.5H4.75a.75.75 0 000 1.5H13a1 1 0 001-1V4.476a1 1 0 00-.28-.695L10.362.305A1 1 0 009.643 0H3Zm9.328 4.5L9.5 1.57v2.68c0 .138.112.25.25.25h2.578ZM1 13.5a3.75 3.75 0 013.75-3.75H7V8.5c0-.445.503-.667.798-.353L10 10.5l-2.202 2.353c-.295.314-.798.091-.798-.353v-1.25H4.75A2.25 2.25 0 002.5 13.5v.75a.75.75 0 01-1.5 0v-.75Z" clip-rule="evenodd"/></svg>';
/// Icon Data of doc_text

  static const String doc_text =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M12.5 6v8.5h-9v-13H8v2.75C8 5.216 8.784 6 9.75 6h2.75Zm-.121-1.5L9.5 1.621V4.25c0 .138.112.25.25.25h2.629ZM2 1a1 1 0 011-1h6.586a1 1 0 01.707.293l3.414 3.414a1 1 0 01.293.707V15a1 1 0 01-1 1H3a1 1 0 01-1-1V1Zm3.75 7a.75.75 0 000 1.5h4.5a.75.75 0 000-1.5h-4.5ZM5 11.25a.75.75 0 01.75-.75h2.5a.75.75 0 010 1.5h-2.5a.75.75 0 01-.75-.75Z" clip-rule="evenodd"/></svg>';
/// Icon Data of doc_versions

  static const String doc_versions =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M14.5 7v5.5h-7v-9H11V6a1 1 0 001 1h2.5Zm-2-3.379L14.379 5.5H12.5V3.621ZM7 2a1 1 0 00-1 1H4a1 1 0 00-1 1H1a1 1 0 00-1 1v6a1 1 0 001 1h2a1 1 0 001 1h2a1 1 0 001 1h8a1 1 0 001-1V5.414a1 1 0 00-.293-.707l-2.414-2.414A1 1 0 0012.586 2H7Zm-1 9.5v-7H4.5v7H6Zm-3-1v-5H1.5v5H3Z" clip-rule="evenodd"/></svg>';
/// Icon Data of document

  static const String document =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M12.5 6v8.5h-9v-13H8v2.75C8 5.216 8.784 6 9.75 6h2.75Zm-.121-1.5L9.5 1.621V4.25c0 .138.112.25.25.25h2.629ZM2 1a1 1 0 011-1h6.586a1 1 0 01.707.293l3.414 3.414a1 1 0 01.293.707V15a1 1 0 01-1 1H3a1 1 0 01-1-1V1Z" clip-rule="evenodd"/></svg>';
/// Icon Data of documents

  static const String documents =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M13.5 6v5.5h-8v-10H9v2.75C9 5.216 9.784 6 10.75 6h2.75Zm-3-4.379L13.379 4.5H10.75a.25.25 0 01-.25-.25V1.621ZM5 0a1 1 0 00-1 1v2H2.25C1.56 3 1 3.56 1 4.25v10.5c0 .69.56 1.25 1.25 1.25h8.25c.69 0 1.25-.56 1.25-1.25v-1.67a.756.756 0 00-.004-.08H14a1 1 0 001-1V4.414a1 1 0 00-.293-.707L11.293.293A1 1 0 0010.586 0H5ZM4 12V4.5H2.5v10h7.75v-1.42c0-.027.001-.054.004-.08H5a1 1 0 01-1-1Z" clip-rule="evenodd"/></svg>';
/// Icon Data of dot_grid

  static const String dot_grid =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M3.5 2.25a1.25 1.25 0 11-2.5 0 1.25 1.25 0 012.5 0Zm11.5 0a1.25 1.25 0 11-2.5 0 1.25 1.25 0 012.5 0Zm-1.25 7a1.25 1.25 0 100-2.5 1.25 1.25 0 000 2.5Zm1.25 4.5a1.25 1.25 0 11-2.5 0 1.25 1.25 0 012.5 0ZM2.25 9.25a1.25 1.25 0 100-2.5 1.25 1.25 0 000 2.5Zm7-1.25a1.25 1.25 0 11-2.5 0 1.25 1.25 0 012.5 0ZM8 15a1.25 1.25 0 100-2.5A1.25 1.25 0 008 15ZM9.25 2.25a1.25 1.25 0 11-2.5 0 1.25 1.25 0 012.5 0ZM2.25 15a1.25 1.25 0 100-2.5 1.25 1.25 0 000 2.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of download

  static const String download =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M11.78 7.159a.75.75 0 00-1.06 0l-1.97 1.97V1.75a.75.75 0 00-1.5 0v7.379l-1.97-1.97a.75.75 0 00-1.06 1.06l3.25 3.25L8 12l.53-.53 3.25-3.25a.75.75 0 000-1.061ZM2.5 9.75a.75.75 0 00-1.5 0V13a2 2 0 002 2h10a2 2 0 002-2V9.75a.75.75 0 00-1.5 0V13a.5.5 0 01-.5.5H3a.5.5 0 01-.5-.5V9.75Z" clip-rule="evenodd"/></svg>';
/// Icon Data of drag

  static const String drag =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M10.78 3.78a.75.75 0 000-1.06L8.53.47 8-.06l-.53.53-2.25 2.25a.75.75 0 001.06 1.06L8 2.06l1.72 1.72a.75.75 0 001.06 0ZM3.84 5.22a.75.75 0 00-1.06 0L.53 7.47 0 8l.53.53 2.25 2.25a.75.75 0 101.06-1.06L2.12 8l1.72-1.72a.75.75 0 000-1.06Zm6.94 6.88a.75.75 0 010 1.06l-2.25 2.25-.53.53-.53-.53-2.25-2.25a.75.75 0 111.06-1.06L8 13.82l1.72-1.72a.75.75 0 011.06 0Zm2.44-6.88a.75.75 0 00-1.06 1.06L13.88 8l-1.72 1.72a.75.75 0 101.06 1.06l2.25-2.25L16 8l-.53-.53-2.25-2.25ZM8 6a.75.75 0 01.75.75v.5h.5a.75.75 0 010 1.5h-.5v.5a.75.75 0 11-1.5 0v-.5h-.5a.75.75 0 110-1.5h.5v-.5A.75.75 0 018 6Z" clip-rule="evenodd"/></svg>';
/// Icon Data of drag_horizontal

  static const String drag_horizontal =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M7.25 4.25a.75.75 0 001.5 0v-2.5a.75.75 0 00-1.5 0v2.5Zm4.91.97a.75.75 0 011.06 0l2.25 2.25L16 8l-.53.53-2.25 2.25a.75.75 0 01-1.06-1.06L13.88 8l-1.72-1.72a.75.75 0 010-1.06ZM8 10a.75.75 0 01-.75-.75v-2.5a.75.75 0 011.5 0v2.5A.75.75 0 018 10Zm0 5a.75.75 0 01-.75-.75v-2.5a.75.75 0 011.5 0v2.5A.75.75 0 018 15ZM2.78 5.22a.75.75 0 011.06 1.06L2.12 8l1.72 1.72a.75.75 0 11-1.06 1.06L.53 8.53 0 8l.53-.53 2.25-2.25Z" clip-rule="evenodd"/></svg>';
/// Icon Data of drag_vertical

  static const String drag_vertical =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M11.75 7.25a.75.75 0 000 1.5h2.5a.75.75 0 000-1.5h-2.5Zm-.97 4.85a.75.75 0 010 1.06l-2.25 2.25-.53.53-.53-.53-2.25-2.25a.75.75 0 111.06-1.06L8 13.82l1.72-1.72a.75.75 0 011.06 0ZM6 8a.75.75 0 01.75-.75h2.5a.75.75 0 010 1.5h-2.5A.75.75 0 016 8ZM1 8a.75.75 0 01.75-.75h2.5a.75.75 0 010 1.5h-2.5A.75.75 0 011 8Zm9.78-5.28a.75.75 0 11-1.06 1.06L8 2.06 6.28 3.78a.75.75 0 01-1.06-1.06L7.47.47 8-.06l.53.53 2.25 2.25Z" clip-rule="evenodd"/></svg>';
/// Icon Data of dumbbell

  static const String dumbbell =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4 4.5v7h1v-7H4ZM3.5 3a1 1 0 00-1 1v8a1 1 0 001 1h2a1 1 0 001-1V8.75h3V12a1 1 0 001 1h2a1 1 0 001-1V4a1 1 0 00-1-1h-2a1 1 0 00-1 1v3.25h-3V4a1 1 0 00-1-1h-2ZM11 4.5v7h1v-7h-1Zm4.25.75A.75.75 0 0116 6v4.5a.75.75 0 01-1.5 0V6a.75.75 0 01.75-.75ZM1.5 6A.75.75 0 000 6v4.5a.75.75 0 001.5 0V6Z" clip-rule="evenodd"/></svg>';
/// Icon Data of duplicate

  static const String duplicate =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4 9.5H2.5v-7h7V4H11V2a1 1 0 00-1-1H2a1 1 0 00-1 1v8a1 1 0 001 1h2V9.5Zm9.5 4h-7v-7h7v7ZM5 6a1 1 0 011-1h8a1 1 0 011 1v8a1 1 0 01-1 1H6a1 1 0 01-1-1V6Z" clip-rule="evenodd"/></svg>';
/// Icon Data of earth

  static const String earth =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M8 14.5c.23 0 .843-.226 1.487-1.514.306-.612.563-1.37.742-2.236H5.771c.179.866.436 1.624.742 2.236C7.157 14.274 7.77 14.5 8 14.5ZM5.554 9.25a14.444 14.444 0 010-2.5h4.892a14.452 14.452 0 010 2.5H5.554Zm6.203 1.5c-.224 1.224-.593 2.308-1.066 3.168a6.525 6.525 0 003.2-3.168h-2.134Zm2.623-1.5h-2.43a16.019 16.019 0 000-2.5h2.429a6.533 6.533 0 010 2.5Zm-10.331 0H1.62a6.533 6.533 0 010-2.5h2.43a15.994 15.994 0 000 2.5Zm-1.94 1.5h2.134c.224 1.224.593 2.308 1.066 3.168a6.525 6.525 0 01-3.2-3.168Zm3.662-5.5h4.458c-.179-.866-.436-1.624-.742-2.236C8.843 1.726 8.23 1.5 8 1.5c-.23 0-.843.226-1.487 1.514-.306.612-.563 1.37-.742 2.236Zm5.986 0h2.134a6.526 6.526 0 00-3.2-3.168c.473.86.842 1.944 1.066 3.168ZM5.31 2.082c-.473.86-.842 1.944-1.066 3.168H2.109a6.525 6.525 0 013.2-3.168ZM8 0a8 8 0 110 16A8 8 0 018 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of ellipsis_h

  static const String ellipsis_h =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" d="M4 8a2 2 0 11-4 0 2 2 0 014 0Zm6 0a2 2 0 11-4 0 2 2 0 014 0Zm4 2a2 2 0 100-4 2 2 0 000 4Z"/></svg>';
/// Icon Data of ellipsis_v

  static const String ellipsis_v =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" d="M8 12a2 2 0 110 4 2 2 0 010-4Zm0-6a2 2 0 110 4 2 2 0 010-4Zm2-4a2 2 0 10-4 0 2 2 0 004 0Z"/></svg>';
/// Icon Data of entity_blocked

  static const String entity_blocked =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M12 2.5H4A1.5 1.5 0 002.5 4v8A1.5 1.5 0 004 13.5h8a1.5 1.5 0 001.5-1.5V4A1.5 1.5 0 0012 2.5ZM4 1a3 3 0 00-3 3v8a3 3 0 003 3h8a3 3 0 003-3V4a3 3 0 00-3-3H4Zm6.217 8.156a2.5 2.5 0 00-3.373-3.373l3.373 3.373Zm-1.06 1.061a2.5 2.5 0 01-3.373-3.373l3.372 3.373ZM8 12a4 4 0 100-8 4 4 0 000 8Z" clip-rule="evenodd"/></svg>';
/// Icon Data of environment

  static const String environment =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="m6.712 5.997 3.291-3.29 3.29 3.29-3.29 3.29-3.29-3.29Zm2.584-4.705a1 1 0 011.414 0l3.998 3.998a1 1 0 010 1.414l-3.998 3.998a1 1 0 01-1.414 0L7.827 9.233l-1.242 1.243a3 3 0 11-1.06-1.06l1.242-1.243-1.469-1.469a1 1 0 010-1.414l3.998-3.998ZM4 13.5a1.5 1.5 0 100-3 1.5 1.5 0 000 3Z" clip-rule="evenodd"/></svg>';
/// Icon Data of epic

  static const String epic =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="m1.5 8.5 2.5-6h8l2.5 6h-13ZM0 8.569v2.181c0 .966.784 1.75 1.75 1.75H2v.75c0 .966.784 1.75 1.75 1.75h8.5a.75.75 0 000-1.5h-8.5a.25.25 0 01-.25-.25v-.75h10.25a.75.75 0 000-1.5h-12a.25.25 0 01-.25-.25V10h13a1.5 1.5 0 001.385-2.077l-2.629-6.308A1 1 0 0012.333 1H3.667a1 1 0 00-.923.615L.115 7.923A1.498 1.498 0 000 8.569Z" clip-rule="evenodd"/></svg>';
/// Icon Data of epic_closed

  static const String epic_closed =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="m4 2.5-2.5 6h13l-2.5-6H4Zm-4 8.25V8.569c-.01-.212.026-.431.115-.646l2.629-6.308A1 1 0 013.667 1h8.666a1 1 0 01.923.615l2.629 6.308A1.5 1.5 0 0114.5 10h-13v.75c0 .138.112.25.25.25h12a.75.75 0 010 1.5H3.5v.75c0 .138.112.25.25.25h8.5a.75.75 0 010 1.5h-8.5A1.75 1.75 0 012 13.25v-.75h-.25A1.75 1.75 0 010 10.75Zm10.28-5.72a.75.75 0 10-1.06-1.06L7.5 5.69l-.72-.72a.75.75 0 10-1.06 1.06l1.25 1.25a.75.75 0 001.06 0l2.25-2.25Z" clip-rule="evenodd"/></svg>';
/// Icon Data of error

  static const String error =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M8 14.5a6.5 6.5 0 100-13 6.5 6.5 0 000 13ZM8 16A8 8 0 108 0a8 8 0 000 16Zm1-5a1 1 0 11-2 0 1 1 0 012 0Zm-.25-6.25a.75.75 0 00-1.5 0v3.5a.75.75 0 001.5 0v-3.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of expand

  static const String expand =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M11.78 11.16a.75.75 0 00-1.06 0l-1.97 1.97V2.87l1.97 1.97a.75.75 0 101.06-1.06L8.53.53 8 0l-.53.53-3.25 3.25a.75.75 0 001.06 1.06l1.97-1.97v10.26l-1.97-1.97a.75.75 0 00-1.06 1.06l3.25 3.25L8 16l.53-.53 3.25-3.25a.75.75 0 000-1.06Z" clip-rule="evenodd"/></svg>';
/// Icon Data of expand_down

  static const String expand_down =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M3.75 1a.75.75 0 000 1.5h8.5a.75.75 0 000-1.5h-8.5Zm8.03 9.159a.75.75 0 00-1.06 0l-1.97 1.97V4.75a.75.75 0 00-1.5 0v7.379l-1.97-1.97a.75.75 0 00-1.06 1.06l3.25 3.25L8 15l.53-.53 3.25-3.25a.75.75 0 000-1.061Z" clip-rule="evenodd"/></svg>';
/// Icon Data of expand_left

  static const String expand_left =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M0 12.25a.75.75 0 001.5 0v-8.5a.75.75 0 00-1.5 0v8.5Zm11.159-8.03a.75.75 0 000 1.06l1.97 1.97H3.75a.75.75 0 100 1.5h9.379l-1.97 1.97a.75.75 0 101.06 1.06l3.25-3.25L16 8l-.53-.53-3.25-3.25a.75.75 0 00-1.061 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of expand_right

  static const String expand_right =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M14.5 12.25a.75.75 0 001.5 0v-8.5a.75.75 0 00-1.5 0v8.5ZM4.841 4.22a.75.75 0 010 1.06l-1.97 1.97h9.379a.75.75 0 010 1.5H2.871l1.97 1.97a.75.75 0 11-1.06 1.06L.53 8.53 0 8l.53-.53 3.25-3.25a.75.75 0 011.061 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of expand_up

  static const String expand_up =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M11.78 5.841a.75.75 0 01-1.06 0l-1.97-1.97v7.379a.75.75 0 01-1.5 0V3.871l-1.97 1.97a.75.75 0 01-1.06-1.06l3.25-3.25L8 1l.53.53 3.25 3.25a.75.75 0 010 1.061ZM3.75 13.5a.75.75 0 000 1.5h8.5a.75.75 0 000-1.5h-8.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of expire

  static const String expire =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M8.175.002a8 8 0 102.309 15.603.75.75 0 00-.466-1.426 6.5 6.5 0 113.996-8.646.75.75 0 001.388-.569A8 8 0 008.175.002ZM8.75 3.75a.75.75 0 00-1.5 0v3.94L5.216 9.723a.75.75 0 101.06 1.06L8.53 8.53l.22-.22V3.75ZM15 15a1 1 0 11-2 0 1 1 0 012 0Zm-.25-6.25a.75.75 0 00-1.5 0v3.5a.75.75 0 001.5 0v-3.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of i_export

  static const String i_export =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M11.78 5.841a.75.75 0 01-1.06 0l-1.97-1.97v7.379a.75.75 0 01-1.5 0V3.871l-1.97 1.97a.75.75 0 01-1.06-1.06l3.25-3.25L8 1l.53.53 3.25 3.25a.75.75 0 010 1.061ZM2.5 9.75a.75.75 0 00-1.5 0V13a2 2 0 002 2h10a2 2 0 002-2V9.75a.75.75 0 00-1.5 0V13a.5.5 0 01-.5.5H3a.5.5 0 01-.5-.5V9.75Z" clip-rule="evenodd"/></svg>';
/// Icon Data of external_link

  static const String external_link =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M10.75 1a.75.75 0 000 1.5h1.69L8.22 6.72a.75.75 0 001.06 1.06l4.22-4.22v1.69a.75.75 0 001.5 0V1h-4.25ZM2.5 4v9a.5.5 0 00.5.5h9a.5.5 0 00.5-.5V8.75a.75.75 0 011.5 0V13a2 2 0 01-2 2H3a2 2 0 01-2-2V4a2 2 0 012-2h4.25a.75.75 0 010 1.5H3a.5.5 0 00-.5.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of eye

  static const String eye =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M0 8s3-6 8-6 8 6 8 6-3 6-8 6-8-6-8-6Zm1.81.13A13.593 13.593 0 011.73 8l.082-.13c.326-.51.806-1.187 1.42-1.856C4.494 4.635 6.12 3.5 8 3.5c1.878 0 3.506 1.135 4.77 2.514A13.705 13.705 0 0114.27 8a14.021 14.021 0 01-1.502 1.986C11.506 11.365 9.88 12.5 8 12.5c-1.878 0-3.506-1.135-4.77-2.514A13.703 13.703 0 011.81 8.13ZM11 8a3 3 0 11-2.117-2.868 1.5 1.5 0 101.985 1.985A3 3 0 0111 8Z" clip-rule="evenodd"/></svg>';
/// Icon Data of eye_slash

  static const String eye_slash =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M14.776 2.284a.75.75 0 00-1.06-1.06l-1.953 1.953C10.707 2.499 9.44 2 8 2 5.506 2 3.533 3.49 2.24 4.86A14.615 14.615 0 00.226 7.576a5.39 5.39 0 00-.028.052l-.008.014-.003.005v.002L.85 8l-.663-.35L.002 8l.185.35L.85 8l-.663.35v.002l.002.002.004.007.012.023c.01.02.026.047.046.082a14.417 14.417 0 00.82 1.262c.47.647 1.13 1.447 1.96 2.18L1.22 13.72a.75.75 0 101.06 1.06L14.776 2.284Zm-10.681 8.56 1.32-1.32a3 3 0 014.109-4.109l1.148-1.147C9.864 3.8 8.969 3.5 8 3.5c-1.88 0-3.483 1.134-4.67 2.39A13.114 13.114 0 001.716 8c.13.213.32.508.567.846a11.98 11.98 0 001.811 1.998Zm9.42-5.166a.75.75 0 011.053.122c.447.564.901 1.2 1.245 1.85l.185.35-.185.35L15.15 8l.663.351-.001.002-.003.005-.008.014a9.81 9.81 0 01-.53.865 14.62 14.62 0 01-1.51 1.903C12.467 12.51 10.494 14 8 14a6.939 6.939 0 01-1.021-.08l-.02-.002-.006-.001h-.002l.12-.741-.12.74a.75.75 0 01.239-1.48h.002l.011.001a6.024 6.024 0 00.235.03c.158.017.362.033.562.033 1.88 0 3.483-1.134 4.67-2.39a13.11 13.11 0 001.616-2.115 12.33 12.33 0 00-.893-1.263.75.75 0 01.121-1.054Z" clip-rule="evenodd"/></svg>';
/// Icon Data of face_neutral

  static const String face_neutral =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M8 14.5a6.5 6.5 0 100-13 6.5 6.5 0 000 13ZM8 16A8 8 0 108 0a8 8 0 000 16Zm-2.5-5.25a.75.75 0 01.75-.75h3.5a.75.75 0 010 1.5h-3.5a.75.75 0 01-.75-.75ZM7 7a1 1 0 11-2 0 1 1 0 012 0Zm3 1a1 1 0 100-2 1 1 0 000 2Z" clip-rule="evenodd"/></svg>';
/// Icon Data of face_unhappy

  static const String face_unhappy =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M8 14.5a6.5 6.5 0 100-13 6.5 6.5 0 000 13ZM8 16A8 8 0 108 0a8 8 0 000 16Zm0-7a3 3 0 00-3 3h6a3 3 0 00-3-3ZM6.002 5a1.5 1.5 0 00-1.3.747.5.5 0 10.866.502.5.5 0 01.865 0 .5.5 0 00.866-.5A1.5 1.5 0 006.002 5Zm3.25.2a1.5 1.5 0 012.047.55.5.5 0 11-.866.5.5.5 0 00-.865-.001.5.5 0 11-.865-.502 1.5 1.5 0 01.55-.547Z" clip-rule="evenodd"/></svg>';
/// Icon Data of false_positive

  static const String false_positive =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M6.229.199a8 8 0 019.727 6.964.75.75 0 01-1.492.157 6.5 6.5 0 10-7.132 7.146.75.75 0 11-.154 1.492 8 8 0 01-.95-15.76Zm5.051 10.02a.75.75 0 10-1.06 1.061L11.94 13l-1.72 1.72a.75.75 0 101.06 1.06L13 14.06l1.72 1.72a.75.75 0 101.06-1.06L14.06 13l1.72-1.72a.75.75 0 10-1.06-1.06L13 11.94l-1.72-1.72Zm.25-3.939a.75.75 0 00-1.06-1.06L6.5 9.19 5.28 7.97a.75.75 0 00-1.06 1.06l1.75 1.75a.75.75 0 001.06 0l4.5-4.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of feature_flag

  static const String feature_flag =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M11.5 5v6a3.5 3.5 0 11-7 0V5a3.5 3.5 0 117 0ZM3 5a5 5 0 0110 0v6a5 5 0 01-10 0V5Zm5 2a2 2 0 100-4 2 2 0 000 4Z" clip-rule="evenodd"/></svg>';
/// Icon Data of feature_flag_disabled

  static const String feature_flag_disabled =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M11.5 5v6a3.5 3.5 0 11-7 0V5a3.5 3.5 0 117 0ZM3 5a5 5 0 0110 0v6a5 5 0 01-10 0V5Zm5 8a2 2 0 100-4 2 2 0 000 4Z" clip-rule="evenodd"/></svg>';
/// Icon Data of file_addition

  static const String file_addition =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4 3.5h8a.5.5 0 01.5.5v8a.5.5 0 01-.5.5H4a.5.5 0 01-.5-.5V4a.5.5 0 01.5-.5ZM2 4a2 2 0 012-2h8a2 2 0 012 2v8a2 2 0 01-2 2H4a2 2 0 01-2-2V4Zm6 7a.75.75 0 01-.75-.75v-1.5h-1.5a.75.75 0 010-1.5h1.5v-1.5a.75.75 0 011.5 0v1.5h1.5a.75.75 0 010 1.5h-1.5v1.5A.75.75 0 018 11Z" clip-rule="evenodd"/></svg>';
/// Icon Data of file_addition_solid

  static const String file_addition_solid =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4 2a2 2 0 00-2 2v8a2 2 0 002 2h8a2 2 0 002-2V4a2 2 0 00-2-2H4Zm4 9a.75.75 0 01-.75-.75v-1.5h-1.5a.75.75 0 010-1.5h1.5v-1.5a.75.75 0 011.5 0v1.5h1.5a.75.75 0 010 1.5h-1.5v1.5A.75.75 0 018 11Z" clip-rule="evenodd"/></svg>';
/// Icon Data of file_deletion

  static const String file_deletion =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4 3.5h8a.5.5 0 01.5.5v8a.5.5 0 01-.5.5H4a.5.5 0 01-.5-.5V4a.5.5 0 01.5-.5ZM2 4a2 2 0 012-2h8a2 2 0 012 2v8a2 2 0 01-2 2H4a2 2 0 01-2-2V4Zm3.75 3.25a.75.75 0 000 1.5h4.5a.75.75 0 000-1.5h-4.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of file_deletion_solid

  static const String file_deletion_solid =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4 2a2 2 0 00-2 2v8a2 2 0 002 2h8a2 2 0 002-2V4a2 2 0 00-2-2H4Zm1.75 5.25a.75.75 0 000 1.5h4.5a.75.75 0 000-1.5h-4.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of file_modified

  static const String file_modified =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4 3.5h8a.5.5 0 01.5.5v8a.5.5 0 01-.5.5H4a.5.5 0 01-.5-.5V4a.5.5 0 01.5-.5ZM2 4a2 2 0 012-2h8a2 2 0 012 2v8a2 2 0 01-2 2H4a2 2 0 01-2-2V4Zm6 6a2 2 0 100-4 2 2 0 000 4Z" clip-rule="evenodd"/></svg>';
/// Icon Data of file_modified_solid

  static const String file_modified_solid =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4 2a2 2 0 00-2 2v8a2 2 0 002 2h8a2 2 0 002-2V4a2 2 0 00-2-2H4Zm4 8a2 2 0 100-4 2 2 0 000 4Z" clip-rule="evenodd"/></svg>';
/// Icon Data of file_tree

  static const String file_tree =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M2.5 1.75a.75.75 0 00-1.5 0v8.5a3 3 0 003 3h3V14a1 1 0 001 1h6a1 1 0 001-1v-3a1 1 0 00-1-1H8a1 1 0 00-1 1v.75H4a1.5 1.5 0 01-1.5-1.5V6.849c.441.255.954.401 1.5.401h3V8a1 1 0 001 1h6a1 1 0 001-1V5a1 1 0 00-1-1H8a1 1 0 00-1 1v.75H4a1.5 1.5 0 01-1.5-1.5v-2.5Zm11 11.75h-5v-2h5v2Zm-5-6v-2h5v2h-5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of filter

  static const String filter =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="m8.5 8.379.44-.44 4.56-4.56V2.5h-11v.879l4.56 4.56.44.44v4l1-1v-3ZM10 12l-2.5 2.5L6 16V9L1.293 4.293A1 1 0 011 3.586V2a1 1 0 011-1h12a1 1 0 011 1v1.586a1 1 0 01-.293.707L10 9v3Z" clip-rule="evenodd"/></svg>';
/// Icon Data of fire

  static const String fire =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M10.728 6.475c.052-.265.07-.534.054-.806a3.645 3.645 0 00-.328-1.272c-.425-.947-1.263-1.941-2.558-3.005-.146-.12-.298-.241-.455-.363A25.558 25.558 0 006 0a7.302 7.302 0 01-.336 2.2c-.419 1.346-1.167 2.472-1.878 3.543C2.862 7.133 2 8.43 2 10a6 6 0 0012 0c0-.593-.093-1.175-.26-1.741a7.39 7.39 0 00-.226-.654 9.204 9.204 0 00-.732-1.436 10.736 10.736 0 00-.328-.5c0 .261-.037.518-.105.769-.05.182-.116.36-.195.535-.09.195-.196.385-.317.567a5.03 5.03 0 01-.335.45 5.856 5.856 0 01-.686.696 6.588 6.588 0 01-1.196.821 2.239 2.239 0 01-.01.005 5.878 5.878 0 01-.177.09c-.432.21-.63-.236-.285-.57l.006-.006a7.474 7.474 0 00.66-.719 5.82 5.82 0 00.298-.405l.011-.018c.158-.238.288-.477.39-.718a3.5 3.5 0 00.215-.69ZM8.11 7.948c.325-.315.572-.608.753-.88.341-.51.444-.93.42-1.307a2.152 2.152 0 00-.199-.75c-.28-.623-.882-1.402-1.997-2.34-.484 1.542-1.328 2.811-2.013 3.842l-.04.06C4.056 8.046 3.5 8.963 3.5 10a4.5 4.5 0 109 0c0-.262-.025-.525-.073-.79a8.026 8.026 0 01-2.34 1.741l-.654-1.35.655 1.35a1.91 1.91 0 01-1.274.157 1.73 1.73 0 01-1.169-.94c-.425-.89-.014-1.756.46-2.214l.006-.006Z" clip-rule="evenodd"/></svg>';
/// Icon Data of first_contribution

  static const String first_contribution =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M8.716.315a1 1 0 00-1.432 0L6.646.97a1 1 0 01-.988.265l-.88-.249a1 1 0 00-1.24.717l-.226.886a1 1 0 01-.723.723l-.886.225a1 1 0 00-.717 1.24l.249.881a1 1 0 01-.265.987l-.655.639a1 1 0 000 1.432l.655.639a1 1 0 01.265.987l-.249.88a1 1 0 00.717 1.24l.886.226a1 1 0 01.723.723l.225.886a1 1 0 001.24.716l.881-.248a1 1 0 01.988.265l.638.655a1 1 0 001.432 0l.639-.655a1 1 0 01.987-.265l.88.248a1 1 0 001.24-.716l.226-.886a1 1 0 01.723-.723l.886-.225a1 1 0 00.716-1.24l-.248-.881a1 1 0 01.265-.988l.655-.638a1 1 0 000-1.432l-.655-.638a1 1 0 01-.265-.988l.248-.88a1 1 0 00-.716-1.24l-.886-.226a1 1 0 01-.723-.723l-.225-.886a1 1 0 00-1.24-.717l-.881.249A1 1 0 019.354.97L8.716.315ZM7 4h2v8H7V6H6V5l1-1Z" clip-rule="evenodd"/></svg>';
/// Icon Data of flag

  static const String flag =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M13.5 10.174v.012c-1.393.72-2.81.247-4.959-.585l-.17-.066C6.763 8.91 4.685 8.105 2.5 8.63V2.814c1.393-.72 2.81-.247 4.959.585l.17.066c1.607.624 3.685 1.43 5.871.904v5.805ZM8 11c-1.83-.708-3.659-1.416-5.5-.81v4.06a.75.75 0 01-1.5 0V2C3.35.2 5.675 1.1 8 2c1.83.708 3.659 1.416 5.5.81A5.068 5.068 0 0015 2v9c-2.35 1.8-4.675.9-7 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of folder

  static const String folder =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M1.5 4V2.5h4.697l1 1.5H1.5ZM0 4V2a1 1 0 011-1h5.465a1 1 0 01.832.445l1.667 2.5.034.055H15a1 1 0 011 1v8a1 1 0 01-1 1H1a1 1 0 01-1-1V4Zm1.5 1.5v7h13v-7h-13Z" clip-rule="evenodd"/></svg>';
/// Icon Data of folder_new

  static const String folder_new =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M1.5 2.5V4h5.697l-1-1.5H1.5ZM0 2v10.75C0 13.44.56 14 1.25 14h6a.75.75 0 000-1.5H1.5v-7h13v1.75a.75.75 0 001.5 0v-2C16 4.56 15.44 4 14.75 4H8.998a1.054 1.054 0 00-.034-.055l-1.667-2.5A1 1 0 006.465 1H1a1 1 0 00-1 1Zm13 13a.75.75 0 01-.75-.75v-1.5h-1.5a.75.75 0 010-1.5h1.5v-1.5a.75.75 0 011.5 0v1.5h1.5a.75.75 0 010 1.5h-1.5v1.5A.75.75 0 0113 15Z" clip-rule="evenodd"/></svg>';
/// Icon Data of folder_o

  static const String folder_o =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M1.5 4V2.5h4.697l1 1.5H1.5ZM0 4V2a1 1 0 011-1h5.465a1 1 0 01.832.445l1.667 2.5.034.055H15a1 1 0 011 1v8a1 1 0 01-1 1H1a1 1 0 01-1-1V4Zm1.5 1.5v7h13v-7h-13Z" clip-rule="evenodd"/></svg>';
/// Icon Data of folder_open

  static const String folder_open =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M1.5 7.33V2.5h4.618l1.453 2h-4.54a1 1 0 00-.965.739L1.5 7.329ZM.005 13.087A.758.758 0 010 13V2.12C0 1.501.501 1 1.12 1h5.191c.359 0 .696.172.907.462L9.062 4h3.954c.409 0 .772.196 1 .5h.95a1 1 0 01.966 1.261l-2.029 7.5a1 1 0 01-.965.739H1.002a1 1 0 01-.997-.913Zm1.65-.587L3.414 6h10.9l-1.759 6.5h-10.9Z" clip-rule="evenodd"/></svg>';
/// Icon Data of food

  static const String food =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M6.75 1A5.75 5.75 0 001 6.75v.518a2 2 0 000 3.464v1.518A2.75 2.75 0 003.75 15h8.5A2.75 2.75 0 0015 12.25v-1.518a2 2 0 000-3.464V6.75A5.75 5.75 0 009.25 1h-2.5ZM14 8.5H2a.5.5 0 000 1h12a.5.5 0 000-1ZM13.5 7v-.25A4.25 4.25 0 009.25 2.5h-2.5A4.25 4.25 0 002.5 6.75V7h11ZM11 11h2.5v1.25c0 .69-.56 1.25-1.25 1.25h-8.5c-.69 0-1.25-.56-1.25-1.25V11H9l1 1 1-1Z" clip-rule="evenodd"/></svg>';
/// Icon Data of fork

  static const String fork =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M5.5 3.5a1 1 0 11-2 0 1 1 0 012 0Zm-.25 2.386a2.501 2.501 0 10-1.5 0v.364a2.5 2.5 0 002.5 2.5 1 1 0 011 1v.364a2.501 2.501 0 101.5 0V9.75a1 1 0 011-1 2.5 2.5 0 002.5-2.5v-.364a2.501 2.501 0 10-1.5 0v.364a1 1 0 01-1 1c-.681 0-1.3.273-1.75.715a2.492 2.492 0 00-1.75-.715 1 1 0 01-1-1v-.364ZM11.5 4.5a1 1 0 100-2 1 1 0 000 2Zm-3.5 9a1 1 0 100-2 1 1 0 000 2Z" clip-rule="evenodd"/></svg>';
/// Icon Data of formula

  static const String formula =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M5.581 4.49A2.75 2.75 0 018.319 2h.931a.75.75 0 010 1.5h-.931a1.25 1.25 0 00-1.245 1.131l-.083.869H9.25a.75.75 0 010 1.5H6.849l-.43 4.51A2.75 2.75 0 013.681 14H2.75a.75.75 0 010-1.5h.931a1.25 1.25 0 001.245-1.132L5.342 7H3.75a.75.75 0 010-1.5h1.735l.096-1.01ZM9.22 9.22a.75.75 0 011.06 0l1.22 1.22 1.22-1.22a.75.75 0 111.06 1.06l-1.22 1.22 1.22 1.22a.75.75 0 11-1.06 1.06l-1.22-1.22-1.22 1.22a.75.75 0 11-1.06-1.06l1.22-1.22-1.22-1.22a.75.75 0 010-1.06Z" clip-rule="evenodd"/></svg>';
/// Icon Data of git

  static const String git =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" d="M15.698 7.287 8.712.302a1.03 1.03 0 00-1.457 0l-1.45 1.45 1.84 1.84a1.223 1.223 0 011.55 1.56l1.773 1.774a1.224 1.224 0 011.267 2.025 1.226 1.226 0 01-2.002-1.334L8.58 5.963v4.353a1.226 1.226 0 11-1.008-.036V5.887a1.226 1.226 0 01-.666-1.608L5.093 2.465l-4.79 4.79a1.031 1.031 0 000 1.457l6.986 6.986a1.03 1.03 0 001.457 0l6.953-6.953a1.03 1.03 0 000-1.458"/></svg>';
/// Icon Data of git_merge

  static const String git_merge =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M10.34 1.22a.75.75 0 00-1.06 0L7.53 2.97 7 3.5l.53.53 1.75 1.75a.75.75 0 101.06-1.06l-.47-.47h.63c.69 0 1.25.56 1.25 1.25v4.614a2.501 2.501 0 101.5 0V5.5a2.75 2.75 0 00-2.75-2.75h-.63l.47-.47a.75.75 0 000-1.06ZM13.5 12.5a1 1 0 11-2 0 1 1 0 012 0Zm-9 0a1 1 0 11-2 0 1 1 0 012 0Zm1.5 0a2.5 2.5 0 11-3.25-2.386V5.886a2.501 2.501 0 111.5 0v4.228A2.501 2.501 0 016 12.5Zm-1.5-9a1 1 0 11-2 0 1 1 0 012 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of github

  static const String github =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M7.976 0A7.977 7.977 0 000 7.976c0 3.522 2.3 6.507 5.431 7.584.392.049.538-.196.538-.392v-1.37c-2.201.49-2.69-1.076-2.69-1.076-.343-.93-.881-1.175-.881-1.175-.734-.489.048-.489.048-.489.783.049 1.224.832 1.224.832.734 1.223 1.859.88 2.3.685.048-.538.293-.88.489-1.076-1.762-.196-3.621-.881-3.621-3.964 0-.88.293-1.566.832-2.153-.05-.147-.343-.978.098-2.055 0 0 .685-.196 2.201.832.636-.196 1.322-.245 2.007-.245s1.37.098 2.006.245c1.517-1.027 2.202-.832 2.202-.832.44 1.077.146 1.908.097 2.104a3.16 3.16 0 01.832 2.153c0 3.083-1.86 3.719-3.62 3.915.293.244.538.733.538 1.467v2.202c0 .196.146.44.538.392A7.984 7.984 0 0016 7.976C15.951 3.572 12.38 0 7.976 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of go_back

  static const String go_back =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="m2.87 7.75 1.97 1.97a.75.75 0 11-1.06 1.06L.53 7.53 0 7l.53-.53 3.25-3.25a.75.75 0 011.06 1.06L2.87 6.25h9.88a3.25 3.25 0 010 6.5h-2a.75.75 0 010-1.5h2a1.75 1.75 0 100-3.5H2.87Z" clip-rule="evenodd"/></svg>';
/// Icon Data of google

  static const String google =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M8 7v2.4h3.97c-.16 1.03-1.2 3.02-3.97 3.02-2.39 0-4.34-1.98-4.34-4.42S5.61 3.58 8 3.58c1.36 0 2.27.58 2.79 1.08l1.9-1.83C11.47 1.69 9.89 1 8 1 4.13 1 1 4.13 1 8s3.13 7 7 7c4.04 0 6.72-2.84 6.72-6.84 0-.46-.05-.81-.11-1.16H8Z" clip-rule="evenodd"/></svg>';
/// Icon Data of group

  static const String group =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M6 4a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0Zm1.5 0a3 3 0 11-6 0 3 3 0 016 0Zm4 5.5a1.5 1.5 0 100-3 1.5 1.5 0 000 3Zm0 1.5a3 3 0 100-6 3 3 0 000 6Zm-7 2.5a1.5 1.5 0 100-3 1.5 1.5 0 000 3Zm0 1.5a3 3 0 100-6 3 3 0 000 6Z" clip-rule="evenodd"/></svg>';
/// Icon Data of hamburger

  static const String hamburger =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M0 3.75A.75.75 0 01.75 3h14.5a.75.75 0 010 1.5H.75A.75.75 0 010 3.75ZM0 8a.75.75 0 01.75-.75h14.5a.75.75 0 010 1.5H.75A.75.75 0 010 8Zm.75 3.5a.75.75 0 000 1.5h14.5a.75.75 0 000-1.5H.75Z" clip-rule="evenodd"/></svg>';
/// Icon Data of heading

  static const String heading =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4.5 2.75a.75.75 0 00-1.5 0v10.5a.75.75 0 001.5 0v-4.5h7v4.5a.75.75 0 001.5 0V2.75a.75.75 0 00-1.5 0v4.5h-7v-4.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of heart

  static const String heart =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M8.753 2.247 8 3l-.753-.753A4.243 4.243 0 001.25 8.25l5.69 5.69L8 15l1.06-1.06 5.69-5.69a4.243 4.243 0 00-5.997-6.003ZM8 12.879l5.69-5.69a2.743 2.743 0 00-3.877-3.881l-.752.753L8 5.12 6.94 4.06l-.753-.752v-.001A2.743 2.743 0 002.31 7.189L8 12.88Z" clip-rule="evenodd"/></svg>';
/// Icon Data of history

  static const String history =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4.806.665A8 8 0 11.612 11.07a.75.75 0 111.385-.575A6.5 6.5 0 102.523 4.5H4.25a.75.75 0 010 1.5H0V1.75a.75.75 0 011.5 0v1.586A8 8 0 014.806.666ZM8 3a.75.75 0 01.75.75v3.94l2.034 2.034a.75.75 0 11-1.06 1.06L7.47 8.53l-.22-.22V3.75A.75.75 0 018 3Z" clip-rule="evenodd"/></svg>';
/// Icon Data of home

  static const String home =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M8.38 1.353 8 1.131l-.38.222-7.25 4.25a.75.75 0 00.76 1.294l.87-.51V14h12V6.387l.87.51a.75.75 0 10.76-1.294l-7.25-4.25Zm4.12 4.154L8 2.87 3.5 5.507V12.5H6V8h4v4.5h2.5V5.507ZM8.5 9.5v3h-1v-3h1Z" clip-rule="evenodd"/></svg>';
/// Icon Data of hook

  static const String hook =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M1 11.125a3.875 3.875 0 007 2.292 3.875 3.875 0 007-2.292V7.002l-1.28 1.28-1.49 1.488a.75.75 0 001.061 1.061l.208-.208v.502a2.375 2.375 0 11-4.75 0v-5.24a2.501 2.501 0 10-1.5 0v5.24a2.375 2.375 0 11-4.75 0v-.502l.208.208a.75.75 0 101.06-1.06L2.28 8.281 1 7.002v4.123ZM9 3.5a1 1 0 10-2 0 1 1 0 002 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of hourglass

  static const String hourglass =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M2.75 0a.75.75 0 000 1.5H3v.593c0 1.26.5 2.468 1.391 3.359L6.94 8l-2.548 2.548A4.75 4.75 0 003 13.907v.593h-.25a.75.75 0 000 1.5h10.5a.75.75 0 000-1.5H13v-.593c0-1.26-.5-2.468-1.391-3.359L9.06 8l2.548-2.548A4.75 4.75 0 0013 2.093V1.5h.25a.75.75 0 000-1.5H2.75Zm8.75 1.5h-7v.593c0 .69.219 1.356.618 1.907h5.764a3.25 3.25 0 00.618-1.907V1.5ZM8 6.94 6.56 5.5h2.88L8 6.94Zm3.5 7.56v-.593a3.25 3.25 0 00-.952-2.298L8 9.06l-2.548 2.548a3.25 3.25 0 00-.952 2.298v.593h7Z" clip-rule="evenodd"/></svg>';
/// Icon Data of image_comment_dark

  static const String image_comment_dark =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M8 16A8 8 0 108 0a8 8 0 000 16Zm2-5H5l-1 1V7a2 2 0 012-2h4a2 2 0 012 2v2a2 2 0 01-2 2Zm0-1.5H5.5V7a.5.5 0 01.5-.5h4a.5.5 0 01.5.5v2a.5.5 0 01-.5.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of image_comment_light

  static const String image_comment_light =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M14.5 8a6.5 6.5 0 11-13 0 6.5 6.5 0 0113 0ZM16 8A8 8 0 110 8a8 8 0 0116 0Zm-6 3H5l-1 1V7a2 2 0 012-2h4a2 2 0 012 2v2a2 2 0 01-2 2Zm0-1.5H5.5V7a.5.5 0 01.5-.5h4a.5.5 0 01.5.5v2a.5.5 0 01-.5.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of i_import

  static const String i_import =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M11.78 7.159a.75.75 0 00-1.06 0l-1.97 1.97V1.75a.75.75 0 00-1.5 0v7.379l-1.97-1.97a.75.75 0 00-1.06 1.06l3.25 3.25L8 12l.53-.53 3.25-3.25a.75.75 0 000-1.061ZM2.5 9.75a.75.75 0 10-1.5 0V13a2 2 0 002 2h10a2 2 0 002-2V9.75a.75.75 0 00-1.5 0V13a.5.5 0 01-.5.5H3a.5.5 0 01-.5-.5V9.75Z" clip-rule="evenodd"/></svg>';
/// Icon Data of incognito

  static const String incognito =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M5.248 2.042a.75.75 0 00-.933.403L2.513 6.5H.75a.75.75 0 100 1.5h2.233a.76.76 0 00.033 0H15.25a.75.75 0 000-1.5h-1.763l-1.802-4.055a.75.75 0 00-.933-.403L8 3.005l-2.752-.963ZM11.846 6.5l-1.25-2.814-2.348.822a.75.75 0 01-.496 0l-2.347-.822L4.155 6.5h7.69Zm-6.346 6a1 1 0 100-2 1 1 0 000 2Zm0 1.5A2.5 2.5 0 008 11.5a2.5 2.5 0 005 0h.25a.75.75 0 000-1.5h-.75a2.496 2.496 0 00-2-1c-.818 0-1.544.393-2 1h-1a2.496 2.496 0 00-2-1c-.818 0-1.544.393-2 1h-.75a.75.75 0 000 1.5H3A2.5 2.5 0 005.5 14Zm5-1.5a1 1 0 100-2 1 1 0 000 2Z" clip-rule="evenodd"/></svg>';
/// Icon Data of information

  static const String information =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M8 16A8 8 0 108 0a8 8 0 000 16ZM9 5a1 1 0 11-2 0 1 1 0 012 0ZM7 7a.75.75 0 000 1.5h.25v2h-1a.75.75 0 000 1.5h3.5a.75.75 0 000-1.5h-1V7H7Z" clip-rule="evenodd"/></svg>';
/// Icon Data of information_o

  static const String information_o =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M14.5 8a6.5 6.5 0 11-13 0 6.5 6.5 0 0113 0ZM16 8A8 8 0 110 8a8 8 0 0116 0Zm-9.75 2.5a.75.75 0 000 1.5h3.5a.75.75 0 000-1.5h-1V7H7a.75.75 0 000 1.5h.25v2h-1ZM8 6a1 1 0 100-2 1 1 0 000 2Z" clip-rule="evenodd"/></svg>';
/// Icon Data of infrastructure_registry

  static const String infrastructure_registry =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="m8 2.732-2.945 1.7L8 6.135l2.945-1.701L8 2.733Zm4.445.834L8 1 3.555 3.566l-1.43-.825a.75.75 0 10-.75 1.298l1.429.826V10l4.446 2.567v1.683a.75.75 0 001.5 0v-1.683L13.196 10V4.865l1.43-.826a.75.75 0 00-.751-1.298l-1.43.825Zm-.749 2.165L8.75 7.433v3.402l2.946-1.701V5.731ZM4.304 9.134l2.946 1.7v-3.4L4.304 5.73v3.403Z" clip-rule="evenodd"/></svg>';
/// Icon Data of issue_block

  static const String issue_block =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M3 1a2 2 0 00-2 2v3.25a.75.75 0 001.5 0V3a.5.5 0 01.5-.5h6a.5.5 0 01.5.5v10.234a.753.753 0 00.57.744.752.752 0 00.851-.393l3.7-7.047a1.75 1.75 0 00-.74-2.366l-2.91-1.514A2 2 0 009 1H3Zm8 3.364v5.844l2.293-4.367a.25.25 0 00-.106-.338L11 4.363Zm-4.783 8.792a2.5 2.5 0 00-3.373-3.373l3.373 3.373Zm-1.06 1.061a2.5 2.5 0 01-3.373-3.373l3.372 3.373ZM4 16a4 4 0 100-8 4 4 0 000 8Z" clip-rule="evenodd"/></svg>';
/// Icon Data of issue_close

  static const String issue_close =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M14.5 8a6.5 6.5 0 11-13 0 6.5 6.5 0 0113 0ZM16 8A8 8 0 110 8a8 8 0 0116 0ZM3.75 7.25a.75.75 0 000 1.5h8.5a.75.75 0 000-1.5h-8.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of issue_closed

  static const String issue_closed =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M9 2.5H3a.5.5 0 00-.5.5v10a.5.5 0 00.5.5h6a.5.5 0 00.5-.5V3a.5.5 0 00-.5-.5ZM3 1a2 2 0 00-2 2v10a2 2 0 002 2h6a2 2 0 001.651-.87.753.753 0 00.098-.145l3.878-7.45a1.75 1.75 0 00-.744-2.361l-2.912-1.516A2 2 0 009 1H3Zm10.297 4.841L11 10.254v-5.89l2.19 1.14a.25.25 0 01.107.337ZM8.28 7.281A.75.75 0 007.22 6.22L5.25 8.19l-.47-.47a.75.75 0 00-1.06 1.06l1 1a.75.75 0 001.06 0l2.5-2.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of issue_new

  static const String issue_new =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M3 1a2 2 0 00-2 2v10a2 2 0 002 2h3.25a.75.75 0 000-1.5H3a.5.5 0 01-.5-.5V3a.5.5 0 01.5-.5h6a.5.5 0 01.5.5v5.25a.75.75 0 001.5 0V4.364l2.19 1.14a.25.25 0 01.107.338l-1.072 2.062a.75.75 0 001.33.692l1.073-2.062a1.75 1.75 0 00-.745-2.36l-2.912-1.516A2 2 0 009 1H3Zm6 12a.75.75 0 01.75-.75h1.5v-1.5a.75.75 0 011.5 0v1.5h1.5a.75.75 0 010 1.5h-1.5v1.5a.75.75 0 01-1.5 0v-1.5h-1.5A.75.75 0 019 13Z" clip-rule="evenodd"/></svg>';
/// Icon Data of issue_open_m

  static const String issue_open_m =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M8 14.5a6.5 6.5 0 100-13 6.5 6.5 0 000 13ZM8 16A8 8 0 108 0a8 8 0 000 16Z" clip-rule="evenodd"/></svg>';
/// Icon Data of issue_type_enhancement

  static const String issue_type_enhancement =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4 0a4 4 0 00-4 4v8a4 4 0 004 4h8a4 4 0 004-4V4a4 4 0 00-4-4H4zm8 9.25V4H6.75a.75.75 0 000 1.5h2.69l-5.22 5.22a.75.75 0 101.06 1.06l5.22-5.22v2.69a.75.75 0 001.5 0z" clip-rule="evenodd"/></svg>';
/// Icon Data of issue_type_feature

  static const String issue_type_feature =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" d="M5 4.5h-.5a1 1 0 000 2h2V6A1.5 1.5 0 005 4.5zm6.5 0H11A1.5 1.5 0 009.5 6v.5h2a1 1 0 100-2z"/><path fill="currentColor" fill-rule="evenodd" d="M4 0a4 4 0 00-4 4v8a4 4 0 004 4h8a4 4 0 004-4V4a4 4 0 00-4-4H4zm.5 3a2.5 2.5 0 000 5h2.076c-.108.509-.26.902-.442 1.215-.36.616-.891 1.008-1.68 1.346a.75.75 0 00.591 1.378c.962-.412 1.806-.978 2.384-1.967.248-.425.437-.911.571-1.47.134.559.323 1.045.571 1.47.578.989 1.422 1.555 2.384 1.967a.75.75 0 00.59-1.378c-.787-.338-1.319-.73-1.679-1.346-.183-.313-.334-.706-.442-1.215H11.5a2.5 2.5 0 000-5H11a3.001 3.001 0 00-2.83 2h-.34A3.001 3.001 0 005 3h-.5z" clip-rule="evenodd"/></svg>';
/// Icon Data of issue_type_feature_flag

  static const String issue_type_feature_flag =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4 0a4 4 0 00-4 4v8a4 4 0 004 4h8a4 4 0 004-4V4a4 4 0 00-4-4H4zm.225 4.803L4 5v6.5a.5.5 0 001 0V9.303A3.177 3.177 0 018 9.5a3.176 3.176 0 003.775-.303L12 9V4l-.225.197A3.176 3.176 0 018 4.5a3.176 3.176 0 00-3.775.303z" clip-rule="evenodd"/></svg>';
/// Icon Data of issue_type_incident

  static const String issue_type_incident =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M0 4a4 4 0 014-4h8a4 4 0 014 4v8a4 4 0 01-4 4H4a4 4 0 01-4-4V4zm6.996.165a1.017 1.017 0 112.012 0L8 11 6.996 4.165zM8 11a1 1 0 110 2 1 1 0 010-2z" clip-rule="evenodd"/></svg>';
/// Icon Data of issue_type_issue

  static const String issue_type_issue =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4 0a4 4 0 00-4 4v8a4 4 0 004 4h8a4 4 0 004-4V4a4 4 0 00-4-4H4zm1 4a1 1 0 00-1 1v6a1 1 0 001 1h4a1 1 0 001-1V5a1 1 0 00-1-1H5zm7.67 3.048a1 1 0 00-.386-1.234L11 5v6l1.67-3.952z" clip-rule="evenodd"/></svg>';
/// Icon Data of issue_type_maintenance

  static const String issue_type_maintenance =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4 0a4 4 0 00-4 4v8a4 4 0 004 4h8a4 4 0 004-4V4a4 4 0 00-4-4H4zm7.293 3.292a3 3 0 00-4.001 4.001l-4 4a1 1 0 101.415 1.414l4-4a3 3 0 004.001-4.001l-2 2.001a1 1 0 11-1.415-1.414l2-2z" clip-rule="evenodd"/></svg>';
/// Icon Data of issue_type_requirements

  static const String issue_type_requirements =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4 0a4 4 0 00-4 4v8a4 4 0 004 4h8a4 4 0 004-4V4a4 4 0 00-4-4H4zm0 4.75A.75.75 0 014.75 4h6.5a.75.75 0 010 1.5h-6.5A.75.75 0 014 4.75zm9.78 5.72a.75.75 0 010 1.06l-2.25 2.25a.75.75 0 01-1.06 0l-1.25-1.25a.75.75 0 111.06-1.06l.72.72 1.72-1.72a.75.75 0 011.06 0zM4 8a.75.75 0 01.75-.75h6.5a.75.75 0 010 1.5h-6.5A.75.75 0 014 8zm0 3.25a.75.75 0 01.75-.75h2.5a.75.75 0 010 1.5h-2.5a.75.75 0 01-.75-.75z" clip-rule="evenodd"/></svg>';
/// Icon Data of issue_type_task

  static const String issue_type_task =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M8 12.5a4.5 4.5 0 100-9 4.5 4.5 0 000 9Zm2.28-6.28a.75.75 0 010 1.06l-2.5 2.5a.75.75 0 01-1.06 0l-1-1a.75.75 0 011.06-1.06l.47.47 1.97-1.97a.75.75 0 011.06 0Z" clip-rule="evenodd"/><path fill="currentColor" fill-rule="evenodd" d="M4 0a4 4 0 00-4 4v8a4 4 0 004 4h8a4 4 0 004-4V4a4 4 0 00-4-4H4Zm4 14A6 6 0 108 2a6 6 0 000 12Z" clip-rule="evenodd"/></svg>';
/// Icon Data of issue_type_test_case

  static const String issue_type_test_case =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" d="M6.751 7.834 7.5 7.4V3.5h1v3.9l.749.434A2.498 2.498 0 0110.5 10h-5c0-.923.5-1.732 1.251-2.166z"/><path fill="currentColor" fill-rule="evenodd" d="M4 0a4 4 0 00-4 4v8a4 4 0 004 4h8a4 4 0 004-4V4a4 4 0 00-4-4H4zm6 3.5v3.035a4 4 0 11-4 0V3.5h-.25a.75.75 0 010-1.5h4.5a.75.75 0 010 1.5H10z" clip-rule="evenodd"/></svg>';
/// Icon Data of issue_update

  static const String issue_update =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M3 1a2 2 0 00-2 2v10a2 2 0 002 2h3.25a.75.75 0 000-1.5H3a.5.5 0 01-.5-.5V3a.5.5 0 01.5-.5h6a.5.5 0 01.5.5v5.25a.75.75 0 001.5 0V4.364l2.19 1.14a.25.25 0 01.107.338l-1.072 2.062a.75.75 0 001.33.692l1.073-2.062a1.75 1.75 0 00-.745-2.36l-2.912-1.516A2 2 0 009 1H3Zm10 12a2 2 0 11-4 0 2 2 0 014 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of issues

  static const String issues =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M3 2.5h6a.5.5 0 01.5.5v10a.5.5 0 01-.5.5H3a.5.5 0 01-.5-.5V3a.5.5 0 01.5-.5ZM1 3a2 2 0 012-2h6a2 2 0 011.97 1.658l2.913 1.516a1.75 1.75 0 01.744 2.36l-3.878 7.45a.753.753 0 01-.098.145c-.36.526-.965.871-1.651.871H3a2 2 0 01-2-2V3Zm10 7.254 2.297-4.413a.25.25 0 00-.106-.337L11 4.364v5.89Z" clip-rule="evenodd"/></svg>';
/// Icon Data of italic

  static const String italic =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M6.5 2a.75.75 0 000 1.5h1.93l-2.412 9H4A.75.75 0 004 14h5.5a.75.75 0 000-1.5H7.57l2.412-9H12A.75.75 0 0012 2H6.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of iteration

  static const String iteration =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M3.391 4.844a3.375 3.375 0 111.485 6.406h-.002a3.865 3.865 0 01-.06 0H.75a.75.75 0 000 1.5h12.379l-.97.97a.75.75 0 101.061 1.06l2.25-2.25L16 12l-.53-.53-2.25-2.25a.75.75 0 10-1.06 1.06l.97.97H8.392A4.874 4.874 0 10.018 8.298a.75.75 0 101.495-.13A3.375 3.375 0 013.39 4.844Z" clip-rule="evenodd"/></svg>';
/// Icon Data of key

  static const String key =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="m6.358 8.763.675-.674-.325-.897A3.5 3.5 0 1110 9.5H7.5v1H6.379l-.44.44-1 1-.439.439V13.5h-2v-.879l3.858-3.858ZM6 15v-2l1-1h2v-1h1a5 5 0 10-4.703-3.297L1 12v3h5Zm5-9a1 1 0 100-2 1 1 0 000 2Z" clip-rule="evenodd"/></svg>';
/// Icon Data of kubernetes

  static const String kubernetes =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" d="M9.449 9.15a.283.283 0 01.067.006l.003-.004 1.734.293a3.435 3.435 0 01-1.388 1.744l-.673-1.626.002-.002a.294.294 0 01.255-.41Zm-2.732.073a.294.294 0 01.085.344l.005.006-.666 1.61A3.448 3.448 0 014.757 9.45l1.72-.292.002.004a.288.288 0 01.238.06Zm1.433.681a.293.293 0 01.108.113h.006l.848 1.532a3.443 3.443 0 01-2.218-.002l.845-1.529h.002a.293.293 0 01.409-.114ZM9.633 7.63l1.3-1.163c.4.647.575 1.408.502 2.164l-1.684-.485-.002-.007a.294.294 0 01-.115-.504V7.63Zm-1.067-.884a.294.294 0 01-.048-.149h-.002l-.099-1.744a3.46 3.46 0 011.995.961L8.99 6.824l-.005-.003a.294.294 0 01-.418-.075Zm-1.409.137a.294.294 0 01-.146-.055l-.003.001-1.433-1.015a3.43 3.43 0 012.008-.961l-.1 1.746-.007.004a.293.293 0 01-.319.28Zm-.775 1.185a.294.294 0 01-.135.08l-.001.005-1.677.484a3.429 3.429 0 01.487-2.17l1.308 1.17-.002.006a.293.293 0 01.02.425ZM8 8.893l-.483-.232-.12-.519.334-.416h.536l.333.417-.12.519-.48.231Z"/><path fill="currentColor" fill-rule="evenodd" d="m14.6 3.714 1.375 5.97a1.049 1.049 0 01-.04.595 1.041 1.041 0 01-.166.298l-3.85 4.789a1.05 1.05 0 01-.372.29c-.026.013-.054.024-.082.035a1.086 1.086 0 01-.377.071l-6.175.002a1.096 1.096 0 01-.778-.338l-.007-.007c-.016-.017-.032-.034-.047-.053L.231 10.58a1.101 1.101 0 01-.167-.3 1.067 1.067 0 01-.039-.593l1.373-5.971A1.073 1.073 0 011.973 3L7.537.341a1.072 1.072 0 01.923 0l5.564 2.657a1.074 1.074 0 01.576.716Zm-4.341 8.427a1.123 1.123 0 01-.056-.134 4.334 4.334 0 001.92-2.415l.142.025a.263.263 0 01.186-.07c.2.039.394.096.581.172.098.044.197.082.299.115l.053.012.033.006.007.002.004.001a.334.334 0 10.148-.649 1.946 1.946 0 01-.034-.008l-.063-.015a2.81 2.81 0 00-.319-.025 3.11 3.11 0 01-.597-.098.374.374 0 01-.144-.145l-.134-.039a4.327 4.327 0 00-.694-2.997 10.898 10.898 0 01.118-.106.263.263 0 01.063-.188c.153-.132.32-.249.495-.348.095-.049.188-.103.277-.162a1.449 1.449 0 00.073-.06.334.334 0 10-.415-.52l-.02.016c-.018.014-.039.03-.053.043a2.746 2.746 0 00-.22.233c-.137.15-.287.286-.45.406a.37.37 0 01-.201.022l-.127.09A4.364 4.364 0 008.367 3.97a11.302 11.302 0 01-.008-.148.265.265 0 01-.11-.167 3.12 3.12 0 01.039-.604c.021-.105.036-.21.045-.317V2.64a.334.334 0 10-.666.01v.085c.009.107.024.212.045.317.032.2.044.402.037.604a.363.363 0 01-.108.172l-.008.141a4.283 4.283 0 00-2.778 1.336 6.015 6.015 0 01-.12-.086.263.263 0 01-.198-.019 3.107 3.107 0 01-.449-.406 2.78 2.78 0 00-.219-.232 1.792 1.792 0 00-.059-.048l-.015-.011a.397.397 0 00-.232-.088.32.32 0 00-.265.118.353.353 0 00.082.49l.005.004.018.015.05.04c.09.06.182.114.277.162.176.1.342.216.495.348a.37.37 0 01.067.192l.107.095a4.31 4.31 0 00-.68 3.004l-.14.041a.46.46 0 01-.143.145 3.108 3.108 0 01-.596.098 2.78 2.78 0 00-.32.025 1.45 1.45 0 00-.062.015l-.027.006h-.003l-.005.002a.334.334 0 10.148.649h.005l.005-.002h.002c.008-.003.016-.004.025-.006l.061-.014a2.71 2.71 0 00.299-.115 3.12 3.12 0 01.58-.17.37.37 0 01.192.067l.145-.025a4.334 4.334 0 001.92 2.398l-.06.145a.33.33 0 01.03.188c-.084.19-.185.372-.302.542a2.773 2.773 0 00-.179.266l-.03.063-.012.027a.334.334 0 10.601.285v-.001a3.145 3.145 0 01.042-.085c.038-.1.07-.201.096-.305.057-.21.144-.412.258-.598a.27.27 0 01.143-.07l.076-.137a4.31 4.31 0 003.073.008 10.147 10.147 0 01.07.128.26.26 0 01.17.103 3.1 3.1 0 01.23.56c.026.104.058.206.096.306l.03.063.011.022a.333.333 0 00.568.106.335.335 0 00.034-.39l-.012-.026-.031-.064a2.774 2.774 0 00-.179-.266 3.098 3.098 0 01-.295-.528.26.26 0 01.026-.197Z" clip-rule="evenodd"/></svg>';
/// Icon Data of kubernetes_agent

  static const String kubernetes_agent =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M7.223.183a1.75 1.75 0 011.518 0l4.996 2.406c.48.231.829.668.947 1.187l1.234 5.407a1.75 1.75 0 01-.338 1.48l-3.233 4.054a.75.75 0 11-1.173-.935l3.233-4.054a.25.25 0 00.049-.212L13.222 4.11a.25.25 0 00-.136-.17L8.09 1.536a.25.25 0 00-.217 0L2.878 3.94a.25.25 0 00-.136.17L1.508 9.515a.25.25 0 00.049.212l3.233 4.054a.75.75 0 11-1.173.935L.384 10.663a1.75 1.75 0 01-.338-1.48L1.28 3.776a1.75 1.75 0 01.947-1.187L7.223.183ZM5.22 11.16a.75.75 0 000 1.06l2.25 2.25L8 15l.53-.53 2.25-2.25a.75.75 0 10-1.06-1.06l-.97.97V5.87l.97.97a.75.75 0 101.06-1.06L8.53 3.53 8 3l-.53.53-2.25 2.25a.75.75 0 001.06 1.06l.97-.97v6.26l-.97-.97a.75.75 0 00-1.06 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of label

  static const String label =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M9.664 1a1.75 1.75 0 00-1.237.512L1.514 8.419a1.75 1.75 0 00-.001 2.475L5.1 14.48a1.75 1.75 0 002.474 0l6.914-6.906A1.75 1.75 0 0015 6.335V1H9.664Zm-.177 1.573a.25.25 0 01.177-.073H13.5v3.835a.25.25 0 01-.073.177L6.513 13.42a.25.25 0 01-.353 0L2.574 9.833a.25.25 0 010-.353l6.913-6.907ZM11 6a1 1 0 100-2 1 1 0 000 2Z" clip-rule="evenodd"/></svg>';
/// Icon Data of labels

  static const String labels =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M6.427.512A1.75 1.75 0 017.664 0H13v3h3v5.335c0 .465-.185.91-.513 1.239L9.573 15.48a1.75 1.75 0 01-2.473 0l-2.293-2.293-1.293-1.293-3-3a1.75 1.75 0 010-2.475L6.428.512ZM11.5 1.5V3h-.836a1.75 1.75 0 00-1.237.512L3.514 9.419c-.06.06-.115.123-.165.19L1.574 7.833a.25.25 0 010-.353l5.913-5.907a.25.25 0 01.177-.073H11.5ZM5.866 12.126l-1.292-1.293a.25.25 0 010-.353l5.913-5.907a.25.25 0 01.177-.073H14.5v3.835a.25.25 0 01-.073.177L8.513 14.42a.25.25 0 01-.353 0l-2.294-2.293ZM12 8a1 1 0 100-2 1 1 0 000 2Z" clip-rule="evenodd"/></svg>';
/// Icon Data of leave

  static const String leave =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M11.16 4.28a.75.75 0 111.06-1.06l3.25 3.25L16 7l-.53.53-3.25 3.25a.75.75 0 01-1.06-1.06l1.97-1.97H3.25a1.75 1.75 0 100 3.5h2a.75.75 0 010 1.5h-2a3.25 3.25 0 010-6.5h9.88l-1.97-1.97Z" clip-rule="evenodd"/></svg>';
/// Icon Data of level_up

  static const String level_up =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M5.28 5.84a.75.75 0 01-1.06-1.06l3.25-3.25L8 1l.53.53 3.25 3.25a.75.75 0 01-1.06 1.06L8.75 3.87V12a1.5 1.5 0 001.5 1.5h4a.75.75 0 010 1.5h-4a3 3 0 01-3-3V3.87L5.28 5.84Z" clip-rule="evenodd"/></svg>';
/// Icon Data of license

  static const String license =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M8 10.5a4.5 4.5 0 100-9 4.5 4.5 0 000 9ZM14 6a5.997 5.997 0 01-2.886 5.13l.58 3.185L12 16l-1.623-.544L8 14.66l-2.377.796L4 16l.306-1.684.58-3.187A6 6 0 1114 6Zm-7.748 6h3.496l.322 1.772-1.594-.534-.476-.16-.476.16-1.594.534L6.252 12ZM9.5 6a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0ZM11 6a3 3 0 11-6 0 3 3 0 016 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of license_sm

  static const String license_sm =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="12" height="12" preserveAspectRatio="xMidYMid meet" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M6 6.5a2.5 2.5 0 100-5 2.5 2.5 0 000 5ZM10 4a3.997 3.997 0 01-1.842 3.369l.517 2.843L9 12l-1.66-.741L6 10.66l-1.34.599L3 12l.325-1.788.517-2.843A4 4 0 1110 4ZM5.252 8h1.496l.268 1.47-.404-.18L6 9.017l-.612.273-.404.18L5.252 8ZM6 5a1 1 0 100-2 1 1 0 000 2Z" clip-rule="evenodd"/></svg>';
/// Icon Data of link

  static const String link =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M9.929 3.132a2.078 2.078 0 112.94 2.94l-.65.648a.75.75 0 001.061 1.06l.649-.648a3.579 3.579 0 00-5.06-5.06L6.218 4.72a3.578 3.578 0 000 5.06.75.75 0 001.061-1.06 2.078 2.078 0 010-2.94L9.93 3.132Zm-.15 3.086a.75.75 0 00-1.057 1.064c.816.81.818 2.13.004 2.942l-2.654 2.647a2.08 2.08 0 01-2.94-2.944l.647-.647a.75.75 0 00-1.06-1.06l-.648.647a3.58 3.58 0 005.06 5.066l2.654-2.647a3.575 3.575 0 00-.007-5.068Z" clip-rule="evenodd"/></svg>';
/// Icon Data of linkedin

  static const String linkedin =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M3 1a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2V3a2 2 0 00-2-2H3Zm1.102 4.297a1.195 1.195 0 100-2.39 1.195 1.195 0 000 2.39Zm1 7.516V6.234h-2v6.579h2ZM6.43 6.234h2v.881c.295-.462.943-1.084 2.148-1.084 1.438 0 2.219.953 2.219 2.766 0 .087.008.484.008.484v3.531h-2v-3.53c0-.485-.102-1.438-1.18-1.438-1.079 0-1.17 1.198-1.195 1.982v2.986h-2V6.234Z" clip-rule="evenodd"/></svg>';
/// Icon Data of list_bulleted

  static const String list_bulleted =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M1 4.75a1 1 0 100-2 1 1 0 000 2ZM4.75 3a.75.75 0 000 1.5h10.5a.75.75 0 000-1.5H4.75Zm0 4.25a.75.75 0 000 1.5h10.5a.75.75 0 000-1.5H4.75Zm-.75 5a.75.75 0 01.75-.75h10.5a.75.75 0 010 1.5H4.75a.75.75 0 01-.75-.75ZM2 8a1 1 0 11-2 0 1 1 0 012 0Zm-1 5.25a1 1 0 100-2 1 1 0 000 2Z" clip-rule="evenodd"/></svg>';
/// Icon Data of list_indent

  static const String list_indent =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4.75 3a.75.75 0 000 1.5h10.5a.75.75 0 000-1.5H4.75Zm2 4.25a.75.75 0 000 1.5h8.5a.75.75 0 000-1.5h-8.5ZM.22 5.72a.75.75 0 011.06 0l1.75 1.75.53.53-.53.53-1.75 1.75A.75.75 0 11.22 9.22L1.44 8 .22 6.78a.75.75 0 010-1.06Zm4.53 5.78a.75.75 0 000 1.5h10.5a.75.75 0 000-1.5H4.75Z" clip-rule="evenodd"/></svg>';
/// Icon Data of list_numbered

  static const String list_numbered =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M0 2h2v4H1V3H0V2Zm1.637 9.008H0v-1h1.637a1.382 1.382 0 01.803 2.506L1.76 13H3v1H0v-.972L1.859 11.7a.382.382 0 00-.222-.693ZM4.75 3a.75.75 0 000 1.5h10.5a.75.75 0 000-1.5H4.75Zm0 4.25a.75.75 0 000 1.5h10.5a.75.75 0 000-1.5H4.75Zm-.75 5a.75.75 0 01.75-.75h10.5a.75.75 0 010 1.5H4.75a.75.75 0 01-.75-.75Z" clip-rule="evenodd"/></svg>';
/// Icon Data of list_outdent

  static const String list_outdent =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4.75 3a.75.75 0 000 1.5h10.5a.75.75 0 000-1.5H4.75Zm2 4.25a.75.75 0 000 1.5h8.5a.75.75 0 000-1.5h-8.5ZM3.34 5.72a.75.75 0 00-1.06 0L.53 7.47 0 8l.53.53 1.75 1.75a.75.75 0 101.06-1.06L2.12 8l1.22-1.22a.75.75 0 000-1.06Zm1.41 5.78a.75.75 0 000 1.5h10.5a.75.75 0 000-1.5H4.75Z" clip-rule="evenodd"/></svg>';
/// Icon Data of list_task

  static const String list_task =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4.78 3.28a.75.75 0 00-1.06-1.06L1.75 4.19l-.47-.47A.75.75 0 00.22 4.78l1 1a.75.75 0 001.06 0l2.5-2.5ZM6 3.75A.75.75 0 016.75 3h8.5a.75.75 0 010 1.5h-8.5A.75.75 0 016 3.75ZM6 8a.75.75 0 01.75-.75h8.5a.75.75 0 010 1.5h-8.5A.75.75 0 016 8Zm.75 3.5a.75.75 0 000 1.5h8.5a.75.75 0 000-1.5h-8.5Zm-1.97-1.28a.75.75 0 010 1.06l-2.5 2.5a.75.75 0 01-1.06 0l-1-1a.75.75 0 111.06-1.06l.47.47 1.97-1.97a.75.75 0 011.06 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of live_preview

  static const String live_preview =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M3 2.5h10a.5.5 0 01.5.5v10a.5.5 0 01-.5.5H3a.5.5 0 01-.5-.5V3a.5.5 0 01.5-.5ZM1 3a2 2 0 012-2h10a2 2 0 012 2v10a2 2 0 01-2 2H3a2 2 0 01-2-2V3Zm9.534 4.172a1 1 0 010 1.656L7.56 10.841A1 1 0 016 10.012V5.988a1 1 0 011.56-.829l2.974 2.013Z" clip-rule="evenodd"/></svg>';
/// Icon Data of live_stream

  static const String live_stream =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M11.596 1.164a.75.75 0 011.054.118 10.75 10.75 0 01-.017 13.438.75.75 0 11-1.17-.94 9.25 9.25 0 00.015-11.562.75.75 0 01.118-1.054Zm-3.84 2.97a.75.75 0 011.048.166 6.281 6.281 0 010 7.381.75.75 0 11-1.214-.881 4.781 4.781 0 000-5.619.75.75 0 01.167-1.047ZM3 10a2 2 0 100-4 2 2 0 000 4Z" clip-rule="evenodd"/></svg>';
/// Icon Data of location

  static const String location =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M12.5 6a4.47 4.47 0 01-.883 2.677L8 13.5 4.383 8.677A4.5 4.5 0 1112.5 6ZM14 6c0 1.34-.439 2.576-1.18 3.574L8.937 14.75 8 16l-.938-1.25L3.18 9.574A6 6 0 1114 6ZM8 8a2 2 0 100-4 2 2 0 000 4Z" clip-rule="evenodd"/></svg>';
/// Icon Data of location_dot

  static const String location_dot =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M11.617 8.677a4.5 4.5 0 10-7.235 0L8 13.5l3.617-4.823Zm1.203.897a6 6 0 10-9.64 0L6.875 14.5H4.75a.75.75 0 000 1.5h6.5a.75.75 0 000-1.5H9.125l3.695-4.926ZM8 8a2 2 0 100-4 2 2 0 000 4Z" clip-rule="evenodd"/></svg>';
/// Icon Data of lock

  static const String lock =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4 5a4 4 0 118 0v1h1a1 1 0 011 1v7a1 1 0 01-1 1H3a1 1 0 01-1-1V7a1 1 0 011-1h1V5Zm6.5 0v1h-5V5a2.5 2.5 0 015 0Zm-7 2.5v6h9v-6h-9ZM9 12V9H7v3h2Z" clip-rule="evenodd"/></svg>';
/// Icon Data of lock_open

  static const String lock_open =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M7.523.029a4 4 0 014.37 3.05.75.75 0 01-1.46.345 2.5 2.5 0 00-4.933.592V7H13a1 1 0 011 1v7a1 1 0 01-1 1H3a1 1 0 01-1-1V8a1 1 0 011-1h1.75H4V4.024A4 4 0 017.523.029ZM3.5 8.5v6h9v-6h-9ZM9 10H7v3h2v-3Z" clip-rule="evenodd"/></svg>';
/// Icon Data of log

  static const String log =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M3.5 2.5v11h9v-11h-9ZM3 1a1 1 0 00-1 1v12a1 1 0 001 1h10a1 1 0 001-1V2a1 1 0 00-1-1H3Zm5 10a.75.75 0 01.75-.75h1.75a.75.75 0 010 1.5H8.75A.75.75 0 018 11Zm-2 1a1 1 0 100-2 1 1 0 000 2Zm2-4a.75.75 0 01.75-.75h1.75a.75.75 0 010 1.5H8.75A.75.75 0 018 8ZM6 9a1 1 0 100-2 1 1 0 000 2Zm2-4a.75.75 0 01.75-.75h1.75a.75.75 0 010 1.5H8.75A.75.75 0 018 5ZM6 6a1 1 0 100-2 1 1 0 000 2Z" clip-rule="evenodd"/></svg>';
/// Icon Data of long_arrow

  static const String long_arrow =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M10.159 10.72a.75.75 0 101.06 1.06l3.25-3.25L15 8l-.53-.53-3.25-3.25a.75.75 0 00-1.061 1.06l1.97 1.97H1.75a.75.75 0 100 1.5h10.379l-1.97 1.97Z" clip-rule="evenodd"/></svg>';
/// Icon Data of mail

  static const String mail =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M2 3.5h12a.5.5 0 01.5.5v.572L8 8.286 1.5 4.572V4a.5.5 0 01.5-.5Zm-.5 2.8V12a.5.5 0 00.5.5h12a.5.5 0 00.5-.5V6.3L8.372 9.8 8 10.014 7.628 9.8 1.5 6.3ZM0 4a2 2 0 012-2h12a2 2 0 012 2v8a2 2 0 01-2 2H2a2 2 0 01-2-2V4Z" clip-rule="evenodd"/></svg>';
/// Icon Data of marquee_selection

  static const String marquee_selection =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M1.75 1H1v2.25a.75.75 0 001.5 0V2.5h.75a.75.75 0 000-1.5h-1.5Zm0 5a.75.75 0 01.75.75v2.5a.75.75 0 01-1.5 0v-2.5A.75.75 0 011.75 6Zm7.106 2.856 1.507 3.517 2.01-2.01-3.517-1.507Zm-.558-1.871 5.344 2.29a1 1 0 01.314 1.627l-.997.996 1.821 1.822a.75.75 0 11-1.06 1.06l-1.822-1.821-.996.997a1 1 0 01-1.627-.314l-2.29-5.344c-.356-.83.483-1.669 1.313-1.313ZM1.75 15H1v-2.25a.75.75 0 011.5 0v.75h.75a.75.75 0 010 1.5h-1.5ZM6 1.75A.75.75 0 016.75 1h2.5a.75.75 0 010 1.5h-2.5A.75.75 0 016 1.75ZM14.25 1H15v2.25a.75.75 0 01-1.5 0V2.5h-.75a.75.75 0 010-1.5h1.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of maximize

  static const String maximize =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M15 5.25a.75.75 0 01-1.5 0V3.56l-3.22 3.22a.75.75 0 11-1.06-1.06l3.22-3.22h-1.69a.75.75 0 010-1.5H15v4.25ZM3.81 13.5l2.97-2.97a.75.75 0 10-1.06-1.06L2.5 12.69v-1.94a.75.75 0 00-1.5 0V15h4.25a.75.75 0 000-1.5H3.81Z" clip-rule="evenodd"/></svg>';
/// Icon Data of media

  static const String media =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M13 2.5H3a.5.5 0 00-.5.5v10a.5.5 0 00.5.5h10a.5.5 0 00.5-.5V3a.5.5 0 00-.5-.5ZM3 1a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2V3a2 2 0 00-2-2H3Zm9 9.857L9.5 8l-2.476 2.83L5.5 9 4 10.8V12h8v-1.143ZM6.5 8a1.5 1.5 0 100-3 1.5 1.5 0 000 3Z" clip-rule="evenodd"/></svg>';
/// Icon Data of media_broken

  static const String media_broken =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M3 1a2 2 0 00-2 2v10a2 2 0 002 2h4.25a.75.75 0 000-1.5H3a.5.5 0 01-.5-.5V3a.5.5 0 01.5-.5h10a.5.5 0 01.5.5v4.25a.75.75 0 001.5 0V3a2 2 0 00-2-2H3Zm12 9.75a.75.75 0 00-1.5 0V13a.5.5 0 01-.5.5h-2.25a.75.75 0 000 1.5H13a2 2 0 002-2v-2.25Zm-3.867-.883L9.5 8l-2.476 2.83L5.5 9 4 10.8V12h5l2.133-2.133ZM6.5 8a1.5 1.5 0 100-3 1.5 1.5 0 000 3Z" clip-rule="evenodd"/></svg>';
/// Icon Data of merge

  static const String merge =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M5.5 3.5a1 1 0 11-2 0 1 1 0 012 0Zm-.044 2.31a2.5 2.5 0 10-1.706.076v4.228a2.501 2.501 0 101.5 0V8.373a5.735 5.735 0 003.86 1.864 2.501 2.501 0 10.01-1.504 4.254 4.254 0 01-3.664-2.922ZM11.5 10.5a1 1 0 100-2 1 1 0 000 2Zm-6 2a1 1 0 11-2 0 1 1 0 012 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of merge_request

  static const String merge_request =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M10.34 1.22a.75.75 0 00-1.06 0L7.53 2.97 7 3.5l.53.53 1.75 1.75a.75.75 0 101.06-1.06l-.47-.47h.63c.69 0 1.25.56 1.25 1.25v4.614a2.501 2.501 0 101.5 0V5.5a2.75 2.75 0 00-2.75-2.75h-.63l.47-.47a.75.75 0 000-1.06ZM13.5 12.5a1 1 0 11-2 0 1 1 0 012 0Zm-9 0a1 1 0 11-2 0 1 1 0 012 0Zm1.5 0a2.5 2.5 0 11-3.25-2.386V5.886a2.501 2.501 0 111.5 0v4.228A2.501 2.501 0 016 12.5Zm-1.5-9a1 1 0 11-2 0 1 1 0 012 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of merge_request_close

  static const String merge_request_close =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M1.22 1.22a.75.75 0 011.06 0L3.5 2.44l1.22-1.22a.75.75 0 011.06 1.06L4.56 3.5l1.22 1.22a.75.75 0 01-1.06 1.06L3.5 4.56 2.28 5.78a.75.75 0 01-1.06-1.06L2.44 3.5 1.22 2.28a.75.75 0 010-1.06ZM7.5 3.5a.75.75 0 01.75-.75h2.25a2.75 2.75 0 012.75 2.75v4.614a2.501 2.501 0 11-1.5 0V5.5c0-.69-.56-1.25-1.25-1.25H8.25a.75.75 0 01-.75-.75Zm5 10a1 1 0 100-2 1 1 0 000 2Zm-8-1a1 1 0 11-2 0 1 1 0 012 0Zm1.5 0a2.5 2.5 0 11-3.25-2.386V7.75a.75.75 0 011.5 0v2.364A2.501 2.501 0 016 12.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of merge_request_close_m

  static const String merge_request_close_m =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M5.28 4.22a.75.75 0 00-1.06 1.06L6.94 8l-2.72 2.72a.75.75 0 101.06 1.06L8 9.06l2.72 2.72a.75.75 0 101.06-1.06L9.06 8l2.72-2.72a.75.75 0 00-1.06-1.06L8 6.94 5.28 4.22Z" clip-rule="evenodd"/></svg>';
/// Icon Data of merge_request_open

  static const String merge_request_open =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M10.34 1.22a.75.75 0 00-1.06 0L7.53 2.97 7 3.5l.53.53 1.75 1.75a.75.75 0 101.06-1.06l-.47-.47h.63c.69 0 1.25.56 1.25 1.25v4.614a2.501 2.501 0 101.5 0V5.5a2.75 2.75 0 00-2.75-2.75h-.63l.47-.47a.75.75 0 000-1.06ZM13.5 12.5a1 1 0 11-2 0 1 1 0 012 0Zm-9 0a1 1 0 11-2 0 1 1 0 012 0Zm1.5 0a2.5 2.5 0 11-3.25-2.386V5.886a2.501 2.501 0 111.5 0v4.228A2.501 2.501 0 016 12.5Zm-1.5-9a1 1 0 11-2 0 1 1 0 012 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of messages

  static const String messages =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="m8.5 8.974 6 2.333V2.693l-6 2.333v3.948Zm6.138-7.944L7 4H3a3 3 0 000 6h.969l1.066 4.445A2.029 2.029 0 007.008 16c1.524 0 2.504-1.618 1.797-2.969l-1.53-2.924 7.363 2.863A1 1 0 0016 12.038V1.962a1 1 0 00-1.362-.932ZM5.51 10h.016l1.95 3.726a.529.529 0 11-.983.369L5.511 10ZM3 5.5h4v3H3a1.5 1.5 0 110-3Z" clip-rule="evenodd"/></svg>';
/// Icon Data of minimize

  static const String minimize =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M14.78 2.28a.75.75 0 00-1.06-1.06L10.5 4.44V2.75a.75.75 0 00-1.5 0V7h4.25a.75.75 0 000-1.5h-1.69l3.22-3.22ZM5.5 11.56v1.69a.75.75 0 001.5 0V9H2.75a.75.75 0 000 1.5h1.69l-3.22 3.22a.75.75 0 101.06 1.06l3.22-3.22Z" clip-rule="evenodd"/></svg>';
/// Icon Data of mobile

  static const String mobile =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M5 1.5h6a.5.5 0 01.5.5v12a.5.5 0 01-.5.5H5a.5.5 0 01-.5-.5V2a.5.5 0 01.5-.5ZM3 2a2 2 0 012-2h6a2 2 0 012 2v12a2 2 0 01-2 2H5a2 2 0 01-2-2V2Zm5 11a1 1 0 100-2 1 1 0 000 2Z" clip-rule="evenodd"/></svg>';
/// Icon Data of mobile_issue_close

  static const String mobile_issue_close =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M12.78 4.62a.75.75 0 010 1.06l-6.097 6.097a.75.75 0 01-1.069-.009L3.211 9.284a.75.75 0 111.078-1.042l1.873 1.935L11.72 4.62a.75.75 0 011.06 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of monitor

  static const String monitor =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M1.5 2.5v8h13v-8h-13ZM1 12h4v1.5H3.75a.75.75 0 000 1.5h8.5a.75.75 0 000-1.5H11V12h4a1 1 0 001-1V2a1 1 0 00-1-1H1a1 1 0 00-1 1v9a1 1 0 001 1Zm5.5 1.5V12h3v1.5h-3Zm5-7.75a.75.75 0 00-1.5 0v2.5a.75.75 0 001.5 0v-2.5ZM7.75 4a.75.75 0 01.75.75v3.5a.75.75 0 01-1.5 0v-3.5A.75.75 0 017.75 4ZM5.5 6.75a.75.75 0 00-1.5 0v1.5a.75.75 0 001.5 0v-1.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of monitor_lines

  static const String monitor_lines =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M1.5 10.5v-8h13v8h-13ZM5 12H1a1 1 0 01-1-1V2a1 1 0 011-1h14a1 1 0 011 1v9a1 1 0 01-1 1h-4v1.5h1.25a.75.75 0 010 1.5h-8.5a.75.75 0 010-1.5H5V12Zm1.5 0v1.5h3V12h-3ZM3 7.75A.75.75 0 013.75 7h5.5a.75.75 0 010 1.5h-5.5A.75.75 0 013 7.75Zm.75-3.25a.75.75 0 000 1.5h8.5a.75.75 0 000-1.5h-8.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of monitor_o

  static const String monitor_o =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M1.5 10.5v-8h13v8h-13ZM5 12H1a1 1 0 01-1-1V2a1 1 0 011-1h14a1 1 0 011 1v9a1 1 0 01-1 1h-4v1.5h1.25a.75.75 0 010 1.5h-8.5a.75.75 0 010-1.5H5V12Zm1.5 0v1.5h3V12h-3Z" clip-rule="evenodd"/></svg>';
/// Icon Data of namespace

  static const String namespace =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M7.3 4.125a.75.75 0 00-1.3-.75l-4.9 8.5a.75.75 0 101.3.75l4.9-8.5ZM8 12a1 1 0 11-2 0 1 1 0 012 0Zm2.5 1a1 1 0 100-2 1 1 0 000 2Zm3.5 0a1 1 0 100-2 1 1 0 000 2Z" clip-rule="evenodd"/></svg>';
/// Icon Data of nature

  static const String nature =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M14.98 2.5A6.37 6.37 0 0015 2V1h-1.75a6.003 6.003 0 00-5.761 4.32A5.99 5.99 0 002.75 3H1v1a6 6 0 006 6h.25v4.25a.75.75 0 001.5 0V8H9a6 6 0 005.98-5.5Zm-6.203 4a4.5 4.5 0 014.473-4h.223A4.5 4.5 0 019 6.5h-.223Zm-6.027-2a4.5 4.5 0 014.473 4H7a4.5 4.5 0 01-4.473-4h.223Z" clip-rule="evenodd"/></svg>';
/// Icon Data of notifications

  static const String notifications =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M8 1a1 1 0 00-1 1v.1A5.002 5.002 0 003 7v4l-1.205 1.328c-.583.643-.127 1.672.74 1.672h3.733a2 2 0 003.464 0h3.733c.867 0 1.323-1.03.74-1.672L13 11V7a5.002 5.002 0 00-4-4.9V2a1 1 0 00-1-1ZM4.5 11.58l-.39.428-.446.492h8.672l-.447-.492-.389-.429V7a3.5 3.5 0 10-7 0v4.58Z" clip-rule="evenodd"/></svg>';
/// Icon Data of notifications_off

  static const String notifications_off =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M8 1a1 1 0 00-1 1v.1A5.002 5.002 0 003 7v4.94l-1.78 1.78a.75.75 0 101.06 1.06L14.776 2.284a.75.75 0 00-1.06-1.06l-2.211 2.21A4.991 4.991 0 009 2.1V2a1 1 0 00-1-1Zm0 2.5c.95 0 1.813.379 2.444.995L4.5 10.439V7A3.5 3.5 0 018 3.5Zm5 4.25a.75.75 0 00-1.5 0v3.817l.194.214.65.719H6.75a.75.75 0 00-.728.932l.011.043A2.02 2.02 0 007.993 15c.737 0 1.389-.4 1.738-1h3.74c.868 0 1.324-1.028.742-1.671L13 10.989V7.75Z" clip-rule="evenodd"/></svg>';
/// Icon Data of object

  static const String object =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M8 14.5c.8 0 1.566-.145 2.274-.409l.665-2.046h2.15a6.472 6.472 0 001.405-4.327l-1.739-1.263.665-2.045a6.512 6.512 0 00-3.68-2.674L8 3 6.26 1.736A6.512 6.512 0 002.58 4.41l.665 2.045-1.739 1.263a6.472 6.472 0 001.406 4.327h2.15l.664 2.046A6.486 6.486 0 008 14.5ZM8 16A8 8 0 108 0a8 8 0 000 16ZM8 5.286l2.853 2.073-1.09 3.354H6.237L5.147 7.36 8 5.286Z" clip-rule="evenodd"/></svg>';
/// Icon Data of overview

  static const String overview =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M2.5 5.5v-3h3v3h-3ZM1 2a1 1 0 011-1h4a1 1 0 011 1v4a1 1 0 01-1 1H2a1 1 0 01-1-1V2Zm8 .25a.75.75 0 01.75-.75h4.5a.75.75 0 010 1.5h-4.5A.75.75 0 019 2.25ZM9.75 5a.75.75 0 000 1.5h4.5a.75.75 0 000-1.5h-4.5ZM2.5 10.5v3h3v-3h-3ZM2 9a1 1 0 00-1 1v4a1 1 0 001 1h4a1 1 0 001-1v-4a1 1 0 00-1-1H2Zm7.75.5a.75.75 0 000 1.5h4.5a.75.75 0 000-1.5h-4.5Zm0 3.5a.75.75 0 000 1.5h4.5a.75.75 0 000-1.5h-4.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of package

  static const String package =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M5.72 2.5 2.92 6h4.33V2.5H5.72Zm3.03 0V6h4.33l-2.8-3.5H8.75Zm-6.25 11v-6h11v6h-11ZM5.48 1a1 1 0 00-.78.375L1.22 5.726a1 1 0 00-.22.625V14a1 1 0 001 1h12a1 1 0 001-1V6.35a1 1 0 00-.22-.624l-3.48-4.35A1 1 0 0010.52 1H5.48Z" clip-rule="evenodd"/></svg>';
/// Icon Data of paper_airplane

  static const String paper_airplane =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M14.968 1.966a.75.75 0 00-.934-.934l-12.5 3.75a.75.75 0 00-.18 1.355L5.952 8.99l-1.731 1.73a.75.75 0 101.06 1.061l1.731-1.73 2.852 4.595a.75.75 0 001.355-.18l3.75-12.5ZM8.101 8.96l2.159 3.48 2.417-8.056L8.1 8.96Zm3.515-5.637L3.56 5.74 7.04 7.9l4.576-4.577Z" clip-rule="evenodd"/></svg>';
/// Icon Data of paperclip

  static const String paperclip =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M9.414 3.05 4.085 8.38a3 3 0 004.243 4.242l2.403-2.403a.75.75 0 111.06 1.06l-2.403 2.404a4.5 4.5 0 01-6.364-6.364l5.33-5.33a3.25 3.25 0 014.596 4.597l-5.33 5.329a2 2 0 01-2.828-2.828l5.33-5.33a.75.75 0 011.06 1.061l-5.33 5.33a.5.5 0 10.708.706l5.33-5.329A1.75 1.75 0 009.413 3.05Z" clip-rule="evenodd"/></svg>';
/// Icon Data of pause

  static const String pause =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M5 3a1 1 0 00-1 1v8a1 1 0 001 1h1a1 1 0 001-1V4a1 1 0 00-1-1H5Zm5 0a1 1 0 00-1 1v8a1 1 0 001 1h1a1 1 0 001-1V4a1 1 0 00-1-1h-1Z" clip-rule="evenodd"/></svg>';
/// Icon Data of pencil

  static const String pencil =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M2.5 13.5v-1.879l7.28-7.28 1.88 1.879-7.28 7.28H2.5Zm10.22-8.341.805-.805a.5.5 0 000-.708l-1.171-1.171a.5.5 0 00-.708 0l-.805.805 1.879 1.88ZM1 13.5V11l9.586-9.586a2 2 0 012.828 0l1.172 1.172a2 2 0 010 2.828L5 15H1v-1.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of pencil_square

  static const String pencil_square =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M14.287.303a1 1 0 111.415 1.414l-.707.708L13.58 1.01l.707-.707Zm0 2.829-6.873 6.873H6V8.59l6.873-6.874 1.415 1.415ZM3 13.5a.5.5 0 01-.5-.5V3a.5.5 0 01.5-.5h6.25a.75.75 0 000-1.5H3a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2V6.75a.75.75 0 00-1.5 0V13a.5.5 0 01-.5.5H3Z" clip-rule="evenodd"/></svg>';
/// Icon Data of pipeline

  static const String pipeline =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M14.457 8.75a6.501 6.501 0 01-12.914 0h.707a.75.75 0 000-1.5h-.707a6.469 6.469 0 011.361-3.285l.5.5a.75.75 0 001.06-1.061l-.5-.5A6.469 6.469 0 017.25 1.543v.707a.75.75 0 001.5 0v-.707a6.47 6.47 0 013.285 1.361l-.5.5a.75.75 0 101.061 1.06l.5-.5a6.469 6.469 0 011.361 3.286h-.707a.75.75 0 000 1.5h.707ZM16 8A8 8 0 110 8a8 8 0 0116 0Zm-8 4a2 2 0 00.75-3.855V4.75a.75.75 0 00-1.5 0v3.395A2 2 0 008 12Z" clip-rule="evenodd"/></svg>';
/// Icon Data of planning

  static const String planning =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M11.25 1a.75.75 0 01.75.75V3h2a1 1 0 011 1v10a1 1 0 01-1 1H2a1 1 0 01-1-1V4a1 1 0 011-1h2V1.75a.75.75 0 011.5 0V3h5V1.75a.75.75 0 01.75-.75ZM2.5 7.5v6h11v-6h-11Zm0-1.5h11V4.5h-11V6Zm7.78 2.97a.75.75 0 010 1.06l-2.25 2.25a.75.75 0 01-1.06 0l-1.25-1.25a.75.75 0 111.06-1.06l.72.72 1.72-1.72a.75.75 0 011.06 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of play

  static const String play =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" d="M11.629 7.306a.835.835 0 010 1.388l-6.401 4.177C4.695 13.218 4 12.825 4 12.176V3.824c0-.649.695-1.042 1.228-.695l6.4 4.177Z"/></svg>';
/// Icon Data of plus

  static const String plus =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M8.75 2.75a.75.75 0 00-1.5 0v4.5h-4.5a.75.75 0 000 1.5h4.5v4.5a.75.75 0 001.5 0v-4.5h4.5a.75.75 0 000-1.5h-4.5v-4.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of plus_square

  static const String plus_square =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4 2a2 2 0 00-2 2v8a2 2 0 002 2h8a2 2 0 002-2V4a2 2 0 00-2-2H4Zm4 9a.75.75 0 01-.75-.75v-1.5h-1.5a.75.75 0 010-1.5h1.5v-1.5a.75.75 0 011.5 0v1.5h1.5a.75.75 0 010 1.5h-1.5v1.5A.75.75 0 018 11Z" clip-rule="evenodd"/></svg>';
/// Icon Data of plus_square_o

  static const String plus_square_o =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4 3.5h8a.5.5 0 01.5.5v8a.5.5 0 01-.5.5H4a.5.5 0 01-.5-.5V4a.5.5 0 01.5-.5ZM2 4a2 2 0 012-2h8a2 2 0 012 2v8a2 2 0 01-2 2H4a2 2 0 01-2-2V4Zm6 7a.75.75 0 01-.75-.75v-1.5h-1.5a.75.75 0 010-1.5h1.5v-1.5a.75.75 0 011.5 0v1.5h1.5a.75.75 0 010 1.5h-1.5v1.5A.75.75 0 018 11Z" clip-rule="evenodd"/></svg>';
/// Icon Data of pod

  static const String pod =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="m8.248 1.87 4.485 2.562L8 7.136 3.268 4.432l4.484-2.563a.5.5 0 01.496 0ZM2.5 10.84V5.72l4.75 2.714v5.409l-4.498-2.57a.5.5 0 01-.252-.435Zm6.25 3.004 4.498-2.57a.5.5 0 00.252-.435V5.721L8.75 8.435v5.409ZM8.992.567a2 2 0 00-1.984 0l-5 2.857A2 2 0 001 5.161v5.678a2 2 0 001.008 1.737l5 2.857a2 2 0 001.984 0l5-2.857A2 2 0 0015 10.839V5.161a2 2 0 00-1.008-1.737l-5-2.857Z" clip-rule="evenodd"/></svg>';
/// Icon Data of podcast

  static const String podcast =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4.397 2.59a6.5 6.5 0 018.227 9.978.75.75 0 001.067 1.054 8 8 0 10-11.382 0 .75.75 0 101.067-1.054A6.5 6.5 0 014.397 2.59ZM8 4.5a3.5 3.5 0 012.5 5.949.75.75 0 101.072 1.05 5 5 0 10-7.144 0A.75.75 0 005.5 10.45 3.5 3.5 0 018 4.5ZM10 8a2 2 0 11-4 0 2 2 0 014 0Zm-1.25 4.25a.75.75 0 00-1.5 0v3a.75.75 0 001.5 0v-3Z" clip-rule="evenodd"/></svg>';
/// Icon Data of power

  static const String power =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M7.25 7.25a.75.75 0 001.5 0V.75a.75.75 0 00-1.5 0v6.5Zm4.04 5.157A5.5 5.5 0 115 3.39a.75.75 0 10-.818-1.257 7 7 0 107.635 0A.75.75 0 0011 3.39a5.5 5.5 0 01.291 9.017Z" clip-rule="evenodd"/></svg>';
/// Icon Data of preferences

  static const String preferences =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M12 6.5a1.5 1.5 0 100-3 1.5 1.5 0 000 3ZM12 8a3 3 0 10-2.905-3.75H1.75a.75.75 0 000 1.5h7.345A3.001 3.001 0 0012 8Zm-6.5 3a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0Zm1.405.75A3.001 3.001 0 011 11a3 3 0 015.905-.75h7.345a.75.75 0 010 1.5H6.905Z" clip-rule="evenodd"/></svg>';
/// Icon Data of profile

  static const String profile =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M8 14.5a6.47 6.47 0 003.25-.87V11.5A2.25 2.25 0 009 9.25H7a2.25 2.25 0 00-2.25 2.25v2.13A6.47 6.47 0 008 14.5Zm4.75-3v.937a6.5 6.5 0 10-9.5 0V11.5a3.752 3.752 0 012.486-3.532 3 3 0 114.528 0A3.752 3.752 0 0112.75 11.5ZM8 16A8 8 0 108 0a8 8 0 000 16ZM9.5 6a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of progress

  static const String progress =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4 5.5h8a2.5 2.5 0 010 5H4a2.5 2.5 0 010-5ZM0 8a4 4 0 014-4h8a4 4 0 010 8H4a4 4 0 01-4-4Zm4-1a1 1 0 000 2h5a1 1 0 000-2H4Z" clip-rule="evenodd"/></svg>';
/// Icon Data of project

  static const String project =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="m9.5 14.5-6-2.5V4l6-2.5v13Zm-6.885-1.244A1 1 0 012 12.333V3.667a1 1 0 01.615-.923L8.923.115A1.5 1.5 0 0111 1.5V2h1.25c.966 0 1.75.783 1.75 1.75v8.5A1.75 1.75 0 0112.25 14H11v.5a1.5 1.5 0 01-2.077 1.385l-6.308-2.629ZM11 12.5h1.25a.25.25 0 00.25-.25v-8.5a.25.25 0 00-.25-.25H11v9Z" clip-rule="evenodd"/></svg>';
/// Icon Data of push_rules

  static const String push_rules =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M3.25 4.07V1.75a.75.75 0 011.5 0v2.32a4.001 4.001 0 010 7.86v2.32a.75.75 0 01-1.5 0v-2.32a4.001 4.001 0 010-7.86ZM4 10.5a2.5 2.5 0 100-5 2.5 2.5 0 000 5ZM9.75 4a.75.75 0 000 1.5h5.5a.75.75 0 000-1.5h-5.5Zm0 3.25a.75.75 0 000 1.5h5.5a.75.75 0 000-1.5h-5.5Zm-.75 4a.75.75 0 01.75-.75h5.5a.75.75 0 010 1.5h-5.5a.75.75 0 01-.75-.75Z" clip-rule="evenodd"/></svg>';
/// Icon Data of question

  static const String question =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M8 16A8 8 0 108 0a8 8 0 000 16ZM4.927 4.99c-.285.429-.427.853-.427 1.27 0 .203.09.392.27.566.18.174.4.26.661.26.443 0 .744-.248.903-.746.168-.475.373-.835.616-1.08.243-.244.62-.366 1.134-.366.439 0 .797.12 1.075.363.277.242.416.54.416.892a.97.97 0 01-.136.502 1.91 1.91 0 01-.336.419 14.35 14.35 0 01-.648.558c-.34.282-.611.525-.812.73-.2.205-.362.443-.483.713-.322 1.245 1.35 1.345 1.736.456.047-.086.118-.18.213-.284.096-.103.223-.223.382-.36a41.14 41.14 0 001.194-1.034c.221-.204.412-.448.573-.73a1.95 1.95 0 00.242-.984c0-.475-.141-.915-.424-1.32-.282-.406-.682-.726-1.2-.962-.518-.235-1.115-.353-1.792-.353-.728 0-1.365.14-1.911.423-.546.282-.961.637-1.246 1.066Zm2.14 7.08a1 1 0 102 0 1 1 0 00-2 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of question_o

  static const String question_o =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M14.5 8a6.5 6.5 0 11-13 0 6.5 6.5 0 0113 0ZM16 8A8 8 0 110 8a8 8 0 0116 0ZM4.927 4.99c-.285.429-.427.853-.427 1.27 0 .203.09.392.27.566.18.174.4.26.661.26.443 0 .744-.248.903-.746.168-.475.373-.835.616-1.08.243-.244.62-.366 1.134-.366.439 0 .797.12 1.075.363.277.242.416.54.416.892a.97.97 0 01-.136.502 1.91 1.91 0 01-.336.419 14.35 14.35 0 01-.648.558c-.34.282-.611.525-.812.73-.2.205-.362.443-.483.713-.322 1.245 1.35 1.345 1.736.456.047-.086.118-.18.213-.284.096-.103.223-.223.382-.36a41.14 41.14 0 001.194-1.034c.221-.204.412-.448.573-.73a1.95 1.95 0 00.242-.984c0-.475-.141-.915-.424-1.32-.282-.406-.682-.726-1.2-.962-.518-.235-1.115-.353-1.792-.353-.728 0-1.365.14-1.911.423-.546.282-.961.637-1.246 1.066Zm2.14 7.08a1 1 0 102 0 1 1 0 00-2 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of quote

  static const String quote =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M1.5 3.75a.75.75 0 00-1.5 0v8.5a.75.75 0 001.5 0v-8.5ZM4.75 3a.75.75 0 000 1.5h7.5a.75.75 0 000-1.5h-7.5Zm0 4.25a.75.75 0 000 1.5h10.5a.75.75 0 000-1.5H4.75Zm-.75 5a.75.75 0 01.75-.75h6.5a.75.75 0 010 1.5h-6.5a.75.75 0 01-.75-.75Z" clip-rule="evenodd"/></svg>';
/// Icon Data of redo

  static const String redo =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M10.095.28A8 8 0 001.5 3.335V1.75a.75.75 0 00-1.5 0V6h4.25a.75.75 0 100-1.5H2.523a6.5 6.5 0 11-.526 5.994.75.75 0 00-1.385.575A8 8 0 1010.095.279Z" clip-rule="evenodd"/></svg>';
/// Icon Data of remove

  static const String remove =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M5.75 3V1.5h4.5V3h-4.5Zm-1.5 0V1a1 1 0 011-1h5.5a1 1 0 011 1v2h2.5a.75.75 0 010 1.5h-.365l-.743 9.653A2 2 0 0111.148 16H4.852a2 2 0 01-1.994-1.847L2.115 4.5H1.75a.75.75 0 010-1.5h2.5Zm-.63 1.5h8.76l-.734 9.538a.5.5 0 01-.498.462H4.852a.5.5 0 01-.498-.462L3.62 4.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of remove_all

  static const String remove_all =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M5.75 3V1.5h4.5V3h-4.5Zm-1.5 0V1a1 1 0 011-1h5.5a1 1 0 011 1v2h2.5a.75.75 0 010 1.5h-.365l-.743 9.653A2 2 0 0111.148 16H4.852a2 2 0 01-1.994-1.847L2.115 4.5H1.75a.75.75 0 010-1.5h2.5Zm-.63 1.5h8.76l-.734 9.538a.5.5 0 01-.498.462H4.852a.5.5 0 01-.498-.462L3.62 4.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of repeat

  static const String repeat =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M5.905.28A8 8 0 0114.5 3.335V1.75a.75.75 0 011.5 0V6h-4.25a.75.75 0 010-1.5h1.727a6.5 6.5 0 10.526 5.994.75.75 0 111.385.575A8 8 0 115.905.279Z" clip-rule="evenodd"/></svg>';
/// Icon Data of requirements

  static const String requirements =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M3 2.5a.5.5 0 00-.5.5v10a.5.5 0 00.5.5h5.25a.75.75 0 010 1.5H3a2 2 0 01-2-2V3a2 2 0 012-2h10a2 2 0 012 2v6.25a.75.75 0 01-1.5 0V3a.5.5 0 00-.5-.5H3Zm12.28 8.72a.75.75 0 010 1.06l-2.5 2.5a.75.75 0 01-1.06 0l-1-1a.75.75 0 111.06-1.06l.47.47 1.97-1.97a.75.75 0 011.06 0ZM4.75 4a.75.75 0 000 1.5h6.5a.75.75 0 000-1.5h-6.5ZM4 8a.75.75 0 01.75-.75h6.5a.75.75 0 010 1.5h-6.5A.75.75 0 014 8Zm.75 2.5a.75.75 0 000 1.5h3.5a.75.75 0 000-1.5h-3.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of retry

  static const String retry =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M7.32.029a8 8 0 017.18 3.307V1.75a.75.75 0 011.5 0V6h-4.25a.75.75 0 010-1.5h1.727A6.5 6.5 0 001.694 6.424.75.75 0 11.239 6.06 8 8 0 017.319.03Zm-3.4 14.852A8 8 0 0015.76 9.94a.75.75 0 00-1.455-.364A6.5 6.5 0 012.523 11.5H4.25a.75.75 0 000-1.5H0v4.25a.75.75 0 001.5 0v-1.586a8 8 0 002.42 2.217Z" clip-rule="evenodd"/></svg>';
/// Icon Data of review_checkmark

  static const String review_checkmark =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M8 3.5a1 1 0 100-2 1 1 0 000 2ZM8 0a2.5 2.5 0 012.45 2H13a1 1 0 011 1v12a1 1 0 01-1 1H3a1 1 0 01-1-1V3a1 1 0 011-1h2.55A2.5 2.5 0 018 0ZM7 5h3.5V3.5h2v11h-9v-11h2V5H7Zm3.53 3.28a.75.75 0 10-1.06-1.06L7.5 9.19l-.47-.47a.75.75 0 00-1.06 1.06l1 1a.75.75 0 001.06 0l2.5-2.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of review_list

  static const String review_list =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M9 2.5a1 1 0 11-2 0 1 1 0 012 0Zm1.45-.5a2.5 2.5 0 00-4.9 0H3a1 1 0 00-1 1v12a1 1 0 001 1h10a1 1 0 001-1V3a1 1 0 00-1-1h-2.55ZM8 5H5.5V3.5h-2v11h9v-11h-2V5H8ZM5 7.75A.75.75 0 015.75 7h4.5a.75.75 0 010 1.5h-4.5A.75.75 0 015 7.75Zm.75 1.75a.75.75 0 000 1.5h4.5a.75.75 0 000-1.5h-4.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of review_warning

  static const String review_warning =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M9 2.5a1 1 0 11-2 0 1 1 0 012 0Zm1.45-.5a2.5 2.5 0 00-4.9 0H3a1 1 0 00-1 1v12a1 1 0 001 1h10a1 1 0 001-1V3a1 1 0 00-1-1h-2.55ZM8 5H5.5V3.5h-2v11h9v-11h-2V5H8Zm1 7a1 1 0 11-2 0 1 1 0 012 0Zm-.25-4.75a.75.75 0 00-1.5 0v2a.75.75 0 001.5 0v-2Z" clip-rule="evenodd"/></svg>';
/// Icon Data of rocket

  static const String rocket =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="m16 .776.027-.803-.803.027-1.309.046A10.75 10.75 0 005.753 4.25H3.667A2.75 2.75 0 00.962 6.504l-.8 4.36L0 11.75h2.69l1.56 1.56V16l.885-.162 4.36-.8a2.75 2.75 0 002.255-2.705v-2.086a10.75 10.75 0 004.204-8.162L16 .775ZM9.348 9.988l-4.2 2.1-1.235-1.236 2.1-4.2a9.25 9.25 0 017.954-5.107l.506-.018-.018.506a9.25 9.25 0 01-5.107 7.955ZM5.75 14.2v-.736l4.268-2.135.232-.116v1.12a1.25 1.25 0 01-1.025 1.23L5.75 14.2Zm-3.214-3.95 2.135-4.268.115-.232h-1.12a1.25 1.25 0 00-1.229 1.025L1.8 10.25h.736ZM10.5 6a.5.5 0 11-1 0 .5.5 0 011 0ZM12 6a2 2 0 11-4 0 2 2 0 014 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of rss

  static const String rss =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M6.529 4.18a9.5 9.5 0 00-3.76-.677.75.75 0 11-.037-1.5A11 11 0 0113.997 13.27a.75.75 0 11-1.5-.036A9.5 9.5 0 006.53 4.18ZM4.624 8.803a4.5 4.5 0 00-1.838-.298.75.75 0 11-.07-1.498 6 6 0 016.277 6.282.75.75 0 01-1.498-.072 4.5 4.5 0 00-2.871-4.414ZM3 14a1 1 0 100-2 1 1 0 000 2Z" clip-rule="evenodd"/></svg>';
/// Icon Data of scale

  static const String scale =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M8 1a.75.75 0 01.75.75V3h5.5a.75.75 0 010 1.5h-1.393l.565.79 1.086 1.521A2.7 2.7 0 0115 8.375C15 9.825 13.88 11 12.5 11S10 9.825 10 8.375a2.7 2.7 0 01.492-1.564l1.086-1.52.565-.791H8.75v9.75a.749.749 0 01-.043.25h2.543a.75.75 0 010 1.5h-6.5a.75.75 0 010-1.5h2.543a.749.749 0 01-.043-.25V4.5H3.857l.565.79 1.086 1.521A2.7 2.7 0 016 8.375C6 9.825 4.88 11 3.5 11S1 9.825 1 8.375a2.7 2.7 0 01.492-1.564l1.086-1.52.565-.791H1.75a.75.75 0 010-1.5h5.5V1.75A.75.75 0 018 1Zm5.5 7.375c0-.27-.083-.508-.215-.695l-.785-1.1-.785 1.1a1.201 1.201 0 00-.215.695c0 .691.516 1.125 1 1.125s1-.434 1-1.125ZM4.285 7.68c.132.187.215.425.215.695 0 .691-.516 1.125-1 1.125s-1-.434-1-1.125c0-.27.083-.508.215-.695l.785-1.1.785 1.1Z" clip-rule="evenodd"/></svg>';
/// Icon Data of scroll_down

  static const String scroll_down =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M10 14a2 2 0 11-4 0 2 2 0 014 0Zm1.78-8.841a.75.75 0 00-1.06 0l-1.97 1.97V.75a.75.75 0 00-1.5 0v6.379l-1.97-1.97a.75.75 0 00-1.06 1.06l3.25 3.25L8 10l.53-.53 3.25-3.25a.75.75 0 000-1.061Z" clip-rule="evenodd"/></svg>';
/// Icon Data of scroll_handle

  static const String scroll_handle =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M8 16A8 8 0 108 0a8 8 0 000 16ZM6 3a1 1 0 011 1v8a1 1 0 11-2 0V4a1 1 0 011-1Zm4 0a1 1 0 011 1v8a1 1 0 11-2 0V4a1 1 0 011-1Z" clip-rule="evenodd"/></svg>';
/// Icon Data of scroll_up

  static const String scroll_up =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M10 2a2 2 0 10-4 0 2 2 0 004 0Zm1.78 8.841a.75.75 0 01-1.06 0l-1.97-1.97v6.379a.75.75 0 01-1.5 0V8.871l-1.97 1.97a.75.75 0 11-1.06-1.06l3.25-3.25L8 6l.53.53 3.25 3.25a.75.75 0 010 1.061Z" clip-rule="evenodd"/></svg>';
/// Icon Data of search

  static const String search =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M11.5 7a4.5 4.5 0 11-9 0 4.5 4.5 0 019 0Zm-.82 4.74a6 6 0 111.06-1.06l3.04 3.04a.75.75 0 11-1.06 1.06l-3.04-3.04Z" clip-rule="evenodd"/></svg>';
/// Icon Data of search_dot

  static const String search_dot =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M11.5 7a4.5 4.5 0 11-9 0 4.5 4.5 0 019 0Zm-.82 4.74a6 6 0 111.06-1.06l3.04 3.04a.75.75 0 11-1.06 1.06l-3.04-3.04ZM9 7a2 2 0 11-4 0 2 2 0 014 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of search_minus

  static const String search_minus =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M11.5 7a4.5 4.5 0 11-9 0 4.5 4.5 0 019 0Zm-.82 4.74a6 6 0 111.06-1.06l3.04 3.04a.75.75 0 11-1.06 1.06l-3.04-3.04ZM4 7a.75.75 0 01.75-.75h4.5a.75.75 0 010 1.5h-4.5A.75.75 0 014 7Z" clip-rule="evenodd"/></svg>';
/// Icon Data of search_plus

  static const String search_plus =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M7 11.5a4.5 4.5 0 100-9 4.5 4.5 0 000 9ZM7 13c1.387 0 2.663-.47 3.68-1.26l3.04 3.04a.75.75 0 101.06-1.06l-3.04-3.04A6 6 0 107 13Zm0-3a.75.75 0 01-.75-.75v-1.5h-1.5a.75.75 0 010-1.5h1.5v-1.5a.75.75 0 011.5 0v1.5h1.5a.75.75 0 010 1.5h-1.5v1.5A.75.75 0 017 10Z" clip-rule="evenodd"/></svg>';
/// Icon Data of search_sm

  static const String search_sm =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="12" height="12" preserveAspectRatio="xMidYMid meet" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M8.5 5.5a3 3 0 11-6 0 3 3 0 016 0Zm-.393 3.668a4.5 4.5 0 111.06-1.06l2.613 2.612a.75.75 0 11-1.06 1.06L8.107 9.168Z" clip-rule="evenodd"/></svg>';
/// Icon Data of settings

  static const String settings =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" d="m6.43 1.168-.74-.123.74.123Zm-.156.939.74.123-.74-.123Zm-.173.2.237.711-.237-.711ZM4.02 3.509l-.498-.56.498.56Zm-.26.05.263-.702-.263.702Zm-.893-.334.263-.703-.263.703Zm-.608.218-.65-.375.65.375ZM1.183 5.307l-.65-.375.65.375Zm.115.636.477-.579-.477.58Zm.736.606-.477.579.477-.58Zm.086.25.735.149-.735-.15Zm0 2.403.735-.15-.735.15Zm-.086.25.476.578-.476-.579Zm-.736.605-.476-.58.476.58Zm-.115.636-.65.375.65-.375Zm1.077 1.864.65-.375-.65.375Zm.608.218.263.703-.263-.703Zm.893-.334-.263-.703.263.703Zm.26.05-.498.56.498-.56Zm2.08 1.202.237-.711-.237.711Zm.173.2.74-.123-.74.123Zm.156.94.74-.124-.74.123Zm3.14 0-.74-.124.74.123Zm.156-.94.74.123-.74-.123Zm.173-.2.238.712-.238-.712Zm2.08-1.202-.497-.561.497.56Zm.26-.05.263-.703-.263.703Zm.893.334-.263.703.263-.703Zm.609-.218-.65-.375.65.375Zm1.076-1.864.65.375-.65-.375Zm-.115-.636-.477.579.477-.58Zm-.736-.606-.476.58.476-.58Zm-.086-.25.735.15-.735-.15Zm0-2.403.735-.15-.735.15Zm.086-.25-.476-.578.476.579Zm.736-.605-.477-.579.477.58Zm.115-.636.65-.375-.65.375ZM13.74 3.443l-.65.375.65-.375Zm-.609-.218-.263-.703.263.703Zm-.893.334-.263-.702.263.702Zm-.26-.05-.497.561.497-.56ZM9.9 2.307l-.237.711.237-.711Zm-.173-.2.74-.123-.74.123Zm-.156-.94-.74.124.74-.123ZM6.924 1.5h2.152V0H6.924v1.5Zm.246-.209a.25.25 0 01-.246.209V0A1.25 1.25 0 005.69 1.044l1.48.247Zm-.156.94.156-.94-1.48-.246-.156.939 1.48.246Zm-.676.787c.343-.114.613-.41.676-.788l-1.48-.246a.494.494 0 01.33-.389l.474 1.423ZM4.518 4.07a5.244 5.244 0 011.82-1.052l-.474-1.423a6.744 6.744 0 00-2.34 1.353l.994 1.122Zm-1.02.192c.36.134.75.048 1.02-.192l-.995-1.122a.494.494 0 01.501-.091l-.526 1.405Zm-.893-.335.893.335.526-1.405-.893-.335-.526 1.405Zm.304-.11a.25.25 0 01-.304.11l.526-1.405a1.25 1.25 0 00-1.521.546l1.3.75ZM1.833 5.683l1.076-1.864-1.299-.75L.534 4.932l1.299.75Zm-.058-.318a.25.25 0 01.058.318l-1.3-.75a1.25 1.25 0 00.289 1.59l.953-1.158Zm.735.606-.735-.606-.953 1.158.735.606.953-1.158Zm.345.978a1.006 1.006 0 00-.345-.978l-.953 1.158a.494.494 0 01-.172-.48l1.47.3ZM2.75 8c0-.361.036-.713.105-1.052l-1.47-.3c-.088.438-.135.89-.135 1.352h1.5Zm.105 1.052A5.277 5.277 0 012.75 8h-1.5c0 .462.047.914.135 1.351l1.47-.299Zm-.345.978c.296-.243.417-.624.345-.978l-1.47.3a.494.494 0 01.172-.48l.953 1.158Zm-.735.606.735-.606-.953-1.158-.735.606.953 1.158Zm.058-.318a.25.25 0 01-.058.318L.822 9.478a1.25 1.25 0 00-.288 1.59l1.299-.75Zm1.076 1.864-1.076-1.864-1.3.75 1.077 1.864 1.3-.75Zm-.304-.109a.25.25 0 01.304.11l-1.299.75c.306.528.949.76 1.521.545l-.526-1.405Zm.893-.335-.893.335.526 1.405.893-.335-.526-1.405Zm1.02.192a1.006 1.006 0 00-1.02-.191l.526 1.404a.494.494 0 01-.5-.091l.994-1.122Zm1.82 1.052a5.243 5.243 0 01-1.82-1.052l-.995 1.122a6.745 6.745 0 002.34 1.353l.475-1.423Zm.676.788a1.006 1.006 0 00-.676-.788l-.474 1.423a.494.494 0 01-.33-.388l1.48-.247Zm.156.939-.156-.94-1.48.248.157.939 1.48-.247Zm-.246-.209a.25.25 0 01.246.209l-1.48.247A1.25 1.25 0 006.925 16v-1.5Zm2.152 0H6.924V16h2.152v-1.5Zm-.246.209a.25.25 0 01.246-.209V16a1.25 1.25 0 001.233-1.044l-1.48-.247Zm.156-.94-.156.94 1.48.247.156-.94-1.48-.246Zm.676-.787c-.343.114-.613.41-.676.788l1.48.246a.494.494 0 01-.33.389l-.474-1.423Zm1.82-1.052a5.244 5.244 0 01-1.82 1.052l.474 1.423a6.745 6.745 0 002.34-1.353l-.994-1.122Zm1.02-.191a1.006 1.006 0 00-1.02.19l.995 1.123a.494.494 0 01-.501.091l.526-1.405Zm.893.334-.893-.335-.526 1.405.893.335.526-1.405Zm-.304.11a.25.25 0 01.304-.11l-.526 1.405a1.25 1.25 0 001.521-.546l-1.299-.75Zm1.076-1.865-1.076 1.864 1.299.75 1.076-1.864-1.299-.75Zm.058.318a.25.25 0 01-.058-.318l1.3.75a1.25 1.25 0 00-.289-1.59l-.953 1.158Zm-.735-.606.735.606.953-1.158-.735-.606-.953 1.158Zm-.345-.978c-.072.354.049.735.345.978l.953-1.158c.15.123.206.311.172.48l-1.47-.3ZM13.25 8c0 .361-.036.713-.105 1.052l1.47.3c.088-.438.135-.89.135-1.352h-1.5Zm-.105-1.052c.069.339.105.69.105 1.052h1.5c0-.462-.046-.914-.135-1.351l-1.47.299Zm.345-.978a1.007 1.007 0 00-.345.978l1.47-.3a.494.494 0 01-.172.48L13.49 5.97Zm.735-.606-.735.606.953 1.158.735-.606-.953-1.158Zm-.058.318a.25.25 0 01.058-.318l.953 1.158a1.25 1.25 0 00.288-1.59l-1.299.75Zm-1.076-1.864 1.076 1.864 1.3-.75-1.077-1.864-1.299.75Zm.304.109a.25.25 0 01-.304-.11l1.299-.75a1.25 1.25 0 00-1.521-.545l.526 1.405Zm-.893.334.893-.334-.526-1.405-.893.335.526 1.404Zm-1.02-.19c.27.24.66.325 1.02.19l-.526-1.404a.494.494 0 01.5.091l-.994 1.122Zm-1.82-1.053a5.244 5.244 0 011.82 1.052l.995-1.122a6.744 6.744 0 00-2.34-1.353l-.475 1.423Zm-.676-.788c.063.379.333.674.676.788l.474-1.423a.494.494 0 01.33.389l-1.48.246Zm-.156-.939.156.94 1.48-.247-.157-.94-1.48.247Zm.246.209a.25.25 0 01-.246-.209l1.48-.246A1.25 1.25 0 009.075 0v1.5ZM10.25 8A2.25 2.25 0 018 10.25v1.5A3.75 3.75 0 0011.75 8h-1.5ZM8 5.75A2.25 2.25 0 0110.25 8h1.5A3.75 3.75 0 008 4.25v1.5ZM5.75 8A2.25 2.25 0 018 5.75v-1.5A3.75 3.75 0 004.25 8h1.5ZM8 10.25A2.25 2.25 0 015.75 8h-1.5A3.75 3.75 0 008 11.75v-1.5Z"/></svg>';
/// Icon Data of severity_critical

  static const String severity_critical =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="12" height="12" preserveAspectRatio="xMidYMid meet" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="m6.706.185 4.088 2.31c.437.246.706.702.706 1.195v4.62c0 .493-.269.949-.706 1.195l-4.088 2.31a1.438 1.438 0 01-1.412 0l-4.088-2.31A1.376 1.376 0 01.5 8.31V3.69c0-.493.269-.949.706-1.195L5.294.185a1.438 1.438 0 011.412 0Z"/></svg>';
/// Icon Data of severity_high

  static const String severity_high =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="12" height="12" preserveAspectRatio="xMidYMid meet" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="m6.713.295 4.992 4.992a1.008 1.008 0 010 1.426l-4.992 4.992a1.008 1.008 0 01-1.426 0L.295 6.713a1.008 1.008 0 010-1.426L5.287.295a1.008 1.008 0 011.426 0Z"/></svg>';
/// Icon Data of severity_info

  static const String severity_info =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="12" height="12" preserveAspectRatio="xMidYMid meet" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M12 6A6 6 0 110 6a6 6 0 0112 0ZM6.75 3.75a.75.75 0 11-1.5 0 .75.75 0 011.5 0ZM6 5.25a.75.75 0 00-.75.75v2.25a.75.75 0 101.5 0V6A.75.75 0 006 5.25Z"/></svg>';
/// Icon Data of severity_low

  static const String severity_low =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="12" height="12" preserveAspectRatio="xMidYMid meet" viewBox="0 0 12 12"><circle cx="6" cy="6" r="6" fill="currentColor" fill-rule="evenodd"/></svg>';
/// Icon Data of severity_medium

  static const String severity_medium =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="12" height="12" preserveAspectRatio="xMidYMid meet" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M5.04 10.454.24 3.182c-.398-.603-.29-1.457.24-1.91C.688 1.097.94 1 1.2 1h9.6c.663 0 1.2.61 1.2 1.364 0 .295-.084.582-.24.818l-4.8 7.272c-.398.603-1.15.725-1.68.273a1.295 1.295 0 01-.24-.273Z"/></svg>';
/// Icon Data of severity_unknown

  static const String severity_unknown =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="12" height="12" preserveAspectRatio="xMidYMid meet" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M6 12A6 6 0 116 0a6 6 0 010 12ZM3.324 4.643c0-.313.107-.631.32-.953.214-.322.526-.589.935-.8.41-.212.887-.317 1.433-.317.508 0 .956.088 1.344.265.389.176.689.417.9.72.212.304.318.635.318.991 0 .281-.06.527-.18.738a2.287 2.287 0 01-.431.548c-.167.153-.465.412-.895.775a3.549 3.549 0 00-.287.27 1.1 1.1 0 00-.16.213c-.289.667-1.543.592-1.302-.342a1.82 1.82 0 01.363-.535 7.98 7.98 0 01.609-.547c.224-.185.385-.325.485-.419.1-.094.184-.199.252-.314a.727.727 0 00.103-.377.854.854 0 00-.313-.669c-.208-.181-.477-.272-.806-.272-.385 0-.668.092-.85.275-.182.183-.336.453-.462.81-.12.373-.345.56-.677.56a.686.686 0 01-.496-.196c-.135-.13-.203-.272-.203-.424ZM6 9.75a.75.75 0 110-1.5.75.75 0 010 1.5Z"/></svg>';
/// Icon Data of share

  static const String share =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M12.5 4.5a1 1 0 100-2 1 1 0 000 2Zm0 1.5a2.5 2.5 0 10-2.469-2.104L5.298 6.263a2.5 2.5 0 100 3.475l4.733 2.366a2.5 2.5 0 10.671-1.341L5.97 8.396a2.519 2.519 0 000-.792l4.733-2.367c.455.47 1.092.763 1.798.763Zm1 6.5a1 1 0 11-2 0 1 1 0 012 0ZM4.5 8a1 1 0 11-2 0 1 1 0 012 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of shield

  static const String shield =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M2.5 5.4V3.132l4.75-1.357v11.608l-1.782-1.528A8.5 8.5 0 012.5 5.401Zm6.25 7.982 1.782-1.528A8.5 8.5 0 0013.5 5.401V3.13L8.75 1.774v11.608ZM1 2l7-2 7 2v3.4a10 10 0 01-3.492 7.593L8 16l-3.508-3.007A10 10 0 011 5.401V2Z" clip-rule="evenodd"/></svg>';
/// Icon Data of sidebar

  static const String sidebar =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M13 2.5H8v11h5a.5.5 0 00.5-.5V3a.5.5 0 00-.5-.5Zm-10 0h3.5v11H3a.5.5 0 01-.5-.5V3a.5.5 0 01.5-.5ZM3 1a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2V3a2 2 0 00-2-2H3Zm1.5 11.5a1 1 0 100-2 1 1 0 000 2Zm1-4.5a1 1 0 11-2 0 1 1 0 012 0Zm-1-2.5a1 1 0 100-2 1 1 0 000 2Z" clip-rule="evenodd"/></svg>';
/// Icon Data of skype

  static const String skype =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" d="M8.028 12.213c-2.325 0-3.388-1.184-3.388-2.053a.754.754 0 01.795-.754c.996 0 .736 1.494 2.593 1.494.95 0 1.508-.572 1.508-1.11 0-.322-.185-.69-.812-.838L6.65 8.429c-1.666-.424-1.957-1.351-1.957-2.21 0-1.785 1.632-2.43 3.186-2.43 1.43 0 3.132.79 3.132 1.86 0 .461-.385.708-.832.708-.85 0-.708-1.194-2.418-1.194-.85 0-1.298.398-1.298.956 0 .558.662.747 1.243.874l1.53.345c1.678.378 2.125 1.363 2.125 2.305 0 1.45-1.122 2.57-3.335 2.57h.002Zm6.422-3.067c.066-.381.1-.767.099-1.153a6.55 6.55 0 00-7.685-6.479A3.854 3.854 0 004.933 1a3.918 3.918 0 00-3.381 5.857 6.555 6.555 0 007.587 7.629 3.852 3.852 0 001.93.514 3.918 3.918 0 003.38-5.854Z"/></svg>';
/// Icon Data of slight_frown

  static const String slight_frown =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M14.5 8a6.5 6.5 0 11-13 0 6.5 6.5 0 0113 0ZM16 8A8 8 0 110 8a8 8 0 0116 0ZM6 8a1 1 0 100-2 1 1 0 000 2Zm5-1a1 1 0 11-2 0 1 1 0 012 0ZM6.63 9.616a2.75 2.75 0 013.751 1.009.75.75 0 01-1.299.75 1.25 1.25 0 00-2.163-.003.75.75 0 11-1.297-.753 2.75 2.75 0 011.007-1.003Z" clip-rule="evenodd"/></svg>';
/// Icon Data of slight_smile

  static const String slight_smile =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M14.5 8a6.5 6.5 0 11-13 0 6.5 6.5 0 0113 0ZM16 8A8 8 0 110 8a8 8 0 0116 0ZM6 8a1 1 0 100-2 1 1 0 000 2Zm5-1a1 1 0 11-2 0 1 1 0 012 0Zm-4.37 4.384a2.749 2.749 0 003.751-1.009.75.75 0 00-1.299-.75 1.25 1.25 0 01-2.163.003.75.75 0 00-1.297.753c.242.417.59.763 1.007 1.003Z" clip-rule="evenodd"/></svg>';
/// Icon Data of smart_card

  static const String smart_card =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M14 3.5H2a.5.5 0 00-.5.5v8a.5.5 0 00.5.5h12a.5.5 0 00.5-.5V4a.5.5 0 00-.5-.5ZM2 2a2 2 0 00-2 2v8a2 2 0 002 2h12a2 2 0 002-2V4a2 2 0 00-2-2H2Zm4 7h3v1a1 1 0 01-1 1H6V9Zm-2 2h1V9H3v1a1 1 0 001 1Zm3-6h1a1 1 0 011 1v2H7V5ZM4 5a1 1 0 00-1 1v2h3V5H4Z" clip-rule="evenodd"/></svg>';
/// Icon Data of smile

  static const String smile =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M14.5 8a6.5 6.5 0 11-13 0 6.5 6.5 0 0113 0ZM16 8A8 8 0 110 8a8 8 0 0116 0ZM5.252 4.7a1.5 1.5 0 012.047.55.5.5 0 01-.866.5.5.5 0 00-.865-.001.5.5 0 11-.865-.502 1.5 1.5 0 01.55-.547ZM8 12a4 4 0 004-4H4a4 4 0 004 4Zm1.252-7.3a1.5 1.5 0 012.047.55.5.5 0 11-.866.5.5.5 0 00-.865-.001.5.5 0 11-.865-.502 1.5 1.5 0 01.55-.547Z" clip-rule="evenodd"/></svg>';
/// Icon Data of smiley

  static const String smiley =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M14.5 8a6.5 6.5 0 11-13 0 6.5 6.5 0 0113 0ZM16 8A8 8 0 110 8a8 8 0 0116 0ZM7 6a1 1 0 11-2 0 1 1 0 012 0Zm1 6a3 3 0 003-3H5a3 3 0 003 3Zm3-6a1 1 0 11-2 0 1 1 0 012 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of snippet

  static const String snippet =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M3.625.1A.75.75 0 014.65.375L8 6.177 11.35.375a.75.75 0 111.3.75L8.864 7.677l1.97 3.412A2.503 2.503 0 0114 13.5a2.5 2.5 0 11-4.425-1.595L7.999 9.176l-.26.45a.75.75 0 01-1.298-.751l.692-1.199L3.35 1.125A.75.75 0 013.625.1ZM5.5 13.5a1 1 0 11-2 0 1 1 0 012 0Zm1.5 0a2.5 2.5 0 11-5 0 2.5 2.5 0 015 0Zm5.5 0a1 1 0 11-2 0 1 1 0 012 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of soft_unwrap

  static const String soft_unwrap =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M.75 4.25a.75.75 0 000 1.5h14.5a.75.75 0 000-1.5H.75Zm9 6a.75.75 0 000 1.5h5.5a.75.75 0 000-1.5h-5.5Zm-4.56 0-.97-.97a.75.75 0 011.06-1.06l2.25 2.25.53.53-.53.53-2.25 2.25a.75.75 0 01-1.06-1.06l.97-.97H.75a.75.75 0 010-1.5h4.44Z" clip-rule="evenodd"/></svg>';
/// Icon Data of soft_wrap

  static const String soft_wrap =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M0 5a.75.75 0 01.75-.75h11.5a3.75 3.75 0 110 7.5H9.87l.97.97a.75.75 0 11-1.06 1.06l-2.25-2.25L7 11l.53-.53 2.25-2.25a.75.75 0 111.06 1.06l-.97.97h2.38a2.25 2.25 0 000-4.5H.75A.75.75 0 010 5Zm6 6a.75.75 0 00-.75-.75H.75a.75.75 0 000 1.5h4.5A.75.75 0 006 11Z" clip-rule="evenodd"/></svg>';
/// Icon Data of sort_highest

  static const String sort_highest =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="m3 14 .53-.53 2.25-2.25a.75.75 0 00-1.06-1.061l-.97.97v-8.38a.75.75 0 00-1.5 0v8.38l-.97-.97a.75.75 0 10-1.06 1.06l2.25 2.25L3 14ZM8.75 4a.75.75 0 000 1.5h6.5a.75.75 0 000-1.5h-6.5Zm0 3.25a.75.75 0 000 1.5h4.5a.75.75 0 000-1.5h-4.5Zm-.75 4a.75.75 0 01.75-.75h1.5a.75.75 0 010 1.5h-1.5a.75.75 0 01-.75-.75Z" clip-rule="evenodd"/></svg>';
/// Icon Data of sort_lowest

  static const String sort_lowest =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="m3 14 .53-.53 2.25-2.25a.75.75 0 00-1.06-1.061l-.97.97v-8.38a.75.75 0 00-1.5 0v8.38l-.97-.97a.75.75 0 10-1.06 1.06l2.25 2.25L3 14Zm5-9.25c0 .414.336.75.75.75h1.5a.75.75 0 000-1.5h-1.5a.75.75 0 00-.75.75Zm.75 4a.75.75 0 110-1.5h4.5a.75.75 0 010 1.5h-4.5Zm0 3.25a.75.75 0 010-1.5h6.5a.75.75 0 010 1.5h-6.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of spam

  static const String spam =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M10.103 1.529 8 0 5.897 1.529l-2.6-.001L2.496 4 .392 5.528 1.196 8l-.804 2.472L2.495 12l.803 2.473 2.6-.001L8 16l2.103-1.529 2.6.001.802-2.473 2.103-1.527L14.804 8l.804-2.472L13.505 4l-.803-2.473-2.6.001Zm1.51 1.5H9.614l-.395-.287L8 1.855l-1.22.887-.395.287H4.387l-.465 1.435-.15.464-.395.287-1.222.886.467 1.435.151.464-.15.464L2.154 9.9l1.222.886.395.287.15.464.466 1.436 1.509-.001h.488l.395.287 1.22.887 1.22-.887.395-.287h1.998l.465-1.435.15-.464.395-.287 1.222-.886-.467-1.435-.15-.465.15-.464.467-1.435-1.22-.886-.396-.287-.15-.464-.466-1.436ZM9 11a1 1 0 11-2 0 1 1 0 012 0Zm-.25-6.25a.75.75 0 00-1.5 0v3.5a.75.75 0 001.5 0v-3.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of stage_all

  static const String stage_all =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M8.75 3a.75.75 0 000 1.5h6.5a.75.75 0 000-1.5h-6.5Zm0 4.25a.75.75 0 000 1.5h6.5a.75.75 0 000-1.5h-6.5Zm-.75 5a.75.75 0 01.75-.75h6.5a.75.75 0 010 1.5h-6.5a.75.75 0 01-.75-.75ZM6.28 7.28a.75.75 0 00-1.06-1.06L2.5 8.94 1.28 7.72A.75.75 0 00.22 8.78l1.75 1.75a.75.75 0 001.06 0l3.25-3.25Z" clip-rule="evenodd"/></svg>';
/// Icon Data of star

  static const String star =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" d="M7.454 1.694a.591.591 0 011.092 0l1.585 3.81a.25.25 0 00.21.154l4.114.33a.591.591 0 01.338 1.038L11.658 9.71a.25.25 0 00-.08.247l.957 4.015a.591.591 0 01-.883.641l-3.522-2.15a.25.25 0 00-.26 0l-3.522 2.15a.591.591 0 01-.883-.641l.957-4.015a.25.25 0 00-.08-.247L1.207 7.026a.591.591 0 01.338-1.038l4.113-.33a.25.25 0 00.211-.153l1.585-3.81Z"/></svg>';
/// Icon Data of star_o

  static const String star_o =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="m7.189 2.332-.001.003-1.319 3.17a.25.25 0 01-.21.153l-3.423.274h-.003l-.095.008-.593.048a.591.591 0 00-.338 1.038l.452.387.073.062.002.002 2.608 2.234a.25.25 0 01.08.247l-.796 3.34-.001.003-.022.093-.138.579a.591.591 0 00.883.641l.507-.31.082-.05.003-.001 2.93-1.79a.25.25 0 01.26 0l2.93 1.79.003.002.082.05.507.31a.591.591 0 00.883-.642l-.138-.579-.022-.093v-.003l-.797-3.34a.25.25 0 01.08-.247l2.608-2.234.002-.002.073-.062.452-.387a.591.591 0 00-.338-1.038l-.593-.048-.095-.008h-.003l-3.422-.274a.25.25 0 01-.211-.153l-1.319-3.17-.001-.003-.037-.089-.228-.549a.591.591 0 00-1.092 0l-.228.55-.037.088ZM8 4.288 7.254 6.08a1.75 1.75 0 01-1.476 1.072l-1.935.155L5.317 8.57a1.75 1.75 0 01.564 1.736l-.45 1.888 1.657-1.012a1.75 1.75 0 011.824 0l1.657 1.012-.45-1.889a1.75 1.75 0 01.564-1.735l1.474-1.263-1.935-.155A1.75 1.75 0 018.746 6.08L8 4.288Z" clip-rule="evenodd"/></svg>';
/// Icon Data of status

  static const String status =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M15.941 7.033a8 8 0 01-14.784 5.112.75.75 0 111.283-.778 6.5 6.5 0 108.922-8.93.75.75 0 01.776-1.284 8 8 0 013.803 5.88ZM9 1a1 1 0 11-2 0 1 1 0 012 0ZM2.804 5a1 1 0 10-1.732-1 1 1 0 001.732 1ZM1 7a1 1 0 110 2 1 1 0 010-2Zm4-4.196a1 1 0 10-1-1.732 1 1 0 001 1.732Z" clip-rule="evenodd"/></svg>';
/// Icon Data of status_active

  static const String status_active =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="12" height="12" preserveAspectRatio="xMidYMid meet" viewBox="0 0 12 12"><circle cx="6" cy="6" r="6" fill="currentColor"/></svg>';
/// Icon Data of status_alert

  static const String status_alert =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="12" height="12" preserveAspectRatio="xMidYMid meet" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M12 6A6 6 0 110 6a6 6 0 0112 0zM5 3a1 1 0 012 0v3a1 1 0 01-2 0V3zm1 5a1 1 0 100 2 1 1 0 000-2z" clip-rule="evenodd"/></svg>';
/// Icon Data of status_cancelled

  static const String status_cancelled =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="12" height="12" preserveAspectRatio="xMidYMid meet" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M6 12A6 6 0 106 0a6 6 0 000 12zM4.28 3.22a.75.75 0 00-1.06 1.06l4.5 4.5a.75.75 0 001.06-1.06l-4.5-4.5z" clip-rule="evenodd"/></svg>';
/// Icon Data of status_closed

  static const String status_closed =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" d="m7.536 8.657 2.828-2.83a1 1 0 011.414 1.416l-3.535 3.535a1 1 0 01-1.415.001l-2.12-2.12a1 1 0 111.413-1.415zM8 16A8 8 0 118 0a8 8 0 010 16zm0-2A6 6 0 108 2a6 6 0 000 12z"/></svg>';
/// Icon Data of status_health

  static const String status_health =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M7.999 1a.75.75 0 01.715.521L12 11.79l1.286-4.018A.75.75 0 0114 7.25h1.25a.75.75 0 010 1.5h-.703l-1.833 5.729a.75.75 0 01-1.428 0L8.005 4.226l-2.29 7.25a.75.75 0 01-1.42.03L3.031 8.03l-.07.208a.75.75 0 01-.711.513H.75a.75.75 0 010-1.5h.96l.578-1.737a.75.75 0 011.417-.02L4.95 8.919l2.335-7.394A.75.75 0 017.999 1Z" clip-rule="evenodd"/></svg>';
/// Icon Data of status_neutral

  static const String status_neutral =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="12" height="12" preserveAspectRatio="xMidYMid meet" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M6 12A6 6 0 106 0a6 6 0 000 12zM3 4.75A.75.75 0 013.75 4h4.5a.75.75 0 010 1.5h-4.5A.75.75 0 013 4.75zm0 2.5a.75.75 0 01.75-.75h3.5a.75.75 0 010 1.5h-3.5A.75.75 0 013 7.25z" clip-rule="evenodd"/></svg>';
/// Icon Data of status_paused

  static const String status_paused =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="12" height="12" preserveAspectRatio="xMidYMid meet" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M12 6A6 6 0 110 6a6 6 0 0112 0zM3 3.5a.5.5 0 01.5-.5h1a.5.5 0 01.5.5v5a.5.5 0 01-.5.5h-1a.5.5 0 01-.5-.5v-5zM7.5 3a.5.5 0 00-.5.5v5a.5.5 0 00.5.5h1a.5.5 0 00.5-.5v-5a.5.5 0 00-.5-.5h-1z" clip-rule="evenodd"/></svg>';
/// Icon Data of status_preparing_borderless

  static const String status_preparing_borderless =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><g fill="currentColor" fill-rule="evenodd"><circle cx="8" cy="8" r="2"/><circle cx="14" cy="8" r="2"/><circle cx="2" cy="8" r="2"/></g></svg>';
/// Icon Data of status_stopped

  static const String status_stopped =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="12" height="12" preserveAspectRatio="xMidYMid meet" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M6 12A6 6 0 106 0a6 6 0 000 12zM3.5 3a.5.5 0 00-.5.5v5a.5.5 0 00.5.5h5a.5.5 0 00.5-.5v-5a.5.5 0 00-.5-.5h-5z" clip-rule="evenodd"/></svg>';
/// Icon Data of status_waiting

  static const String status_waiting =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="12" height="12" preserveAspectRatio="xMidYMid meet" viewBox="0 0 12 12"><path fill="currentColor" fill-rule="evenodd" d="M6 10a4 4 0 100-8 4 4 0 000 8zm0 2A6 6 0 106 0a6 6 0 000 12z" clip-rule="evenodd"/></svg>';
/// Icon Data of stop

  static const String stop =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><rect width="10" height="10" x="3" y="3" fill="currentColor" rx="2"/></svg>';
/// Icon Data of strikethrough

  static const String strikethrough =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M7.886 1a3.136 3.136 0 00-2.41 5.144L6.4 7.25H2.75a.75.75 0 000 1.5h4.899l1.722 2.066A1.636 1.636 0 018.114 13.5H8a1.75 1.75 0 01-1.75-1.75.75.75 0 00-1.5 0A3.25 3.25 0 008 15h.114a3.136 3.136 0 002.41-5.144L9.6 8.75h3.649a.75.75 0 000-1.5H8.351L6.63 5.184A1.636 1.636 0 017.886 2.5H8c.966 0 1.75.784 1.75 1.75a.75.75 0 001.5 0A3.25 3.25 0 008 1h-.114Z" clip-rule="evenodd"/></svg>';
/// Icon Data of subgroup

  static const String subgroup =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M6 4a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0Zm1.5 0a3 3 0 11-6 0 3 3 0 016 0Zm7 4a3 3 0 11-6 0 3 3 0 016 0Zm-10 5.5a1.5 1.5 0 100-3 1.5 1.5 0 000 3Zm0 1.5a3 3 0 100-6 3 3 0 000 6Z" clip-rule="evenodd"/></svg>';
/// Icon Data of subscript

  static const String subscript =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M2.22 4.22a.75.75 0 011.06 0L6 6.94l2.72-2.72a.75.75 0 011.06 1.06L7.06 8l2.72 2.72a.75.75 0 11-1.06 1.06L6 9.06l-2.72 2.72a.75.75 0 01-1.06-1.06L4.94 8 2.22 5.28a.75.75 0 010-1.06Zm10.407 6.782H11V10h1.627c.362 0 .71.144.967.402a1.392 1.392 0 01-.17 2.11l-.675.486h1.232V14H11v-.974l1.847-1.33a.385.385 0 00.047-.583.378.378 0 00-.267-.111Z" clip-rule="evenodd"/></svg>';
/// Icon Data of substitute

  static const String substitute =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="m0 5 .53-.53 2.25-2.25a.75.75 0 011.06 1.06l-.969.97h10.38a.75.75 0 010 1.5H2.87l.97.97a.75.75 0 01-1.06 1.06L.53 5.53 0 5Zm16 6-.53-.53-2.25-2.25a.75.75 0 10-1.061 1.06l.97.97H2.748a.75.75 0 000 1.5h10.38l-.97.97a.75.75 0 101.06 1.06l2.25-2.25L16 11Z" clip-rule="evenodd"/></svg>';
/// Icon Data of superscript

  static const String superscript =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M12.627 3.002H11V2h1.627c.362 0 .71.144.967.402a1.392 1.392 0 01-.17 2.11l-.675.486h1.232V6H11v-.974l1.847-1.33a.385.385 0 00.047-.583.378.378 0 00-.267-.111ZM2.22 4.22a.75.75 0 011.06 0L6 6.94l2.72-2.72a.75.75 0 011.06 1.06L7.06 8l2.72 2.72a.75.75 0 11-1.06 1.06L6 9.06l-2.72 2.72a.75.75 0 01-1.06-1.06L4.94 8 2.22 5.28a.75.75 0 010-1.06Z" clip-rule="evenodd"/></svg>';
/// Icon Data of symlink

  static const String symlink =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" d="M10 12V9H6a3 3 0 00-3 3v2a5 5 0 013-9h4V2l5 5-5 5Z"/></svg>';
/// Icon Data of table

  static const String table =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M2.5 5V3h11v2h-11Zm0 1.5v3h4.75v-3H2.5Zm6.25 0v3h4.75v-3H8.75ZM2.5 11h4.75v3H2.5v-3Zm6.25 0v3h4.75v-3H8.75ZM1 2.5a1 1 0 011-1h12a1 1 0 011 1v12a1 1 0 01-1 1H2a1 1 0 01-1-1v-12Z" clip-rule="evenodd"/></svg>';
/// Icon Data of tablet

  static const String tablet =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M12 1.5H4a.5.5 0 00-.5.5v1h9V2a.5.5 0 00-.5-.5Zm.5 3h-9V14a.5.5 0 00.5.5h8a.5.5 0 00.5-.5V4.5ZM4 0a2 2 0 00-2 2v12a2 2 0 002 2h8a2 2 0 002-2V2a2 2 0 00-2-2H4Zm1 12.25a.75.75 0 01.75-.75h4.75a.75.75 0 010 1.5H5.75a.75.75 0 01-.75-.75Z" clip-rule="evenodd"/></svg>';
/// Icon Data of tachometer

  static const String tachometer =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M13.545 2.1a.75.75 0 01.274 1.025l-3.472 6.007a3 3 0 11-1.208-.908l1.759-3.042a6.5 6.5 0 00-2.148-.639V5a.75.75 0 11-1.5 0v-.457a6.5 6.5 0 00-1.829.49l.229.396a.75.75 0 11-1.3.75l-.228-.396a6.5 6.5 0 00-1.339 1.34l.396.227a.75.75 0 01-.75 1.3l-.396-.229a6.5 6.5 0 00-.498 1.905.75.75 0 01-1.492-.155A8 8 0 0111.65 3.88l.87-1.506a.75.75 0 011.025-.274Zm-.107 4.047a.75.75 0 011.047.169 8 8 0 011.51 4.963.75.75 0 11-1.499-.052 6.5 6.5 0 00-1.227-4.033.75.75 0 01.17-1.047ZM9.5 11a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of tag

  static const String tag =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M11.172 5.5a.5.5 0 01.353.146l1.06-1.06-1.06 1.06L13.88 8l-2.354 2.354a.5.5 0 01-.353.146H2a.5.5 0 01-.5-.5V6a.5.5 0 01.5-.5h9.172Zm3.767 1.44-2.353-2.354A2 2 0 0011.172 4H2a2 2 0 00-2 2v4a2 2 0 002 2h9.172a2 2 0 001.414-.586l2.353-2.353L16 8l-1.06-1.06Zm-8.189.31a.75.75 0 000 1.5h3.5a.75.75 0 000-1.5h-3.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of tanuki

  static const String tanuki =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="m14.329 6.922-.02-.053-1.239-3.232-.668 2.044-.337 1.035H3.936L3.598 5.68l-.667-2.043-1.24 3.237-.005.01-.004.01-.018.045a2.543 2.543 0 00.848 2.93l.005.004.01.008.01.008.009.006 3.302 2.474.004.004 1.644 1.242.5.379.501-.379 1.644-1.242.006-.004 3.325-2.489.007-.005.009-.007a2.543 2.543 0 00.844-2.939l-.003-.008Zm.053 4.152-3.337 2.497-1.643 1.242-.999.755a.675.675 0 01-.813 0l-.999-.755-1.643-1.242-3.313-2.483-.018-.014-.008-.006A4.043 4.043 0 01.267 6.395l.023-.057 1.9-4.963.013-.033.081-.212.023-.06.158-.413A.565.565 0 012.941.3a.576.576 0 01.613.415l.134.41.005.017.078.239.011.033 1.242 3.802h5.953l1.241-3.802.011-.033.078-.24.006-.016.133-.41a.575.575 0 011.088-.058l.157.409.025.064.08.211.013.033 1.903 4.963.022.058a4.043 4.043 0 01-1.343 4.671l-.01.008Z" clip-rule="evenodd"/></svg>';
/// Icon Data of tanuki_verified

  static const String tanuki_verified =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="m15.712 6.337.022.058a4.043 4.043 0 01-1.343 4.671l-.01.008-3.336 2.497-1.643 1.242-.999.755a.675.675 0 01-.813 0l-.999-.755-1.643-1.242-3.313-2.483-.018-.014-.008-.007A4.043 4.043 0 01.267 6.395l.023-.057L2.466.657a.565.565 0 01.475-.358.576.576 0 01.613.416l1.47 4.5h5.953l1.47-4.5a.575.575 0 011.087-.058l2.178 5.68ZM11.28 8.53a.75.75 0 00-1.06-1.06L7.5 10.19 6.28 8.97a.75.75 0 10-1.06 1.06l1.75 1.75a.75.75 0 001.06 0l3.25-3.25Z" clip-rule="evenodd"/></svg>';
/// Icon Data of task_done

  static const String task_done =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M3 13.5a.5.5 0 01-.5-.5V3a.5.5 0 01.5-.5h9.25a.75.75 0 000-1.5H3a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2V9.75a.75.75 0 00-1.5 0V13a.5.5 0 01-.5.5H3Zm12.78-8.82a.75.75 0 00-1.06-1.06L9.162 9.177 7.289 7.241a.75.75 0 10-1.078 1.043l2.403 2.484a.75.75 0 001.07.01L15.78 4.68Z" clip-rule="evenodd"/></svg>';
/// Icon Data of template

  static const String template =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M13.5 2.5h-11V5h11V2.5Zm0 4h-7v7h7v-7ZM5 6.5H2.5v7H5v-7ZM2.5 1H1v14h14V1H2.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of terminal

  static const String terminal =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M14 3.5H2a.5.5 0 00-.5.5v8a.5.5 0 00.5.5h12a.5.5 0 00.5-.5V4a.5.5 0 00-.5-.5ZM2 2a2 2 0 00-2 2v8a2 2 0 002 2h12a2 2 0 002-2V4a2 2 0 00-2-2H2Zm5 8.25a.75.75 0 01.75-.75h4.5a.75.75 0 010 1.5h-4.5a.75.75 0 01-.75-.75ZM4.28 5.22a.75.75 0 00-1.06 1.06L4.94 8 3.22 9.72a.75.75 0 101.06 1.06l2.25-2.25.53-.53-.53-.53-2.25-2.25Z" clip-rule="evenodd"/></svg>';
/// Icon Data of terraform

  static const String terraform =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" d="M5.321 7.576 1 5.051V0l4.321 2.525v5.051ZM10.117 16l-4.321-2.525V8.424l4.321 2.525V16ZM5.796 2.819l4.321 2.528v5.048L5.796 7.869V2.82Zm4.796 7.576 4.321-2.523V2.819l-4.321 2.528v5.048Z"/></svg>';
/// Icon Data of text_description

  static const String text_description =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M0 3.75A.75.75 0 01.75 3h14.5a.75.75 0 010 1.5H.75A.75.75 0 010 3.75ZM0 8a.75.75 0 01.75-.75h14.5a.75.75 0 010 1.5H.75A.75.75 0 010 8Zm.75 3.5a.75.75 0 000 1.5h9.5a.75.75 0 000-1.5H.75Z" clip-rule="evenodd"/></svg>';
/// Icon Data of thumb_down

  static const String thumb_down =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="m10.72 13.402-.27-.902L10 11h3.989a2 2 0 001.932-2.517l-1.19-4.448A4 4 0 009.852 1.2L3 3H0v8h3l4.39 4.39c1.47 1.47 3.928.002 3.33-1.988ZM3 9.5H1.5v-5H3v5Zm7.232-6.85L4.5 4.157v6.222l3.951 3.951c.12.119.22.149.296.155a.533.533 0 00.314-.08.533.533 0 00.22-.238.466.466 0 00.003-.334l-.72-2.402-.58-1.931h6.005a.5.5 0 00.483-.63l-1.19-4.447a2.5 2.5 0 00-3.05-1.772Z" clip-rule="evenodd"/></svg>';
/// Icon Data of thumb_up

  static const String thumb_up =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="m10.72 2.598-.27.902L10 5h3.989a2 2 0 011.932 2.517l-1.19 4.448a4 4 0 01-4.88 2.835L3 13H0V5h3L7.39.61c1.47-1.47 3.928-.002 3.33 1.988ZM3 6.5H1.5v5H3v-5Zm7.232 6.85L4.5 11.843V5.621L8.451 1.67a.466.466 0 01.296-.155.533.533 0 01.314.08c.11.065.183.154.22.238.03.07.051.173.003.334l-.72 2.402-.58 1.931h6.005a.5.5 0 01.483.63l-1.19 4.447a2.5 2.5 0 01-3.05 1.773Z" clip-rule="evenodd"/></svg>';
/// Icon Data of thumbtack

  static const String thumbtack =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M11.28 1.22a.75.75 0 00-1.26.7L6.69 5.25H4.206c-1.114 0-1.671 1.346-.884 2.134l1.911 1.911-3.72 4.135A2 2 0 001 14.768V15h.233a2 2 0 001.337-.513l4.135-3.721 1.911 1.91c.788.788 2.134.23 2.134-.883V9.31l3.33-3.33a.75.75 0 00.7-1.261l-.603-.604-2.293-2.293-.604-.603ZM12.94 5 11 3.06 8.06 6 10 7.94 12.94 5ZM6.69 6.75l2.56 2.56v1.88L4.81 6.75h1.88Z" clip-rule="evenodd"/></svg>';
/// Icon Data of time_out

  static const String time_out =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M6.229.199a8 8 0 019.727 6.964.75.75 0 01-1.492.157 6.5 6.5 0 10-7.132 7.146.75.75 0 11-.154 1.492 8 8 0 01-.95-15.76ZM8 3a.75.75 0 01.75.75V9h-4a.75.75 0 010-1.5h2.5V3.75A.75.75 0 018 3Zm2.22 7.22a.75.75 0 011.06 0L13 11.94l1.72-1.72a.75.75 0 111.06 1.06L14.06 13l1.72 1.72a.75.75 0 11-1.06 1.06L13 14.06l-1.72 1.72a.75.75 0 11-1.06-1.06L11.94 13l-1.72-1.72a.75.75 0 010-1.06Z" clip-rule="evenodd"/></svg>';
/// Icon Data of timer

  static const String timer =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M6 1a1 1 0 011-1h2a1 1 0 010 2v.07a6.958 6.958 0 012.812 1.058l.908-.908a.75.75 0 111.06 1.06l-.8.8A7 7 0 117 2.07V2a1 1 0 01-1-1Zm7.5 8a5.5 5.5 0 11-11 0 5.5 5.5 0 0111 0ZM8.75 5.75a.75.75 0 00-1.5 0v3.56l.22.22 2.254 2.254a.75.75 0 101.06-1.06L8.75 8.689V5.75Z" clip-rule="evenodd"/></svg>';
/// Icon Data of title

  static const String title =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M.75 2a.75.75 0 00-.75.75v1.5a.75.75 0 001.5 0V3.5h2.75v9h-.5a.75.75 0 000 1.5h2.5a.75.75 0 000-1.5h-.5v-9H8.5v.75a.75.75 0 001.5 0v-1.5A.75.75 0 009.25 2H.75ZM8 7.75A.75.75 0 018.75 7h6.5a.75.75 0 010 1.5h-6.5A.75.75 0 018 7.75Zm0 3.5a.75.75 0 01.75-.75h6.5a.75.75 0 010 1.5h-6.5a.75.75 0 01-.75-.75Z" clip-rule="evenodd"/></svg>';
/// Icon Data of todo_add

  static const String todo_add =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M13 6a.75.75 0 01-.75-.75v-1.5h-1.5a.75.75 0 010-1.5h1.5V.75a.75.75 0 011.5 0v1.5h1.5a.75.75 0 010 1.5h-1.5v1.5A.75.75 0 0113 6ZM3 13.5a.5.5 0 01-.5-.5V3a.5.5 0 01.5-.5h4.25a.75.75 0 000-1.5H3a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2V8.75a.75.75 0 00-1.5 0V13a.5.5 0 01-.5.5H3Z" clip-rule="evenodd"/></svg>';
/// Icon Data of todo_done

  static const String todo_done =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M3 13.5a.5.5 0 01-.5-.5V3a.5.5 0 01.5-.5h9.25a.75.75 0 000-1.5H3a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2V9.75a.75.75 0 00-1.5 0V13a.5.5 0 01-.5.5H3Zm12.78-8.82a.75.75 0 00-1.06-1.06L9.162 9.177 7.289 7.241a.75.75 0 10-1.078 1.043l2.403 2.484a.75.75 0 001.07.01L15.78 4.68Z" clip-rule="evenodd"/></svg>';
/// Icon Data of token

  static const String token =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M5 4.5h6a3.5 3.5 0 110 7H5a3.5 3.5 0 110-7ZM0 8a5 5 0 015-5h6a5 5 0 010 10H5a5 5 0 01-5-5Zm11 1a1 1 0 100-2 1 1 0 000 2ZM9 8a1 1 0 11-2 0 1 1 0 012 0ZM5 9a1 1 0 100-2 1 1 0 000 2Z" clip-rule="evenodd"/></svg>';
/// Icon Data of trend_down

  static const String trend_down =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="12" height="12" preserveAspectRatio="xMidYMid meet" viewBox="0 0 12 12"><path fill="currentColor" d="M5.143 9.847a1 1 0 001.715 0l3.999-6.665a1 1 0 00-.858-1.515H2.001a1 1 0 00-.858 1.515l4 6.665z"/></svg>';
/// Icon Data of trend_static

  static const String trend_static =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="12" height="12" preserveAspectRatio="xMidYMid meet" viewBox="0 0 12 12"><path fill="currentColor" d="M9.847 5.142a1 1 0 010 1.715l-6.665 4a1 1 0 01-1.515-.858V2.001a1 1 0 011.515-.858l6.665 4z"/></svg>';
/// Icon Data of trend_up

  static const String trend_up =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="12" height="12" preserveAspectRatio="xMidYMid meet" viewBox="0 0 12 12"><path fill="currentColor" d="M5.143 2.153a1 1 0 011.715 0l3.999 6.665a1 1 0 01-.858 1.515H2.001a1 1 0 01-.858-1.515l4-6.665z"/></svg>';
/// Icon Data of trigger_source

  static const String trigger_source =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M7.25.75a.75.75 0 011.5 0V2c0 2.9 2.35 5.25 5.25 5.25h1.25a.75.75 0 010 1.5H14A5.25 5.25 0 008.75 14v1.25a.75.75 0 01-1.5 0V14c0-2.9-2.35-5.25-5.25-5.25H.75a.75.75 0 010-1.5H2C4.9 7.25 7.25 4.9 7.25 2V.75ZM5.095 8A6.78 6.78 0 018 10.905 6.78 6.78 0 0110.905 8 6.78 6.78 0 018 5.095 6.78 6.78 0 015.095 8Z" clip-rule="evenodd"/></svg>';
/// Icon Data of twitter

  static const String twitter =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" d="M5.032 14.286c6.037 0 9.34-4.837 9.34-9.032 0-.137 0-.274-.01-.41A6.56 6.56 0 0016 3.2c-.6.256-1.235.425-1.885.5a3.207 3.207 0 001.443-1.757c-.645.37-1.35.63-2.085.77a3.322 3.322 0 00-1.862-.958 3.384 3.384 0 00-2.082.334 3.223 3.223 0 00-1.442 1.49 3.08 3.08 0 00-.208 2.03 9.57 9.57 0 01-3.747-.963 9.269 9.269 0 01-3.018-2.354 3.086 3.086 0 00-.36 2.314c.189.787.68 1.475 1.376 1.924a3.344 3.344 0 01-1.49-.398v.04c0 .734.263 1.444.743 2.01a3.3 3.3 0 001.89 1.102c-.483.128-.99.146-1.482.055a3.19 3.19 0 001.168 1.577 3.36 3.36 0 001.9.627A6.732 6.732 0 010 12.86a9.527 9.527 0 005.032 1.423"/></svg>';
/// Icon Data of unapproval

  static const String unapproval =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4 6.5a1.5 1.5 0 100-3 1.5 1.5 0 000 3ZM7 5a2.99 2.99 0 01-.87 2.113A3.997 3.997 0 018 10.5V12a2 2 0 01-2 2H2a2 2 0 01-2-2v-1.5c0-1.427.747-2.679 1.87-3.387A3 3 0 117 5Zm-5.5 5.5a2.5 2.5 0 015 0V12a.5.5 0 01-.5.5H2a.5.5 0 01-.5-.5v-1.5Zm9.72-6.28a.75.75 0 011.06 0l1.22 1.22 1.22-1.22a.75.75 0 111.06 1.06L14.56 6.5l1.22 1.22a.75.75 0 01-1.06 1.06L13.5 7.56l-1.22 1.22a.75.75 0 11-1.06-1.06l1.22-1.22-1.22-1.22a.75.75 0 010-1.06Z" clip-rule="evenodd"/></svg>';
/// Icon Data of unassignee

  static const String unassignee =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M4 6.5a1.5 1.5 0 100-3 1.5 1.5 0 000 3ZM7 5a2.99 2.99 0 01-.87 2.113A3.997 3.997 0 018 10.5V12a2 2 0 01-2 2H2a2 2 0 01-2-2v-1.5c0-1.427.747-2.679 1.87-3.387A3 3 0 117 5Zm-5.5 5.5a2.5 2.5 0 015 0V12a.5.5 0 01-.5.5H2a.5.5 0 01-.5-.5v-1.5Zm9.25-4.25a.75.75 0 000 1.5h4.5a.75.75 0 000-1.5h-4.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of underline

  static const String underline =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M5.5 2.75a.75.75 0 00-1.5 0V7a4 4 0 108 0V2.75a.75.75 0 00-1.5 0V7a2.5 2.5 0 01-5 0V2.75ZM3.75 12.5a.75.75 0 000 1.5h8.5a.75.75 0 000-1.5h-8.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of unlink

  static const String unlink =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M6 1.75a.75.75 0 00-1.5 0V4.5H1.75a.75.75 0 000 1.5H6V1.75Zm5.151 1.16a1.371 1.371 0 011.94 1.939l-2.871 2.87a.75.75 0 101.06 1.061l2.871-2.87a2.871 2.871 0 00-4.06-4.061L7.22 4.719A.75.75 0 008.28 5.78l2.871-2.87ZM10 14.25a.75.75 0 001.5 0V11.5h2.75a.75.75 0 000-1.5H10v4.25ZM5.781 7.22a.75.75 0 010 1.06l-2.87 2.871a1.371 1.371 0 001.939 1.94l2.87-2.871a.75.75 0 111.061 1.06l-2.87 2.871a2.871 2.871 0 01-4.061-4.06L4.72 7.22a.75.75 0 011.061 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of unstage_all

  static const String unstage_all =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M0 3.75a.75.75 0 011.5 0v.943a5 5 0 11.67 7.246.75.75 0 11.924-1.182A3.5 3.5 0 102.088 6.5H4.25a.75.75 0 010 1.5H0V3.75Zm12 0a.75.75 0 01.75-.75h2.5a.75.75 0 010 1.5h-2.5a.75.75 0 01-.75-.75ZM12 8a.75.75 0 01.75-.75h2.5a.75.75 0 010 1.5h-2.5A.75.75 0 0112 8Zm.75 3.5a.75.75 0 000 1.5h2.5a.75.75 0 000-1.5h-2.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of upgrade

  static const String upgrade =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="m14 6-1.5-1.5-3.44-3.44L8 0 6.94 1.06 3.5 4.5 2 6h3v7h6V6h3ZM8 2.121 10.379 4.5H9.5v7h-3v-7h-.879L8 2.121ZM5.75 14.5a.75.75 0 000 1.5h4.5a.75.75 0 000-1.5h-4.5Z" clip-rule="evenodd"/></svg>';
/// Icon Data of upload

  static const String upload =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M11.78 5.841a.75.75 0 01-1.06 0l-1.97-1.97v7.379a.75.75 0 01-1.5 0V3.871l-1.97 1.97a.75.75 0 01-1.06-1.06l3.25-3.25L8 1l.53.53 3.25 3.25a.75.75 0 010 1.061ZM2.5 9.75a.75.75 0 00-1.5 0V13a2 2 0 002 2h10a2 2 0 002-2V9.75a.75.75 0 00-1.5 0V13a.5.5 0 01-.5.5H3a.5.5 0 01-.5-.5V9.75Z" clip-rule="evenodd"/></svg>';
/// Icon Data of user

  static const String user =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M10.5 5a2.5 2.5 0 11-5 0 2.5 2.5 0 015 0Zm.514 2.63a4 4 0 10-6.028 0A4.002 4.002 0 002 11.5V13a2 2 0 002 2h8a2 2 0 002-2v-1.5a4.002 4.002 0 00-2.986-3.87ZM8 9H6a2.5 2.5 0 00-2.5 2.5V13a.5.5 0 00.5.5h8a.5.5 0 00.5-.5v-1.5A2.5 2.5 0 0010 9H8Z" clip-rule="evenodd"/></svg>';
/// Icon Data of users

  static const String users =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M6.5 4a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0Zm.63 2.113a3 3 0 10-4.259 0A3.997 3.997 0 001 9.5V13a2 2 0 002 2h4c.597 0 1.134-.262 1.5-.677.366.415.903.677 1.5.677h3a2 2 0 002-2v-2c0-1.218-.622-2.29-1.565-2.917a2.5 2.5 0 10-3.87 0c-.241.16-.462.35-.656.564a4.005 4.005 0 00-1.78-2.534ZM5 7a2.5 2.5 0 00-2.5 2.5V13a.5.5 0 00.5.5h4a.5.5 0 00.5-.5V9.5A2.5 2.5 0 005 7Zm7.5-.5a1 1 0 11-2 0 1 1 0 012 0Zm-1 2.5a2 2 0 00-2 2v2a.5.5 0 00.5.5h3a.5.5 0 00.5-.5v-2a2 2 0 00-2-2Z" clip-rule="evenodd"/></svg>';
/// Icon Data of volume_up

  static const String volume_up =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M1.5 5H3l3.364-2.77A1 1 0 018 3.002v9.996a1 1 0 01-1.636.772L3 11H0V5h1.5ZM3 9.5H1.5v-3h2.038l.416-.342L6.5 4.061v7.878L3.954 9.842 3.538 9.5H3Zm9.457-7.267a.75.75 0 011.06-.026 7.999 7.999 0 010 11.586.75.75 0 01-1.034-1.086 6.5 6.5 0 000-9.414.75.75 0 01-.026-1.06Zm-2.292 2.034a.75.75 0 011.057-.09 5 5 0 01.001 7.645.75.75 0 11-.967-1.146 3.5 3.5 0 000-5.353.75.75 0 01-.091-1.056Z" clip-rule="evenodd"/></svg>';
/// Icon Data of warning

  static const String warning =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M8.429 2.746a.5.5 0 00-.858 0L1.58 12.743a.5.5 0 00.429.757h11.984a.5.5 0 00.43-.757L8.428 2.746Zm-2.144-.77C7.06.68 8.939.68 9.715 1.975l5.993 9.996c.799 1.333-.161 3.028-1.716 3.028H2.008C.453 15-.507 13.305.292 11.972l5.993-9.997ZM9 11.5a1 1 0 11-2 0 1 1 0 012 0Zm-.25-5.75a.75.75 0 00-1.5 0v3a.75.75 0 001.5 0v-3Z" clip-rule="evenodd"/></svg>';
/// Icon Data of warning_solid

  static const String warning_solid =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M6.285 1.975C7.06.68 8.939.68 9.715 1.975l5.993 9.997c.799 1.333-.161 3.028-1.716 3.028H2.008C.453 15-.507 13.305.292 11.972l5.993-9.997ZM8 5a.75.75 0 01.75.75v3a.75.75 0 01-1.5 0v-3A.75.75 0 018 5Zm1 6.5a1 1 0 11-2 0 1 1 0 012 0Z" clip-rule="evenodd"/></svg>';
/// Icon Data of weight

  static const String weight =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M9.25 3.75a1.25 1.25 0 11-2.5 0 1.25 1.25 0 012.5 0ZM10.45 5a2.75 2.75 0 10-4.9 0H3.82a1 1 0 00-.98.804l-1.6 8A1 1 0 002.22 15h11.56a1 1 0 00.98-1.196l-1.6-8A1 1 0 0012.18 5h-1.73ZM8 6.5H4.23l-1.4 7h10.34l-1.4-7H8Z" clip-rule="evenodd"/></svg>';
/// Icon Data of work

  static const String work =
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M6 1a1.75 1.75 0 00-1.75 1.75V4H3a2 2 0 00-2 2v7a2 2 0 002 2h10a2 2 0 002-2V6a2 2 0 00-2-2h-1.25V2.75A1.75 1.75 0 0010 1H6Zm4.25 3V2.75A.25.25 0 0010 2.5H6a.25.25 0 00-.25.25V4h4.5ZM3 5.5h10a.5.5 0 01.5.5v1h-11V6a.5.5 0 01.5-.5Zm-.5 3V13a.5.5 0 00.5.5h10a.5.5 0 00.5-.5V8.5H9V10H7V8.5H2.5Z" clip-rule="evenodd"/></svg>';
  static const List iconsList = [
    abuse,
    accessibility,
    account,
    admin,
    api,
    appearance,
    applications,
    approval,
    approval_solid,
    archive,
    arrow_down,
    arrow_left,
    arrow_right,
    arrow_up,
    assignee,
    at,
    attention,
    attention_solid,
    attention_solid_sm,
    autoplay,
    bitbucket,
    bold,
    book,
    bookmark,
    branch,
    branch_deleted,
    brand_zoom,
    bug,
    bulb,
    bullhorn,
    calendar,
    cancel,
    canceled_circle,
    car,
    chart,
    check,
    check_circle,
    check_circle_dashed,
    check_circle_filled,
    check_sm,
    cherry_pick_commit,
    chevron_double_lg_left,
    chevron_double_lg_right,
    chevron_down,
    chevron_left,
    chevron_lg_down,
    chevron_lg_left,
    chevron_lg_right,
    chevron_lg_up,
    chevron_right,
    chevron_up,
    clear,
    clear_all,
    clock,
    close,
    cloud_gear,
    code,
    collapse,
    collapse_left,
    collapse_right,
    comment,
    comment_dots,
    comment_next,
    comments,
    commit,
    comparison,
    connected,
    container_image,
    copy_to_clipboard,
    credit_card,
    dash,
    dashboard,
    deployments,
    details_block,
    disk,
    doc_changes,
    doc_chart,
    doc_code,
    doc_compressed,
    doc_expand,
    doc_image,
    doc_new,
    doc_symlink,
    doc_text,
    doc_versions,
    document,
    documents,
    dot_grid,
    download,
    drag,
    drag_horizontal,
    drag_vertical,
    dumbbell,
    duplicate,
    earth,
    ellipsis_h,
    ellipsis_v,
    entity_blocked,
    environment,
    epic,
    epic_closed,
    error,
    expand,
    expand_down,
    expand_left,
    expand_right,
    expand_up,
    expire,
    i_export,
    external_link,
    eye,
    eye_slash,
    face_neutral,
    face_unhappy,
    false_positive,
    feature_flag,
    feature_flag_disabled,
    file_addition,
    file_addition_solid,
    file_deletion,
    file_deletion_solid,
    file_modified,
    file_modified_solid,
    file_tree,
    filter,
    fire,
    first_contribution,
    flag,
    folder,
    folder_new,
    folder_o,
    folder_open,
    food,
    fork,
    formula,
    git,
    git_merge,
    github,
    go_back,
    google,
    group,
    hamburger,
    heading,
    heart,
    history,
    home,
    hook,
    hourglass,
    image_comment_dark,
    image_comment_light,
    i_import,
    incognito,
    information,
    information_o,
    infrastructure_registry,
    issue_block,
    issue_close,
    issue_closed,
    issue_new,
    issue_open_m,
    issue_type_enhancement,
    issue_type_feature,
    issue_type_feature_flag,
    issue_type_incident,
    issue_type_issue,
    issue_type_maintenance,
    issue_type_requirements,
    issue_type_task,
    issue_type_test_case,
    issue_update,
    issues,
    italic,
    iteration,
    key,
    kubernetes,
    kubernetes_agent,
    label,
    labels,
    leave,
    level_up,
    license,
    license_sm,
    link,
    linkedin,
    list_bulleted,
    list_indent,
    list_numbered,
    list_outdent,
    list_task,
    live_preview,
    live_stream,
    location,
    location_dot,
    lock,
    lock_open,
    log,
    long_arrow,
    mail,
    marquee_selection,
    maximize,
    media,
    media_broken,
    merge,
    merge_request,
    merge_request_close,
    merge_request_close_m,
    merge_request_open,
    messages,
    minimize,
    mobile,
    mobile_issue_close,
    monitor,
    monitor_lines,
    monitor_o,
    namespace,
    nature,
    notifications,
    notifications_off,
    object,
    overview,
    package,
    paper_airplane,
    paperclip,
    pause,
    pencil,
    pencil_square,
    pipeline,
    planning,
    play,
    plus,
    plus_square,
    plus_square_o,
    pod,
    podcast,
    power,
    preferences,
    profile,
    progress,
    project,
    push_rules,
    question,
    question_o,
    quote,
    redo,
    remove,
    remove_all,
    repeat,
    requirements,
    retry,
    review_checkmark,
    review_list,
    review_warning,
    rocket,
    rss,
    scale,
    scroll_down,
    scroll_handle,
    scroll_up,
    search,
    search_dot,
    search_minus,
    search_plus,
    search_sm,
    settings,
    severity_critical,
    severity_high,
    severity_info,
    severity_low,
    severity_medium,
    severity_unknown,
    share,
    shield,
    sidebar,
    skype,
    slight_frown,
    slight_smile,
    smart_card,
    smile,
    smiley,
    snippet,
    soft_unwrap,
    soft_wrap,
    sort_highest,
    sort_lowest,
    spam,
    stage_all,
    star,
    star_o,
    status,
    status_active,
    status_alert,
    status_cancelled,
    status_closed,
    status_health,
    status_neutral,
    status_paused,
    status_preparing_borderless,
    status_stopped,
    status_waiting,
    stop,
    strikethrough,
    subgroup,
    subscript,
    substitute,
    superscript,
    symlink,
    table,
    tablet,
    tachometer,
    tag,
    tanuki,
    tanuki_verified,
    task_done,
    template,
    terminal,
    terraform,
    text_description,
    thumb_down,
    thumb_up,
    thumbtack,
    time_out,
    timer,
    title,
    todo_add,
    todo_done,
    token,
    trend_down,
    trend_static,
    trend_up,
    trigger_source,
    twitter,
    unapproval,
    unassignee,
    underline,
    unlink,
    unstage_all,
    upgrade,
    upload,
    user,
    users,
    volume_up,
    warning,
    warning_solid,
    weight,
    work,
  ];
}
